        ...
        if twilio_client:
            try:
                ...
                twilio_client.messages.create(
                    body=f"{ben_name} souhaite vous ajouter comme gardien sur Chutex, l'application de teleassistance. Inscrivez-vous sur https://nora-health.preview.emergentagent.com pour veiller sur votre proche.",
                    from_=TWILIO_NUMBER,
                    to=cleaned,
                )
                sms_sent = True
...
async def create_prescription(data: PrescriptionCreate, user=Depends(get_current_user)):
    ...
    sub_link = "https://nora-health.preview.emergentagent.com/subscription"
    await send_sms(
        cleaned_phone,
