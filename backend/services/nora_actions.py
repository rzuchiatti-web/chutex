...
async def delete_exercise(uid: str, exercise_id: str) -> dict:
    """
    Delete an exercise. Only Nora-assigned exercises can be deleted.
    Never delete guardian/coach prescribed exercises.
    """
    ex = await db.pro_assigned_exercises.find_one({"id": exercise_id, "beneficiary_id": uid}, {"_id": 0})
    if not ex:
        return {"success": False, "message": "Exercice non trouve."}

    if not ex.get("nora_assigned"):
        prescriber = ex.get("professional_name", "un coach/gardien")
        return {
            "success": False,
            "reason": "prescription_protegee",
            "message": f"Impossible de supprimer cet exercice : il a ete prescrit par {prescriber}.",
        }

    await db.pro_assigned_exercises.delete_one({"id": exercise_id, "beneficiary_id": uid})
    return {
        "success": True,
        "title": ex.get("title", ""),
        "message": f"Exercice '{ex.get('title', '')}' supprime.",
    }


async def update_meal_plan(uid: str, user: dict, meals: list) -> dict:
    """
    Save a meal plan generated by Nora into the daily cache.
    meals: list of {type, label, name, description, calories, proteines_g, glucides_g, lipides_g, time, ingredients, recipe, prep_time}
    """
    today_str = datetime.now(timezone.utc).strftime("%Y-%m-%d")
    ...
    if existing and existing.get("recommendations"):
        ...
        recs["meals"] = meals
        if total_cal > 0:
            recs["daily_calories"] = total_cal
        recs["nora_meal_plan"] = True
        recs["meal_plan_at"] = datetime.now(timezone.utc).isoformat()
        await db.minceur_daily_cache.update_one(
            {"user_id": uid, "date": today_str},
            {"$set": {"recommendations": recs}}
        )
    else:
        total_p = sum(m.get("proteines_g", 0) for m in meals)
        total_g = sum(m.get("glucides_g", 0) for m in meals)
        total_l = sum(m.get("lipides_g", 0) for m in meals)
        await db.minceur_daily_cache.update_one(
            {"user_id": uid, "date": today_str},
            {"$set": {
                "user_id": uid,
                ...
                "recommendations": {
                    ...
                    "macros": {"proteines_g": total_p, "glucides_g": total_g, "lipides_g": total_l},
                    "meals": meals,
                    "nora_meal_plan": True,
                    "meal_plan_at": datetime.now(timezone.utc).isoformat(),
                },
            }},
            upsert=True,
        )
