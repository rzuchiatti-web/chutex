...
7. **Animated Dark Background** — Created `AnimatedDarkBg.tsx` reusable component with 3 violet/purple glow orbs with filter blur, slowly floating and pulsing (20-28s cycles) + vignette overlay. Applied to ALL pages: Dashboard, Health, Programs, Profile (both beneficiary and guardian).
8. **Guardian Dashboard** — Applied AnimatedDarkBg, changed alert card to show green background when no active alerts (matching beneficiary AlertBanner behavior).
9. **Force Dark Mode** — Removed light mode toggle, forced `isDark = true` by default on beneficiary dashboard.
10. **Daily Objectives Layout** — Changed from 2x2 grid to single column layout with larger images (64x64) and horizontal card design (image left, text right).
