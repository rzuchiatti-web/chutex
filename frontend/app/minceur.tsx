  ...
  return (
    <div data-testid="nora-analysis-overlay" style={{
      ...
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        @keyframes noraSlideUp{from{padding-top:30vh}to{padding-top:40px}}
      ` }} />

      <div style={{
        flex: 1, overflowY: 'auto', WebkitOverflowScrolling: 'touch',
        display: 'flex', flexDirection: 'column', alignItems: 'center',
        paddingTop: phase === 'intro' ? '30vh' : 40,
        ...
        {phase === 'intro' && (
          <div style={{ textAlign: 'center', animation: 'noraTextIn 0.6s ease 0.3s both' } as any}>
            ...
            <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.25)', marginTop: 8 }}>Votre bilan nutritionnel personnalise</div>
          </div>
        )}
        ...
        {(phase === 'typing' || phase === 'done') && (
          <div style={{ width: '100%', maxWidth: 380, animation: 'noraTextIn 0.5s ease both' } as any}>
            <div style={{ textAlign: 'center', marginBottom: 20 } as any}>
              <div style={{ fontSize: 16, fontWeight: 800, color: '#FFF' }}>Analyse de Nora</div>
            ...
            {phase === 'typing' && <span style={{ display: 'block', textAlign: 'center', color: 'rgba(255,255,255,0.12)', fontSize: 13 }}>|</span>}
          </div>
        )}
      </div>
...
function SwipePicker({ values, selected, onChange, unit, color }: { values: number[]; selected: number; onChange: (v: number) => void; unit: string; color: string }) {
  ...
  const IW = 54;
  const pickerId = useRef(Math.random().toString(36).slice(2, 8));
  ...
  // Initial centering — only on first mount
  useEffect(() => {
    isInit.current = true;
  ...
  // When selected changes externally (not from user scroll), re-center
  useEffect(() => {
    if (!isInit.current && !isTouching.current && selected !== lastUserValue.current) {
      scrollToSelected(selected);
      lastUserValue.current = selected;
    }
  }, [selected]);
  ...
  const onScroll = () => {
    ...
    // Only snap if THIS picker is being actively touched/scrolled
    ...
    snapTimer.current = setTimeout(() => {
      ...
      if (values[best] !== selected) {
        lastUserValue.current = values[best];
  ...
  const onMouseDown = () => { isTouching.current = true; };
  ...
  return (
    <div style={{ position: 'relative', height: 64, margin: '0 -18px' } as any}>
      ...
      <div ref={containerRef} onScroll={onScroll} onTouchStart={onTouchStart} onMouseDown={onMouseDown} style={{ display: 'flex', height: '100%', alignItems: 'center', overflowX: 'auto', scrollbarWidth: 'none', WebkitOverflowScrolling: 'touch', cursor: 'grab', userSelect: 'none' } as any}>
        ...
        {values.map(v => {
          ...
          return (
            <div key={v} ref={el => { itemRefs.current[v] = el; }} onClick={() => { isTouching.current = true; lastUserValue.current = v; onChange(v); setTimeout(() => { scrollToSelected(v); isTouching.current = false; }, 10); }} style={{ width: IW, flexShrink: 0, textAlign: 'center', cursor: 'pointer', padding: '8px 0' } as any}>
              <div style={{ fontSize: sel ? 24 : 14, fontWeight: sel ? 900 : 400, color: sel ? '#FFF' : 'rgba(255,255,255,0.15)', transition: 'all 0.15s', textShadow: sel ? `0 0 20px ${color}40` : 'none', lineHeight: 1.2 }}>
        ...
        <div style={{ maxWidth: 480, margin: '0 auto', padding: 'calc(env(safe-area-inset-top, 20px) + 12px) 20px 120px' } as any}>
          ...
          {!loading && data && (
            <>
              ...
              {/* ══ NORA BUTTON — between graph card and recommendations ══ */}
              {recs?.nora_insight && (
              ...
              {recs && (
                <div style={{ ...fade(0.2) } as any}>
                  ...
                  {recs.meals && <div data-testid="meals-section" style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 14 } as any}>
                    {recs.meals.map((meal: any, i: number) => { const tp = meal.type || ['breakfast', 'lunch', 'snack', 'dinner'][i] || 'lunch'; const meta = MM[tp] || MM.lunch; const col = MC[tp] || '#FFF'; const dn = tracked[`meal_${i}`]; const isProMeal = meal.source === 'pro' && meal.assignment_id; return (
