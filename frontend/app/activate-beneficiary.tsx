...
const WebInput = ({ label, val, onChange, placeholder, type }: any) => Platform.OS === 'web' ? (
  <div style={{ marginBottom: 14 }}>
    <div style={{ fontSize: 11, fontWeight: '700', color: colors.textSecondary, marginBottom: 6, textTransform: 'uppercase' as any, letterSpacing: 1 }}>{label}</div>
...
export default function ActivateBeneficiaryScreen() {
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: colors.background }} testID="activate-beneficiary-screen">
      <View style={{ flexDirection: 'row', alignItems: 'center', paddingHorizontal: 16, paddingVertical: 14 }}>
        ...
        <Text style={{ flex: 1, fontSize: 22, fontWeight: '900', color: colors.textPrimary }}>Espace beneficiaire</Text>
      </View>
      ...
      <ScrollView contentContainerStyle={{ paddingHorizontal: 16, paddingBottom: 40 }}>
        ...
        <Text style={{ fontSize: 11, color: colors.textSecondary, textAlign: 'center', marginBottom: 20 }}>Etape {step + 1} / 2</Text>
        ...
        {step === 0 && (
          <GlassCard>
            <Text style={{ fontSize: 18, fontWeight: '800', color: colors.textPrimary, marginBottom: 16 }}>Informations personnelles</Text>
            <WebInput label="Date de naissance" val={dob} onChange={setDob} placeholder="JJ/MM/AAAA" />
            <Text style={{ fontSize: 11, fontWeight: '700', color: colors.textSecondary, marginBottom: 8, textTransform: 'uppercase', letterSpacing: 1 }}>Genre</Text>
            ...
            <Text style={{ fontSize: 11, fontWeight: '700', color: colors.textSecondary, marginBottom: 8, textTransform: 'uppercase', letterSpacing: 1 }}>Groupe sanguin</Text>
        ...
        {step === 1 && (
          <GlassCard>
            <Text style={{ fontSize: 18, fontWeight: '800', color: colors.textPrimary, marginBottom: 16 }}>Informations medicales</Text>
            ...
            <Text style={{ fontSize: 16, fontWeight: '800', color: colors.textPrimary, marginTop: 8, marginBottom: 12 }}>Contact d'urgence</Text>
        ...
        <View style={{ flexDirection: 'row', gap: 12, marginTop: 8 }}>
          {step > 0 && <TouchableOpacity style={{ flex: 1, paddingVertical: 14, borderRadius: 9999, backgroundColor: 'rgba(0,0,0,0.06)', alignItems: 'center' }} onPress={() => setStep(0)}>
            <Text style={{ fontSize: 14, fontWeight: '700', color: colors.textSecondary }}>Retour</Text>
          </TouchableOpacity>}
          {step === 0 ? (
            <TouchableOpacity testID="next-step-btn" style={{ flex: 1, backgroundColor: colors.background, paddingVertical: 14, borderRadius: 9999, alignItems: 'center' }} onPress={() => setStep(1)}>
              <Text style={{ color: '#FFF', fontSize: 15, fontWeight: '800' }}>SUIVANT</Text>
            </TouchableOpacity>
          ) : (
            <TouchableOpacity testID="activate-btn" style={{ flex: 1, backgroundColor: colors.background, paddingVertical: 14, borderRadius: 9999, alignItems: 'center' }} onPress={submit} disabled={saving}>
              {saving ? <ActivityIndicator color={colors.textPrimary} /> : <Text style={{ color: '#FFF', fontSize: 15, fontWeight: '800' }}>ACTIVER</Text>}
            </TouchableOpacity>
          )}
        </View>
      </ScrollView>
