...
const WebInput = ({ label, val, onChange, placeholder, type }: any) => Platform.OS === 'web' ? (
  <div style={{ marginBottom: 14 }}>
    <div style={{ fontSize: 11, fontWeight: '700', color: '#5A6068', marginBottom: 6, textTransform: 'uppercase' as any, letterSpacing: 1 }}>{label}</div>
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: '#F5F6F8' }} testID="activate-beneficiary-screen">
      ...
      <ScrollView contentContainerStyle={{ paddingHorizontal: 16, paddingBottom: 40 }}>
        ...
        <Text style={{ fontSize: 11, color: '#5A6068', textAlign: 'center', marginBottom: 20 }}>Etape {step + 1} / 2</Text>
        ...
        {step === 0 && (
          <GlassCard>
            <Text style={{ fontSize: 18, fontWeight: '800', color: '#1A1D21', marginBottom: 16 }}>Informations personnelles</Text>
            <WebInput label="Date de naissance" val={dob} onChange={setDob} placeholder="JJ/MM/AAAA" />
            <Text style={{ fontSize: 11, fontWeight: '700', color: '#5A6068', marginBottom: 8, textTransform: 'uppercase', letterSpacing: 1 }}>Genre</Text>
            ...
            <Text style={{ fontSize: 11, fontWeight: '700', color: '#5A6068', marginBottom: 8, textTransform: 'uppercase', letterSpacing: 1 }}>Groupe sanguin</Text>
        ...
        {step === 1 && (
          <GlassCard>
            <Text style={{ fontSize: 18, fontWeight: '800', color: '#1A1D21', marginBottom: 16 }}>Informations medicales</Text>
            ...
            <Text style={{ fontSize: 16, fontWeight: '800', color: '#1A1D21', marginTop: 8, marginBottom: 12 }}>Contact d'urgence</Text>
        ...
        <View style={{ flexDirection: 'row', gap: 12, marginTop: 8 }}>
          {step > 0 && <TouchableOpacity style={{ flex: 1, paddingVertical: 14, borderRadius: 9999, backgroundColor: 'rgba(0,0,0,0.06)', alignItems: 'center' }} onPress={() => setStep(0)}>
            <Text style={{ fontSize: 14, fontWeight: '700', color: '#5A6068' }}>Retour</Text>
          </TouchableOpacity>}
          {step === 0 ? (
            ...
            <TouchableOpacity testID="activate-btn" style={{ flex: 1, backgroundColor: '#F5F6F8', paddingVertical: 14, borderRadius: 9999, alignItems: 'center' }} onPress={submit} disabled={saving}>
              {saving ? <ActivityIndicator color="#1A1D21" /> : <Text style={{ color: '#FFF', fontSize: 15, fontWeight: '800' }}>ACTIVER</Text>}
            </TouchableOpacity>
          )}
        </View>
      </ScrollView>
