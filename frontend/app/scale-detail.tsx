...
export default function ScaleDetailScreen() {
  ...
  const StatCard = ({ label, value, unit, icon, color, trend }: any) => (
    <GC style={{ flex: 1, alignItems: 'center', padding: 14, marginBottom: 0 }}>
      ...
      <Text style={{ fontSize: 8, color: '#6B7084', letterSpacing: 0.5, marginTop: 2, textAlign: 'center' }}>{label}</Text>
  ...
  return (
    <View style={{ flex: 1, backgroundColor: '#F5F6F8' }}>
      <View style={{ flexDirection: 'row', alignItems: 'center', paddingHorizontal: 16, paddingVertical: 14 }}>
        ...
        <Text style={{ flex: 1, fontSize: 18, fontWeight: '900', color: '#1E1F24' }}>Balance Connectee</Text>
        {isConnected && (
      ...
      <ScrollView contentContainerStyle={{ paddingHorizontal: 16, paddingBottom: 80 }}
        ...
        {!isConnected ? (
          <GC style={{ alignItems: 'center', padding: 24, borderWidth: 2, borderColor: '#2196F3', borderStyle: 'dashed' }} data-testid="ble-connect-section">
            ...
            <Text style={{ fontSize: 16, fontWeight: '800', color: '#1E1F24', marginBottom: 4 }}>Connecter votre balance</Text>
            <Text style={{ fontSize: 12, color: '#6B7084', textAlign: 'center', marginBottom: 16, lineHeight: 18 }}>
            ...
            <TouchableOpacity testID="scan-scale-btn" onPress={isNative ? startScan : () => Alert.alert('Bluetooth', 'Utilisez l\'app mobile pour connecter la balance.')}
              ...
              {bleState === 'scanning' ? <ActivityIndicator color="#1E1F24" size="small" /> : <Icon name="bluetooth" size={18} color="#1E1F24" />}
          ...
          <GC style={{ padding: 16, borderLeftWidth: 4, borderLeftColor: '#4CAF50' }} data-testid="ble-connected-section">
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12 }}>
              ...
              <View style={{ flex: 1 }}>
                <Text style={{ fontSize: 15, fontWeight: '800', color: '#1E1F24' }}>{connectedDevice || 'Balance Lefu'}</Text>
            ...
            {bleState === 'weighing' && liveMeasurement && (
              <View style={{ alignItems: 'center', marginTop: 16 }}>
                ...
                <Text style={{ fontSize: 11, color: '#6B7084' }}>Stabilisation en cours...</Text>
            ...
            {bleState === 'connected' && (
              <TouchableOpacity testID="start-weighing-btn" onPress={startWeighing}
                ...
                <Icon name="scale-outline" size={18} color="#1E1F24" />
            ...
            {bleState === 'done' && (
              <TouchableOpacity onPress={() => { setBleState('connected'); setLiveMeasurement(null); }}
                ...
                <Icon name="refresh" size={18} color="#1E1F24" />
        ...
        {latest && (
          <GC style={{ alignItems: 'center', padding: 24 }}>
            <Text style={{ fontSize: 48, fontWeight: '900', color: '#1E1F24' }}>{latest.weight}<Text style={{ fontSize: 18 }}> kg</Text></Text>
            <Text style={{ fontSize: 13, color: '#6B7084', marginTop: 4 }}>
            ...
            {diff('weight') !== null && Math.abs(diff('weight')) > 0.05 && (
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 4, marginTop: 6 }}>
                ...
                <Text style={{ fontSize: 11, color: '#6B7084' }}>vs precedent</Text>
              </View>
            )}
          </GC>
        )}
        ...
        <View style={{ flexDirection: 'row', gap: 6, marginBottom: 12 }}>
          {(['7', '30', '90'] as const).map(p => (
            <TouchableOpacity key={p} style={[{ flex: 1, paddingVertical: 8, borderRadius: 10, alignItems: 'center', borderWidth: 1, borderColor: period === p ? '#000' : 'rgba(255,255,255,0.10)' }, period === p && { backgroundColor: '#F5F6F8' }]}
              onPress={() => setPeriod(p)}>
        ...
        {chartData.length > 1 && (
          <GC style={{ padding: 16 }}>
            <Text style={{ fontSize: 13, fontWeight: '800', color: '#1E1F24', marginBottom: 12 }}>Evolution du poids</Text>
        ...
        {latest && <>
          ...
          {chartData.length > 1 && (
            <GC style={{ padding: 16 }}>
              <Text style={{ fontSize: 13, fontWeight: '800', color: '#1E1F24', marginBottom: 12 }}>Masse grasse / Muscles</Text>
              ...
              <View style={{ flexDirection: 'row', gap: 12, marginTop: 8 }}>
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 4 }}><View style={{ width: 10, height: 3, borderRadius: 2, backgroundColor: '#FF9800' }} /><Text style={{ fontSize: 9, color: '#6B7084' }}>Masse grasse %</Text></View>
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 4 }}><View style={{ width: 10, height: 3, borderRadius: 2, backgroundColor: '#10B981' }} /><Text style={{ fontSize: 9, color: '#6B7084' }}>Muscles kg</Text></View>
              </View>
            </GC>
          )}
          ...
          <GC>
            <Text style={{ fontSize: 13, fontWeight: '800', color: '#1E1F24', marginBottom: 4 }}>Composition corporelle complete</Text>
            <Text style={{ fontSize: 10, color: '#6B7084', marginBottom: 12 }}>{[
            ...
            {[
              { icon: 'body-outline', label: 'Poids', value: latest.weight, unit: 'kg', color: '#1E1F24' },
          ...
          {/* History list */}
          <GC>
            <Text style={{ fontSize: 13, fontWeight: '800', color: '#1E1F24', marginBottom: 12 }}>Historique des pesees</Text>
            {filtered.slice(0, 10).map((r: any, i: number) => (
              <View key={r.id} style={{ flexDirection: 'row', alignItems: 'center', paddingVertical: 8, borderBottomWidth: i < 9 ? 0.5 : 0, borderBottomColor: 'rgba(0,0,0,0.04)' }}>
                <Text style={{ fontSize: 12, color: '#6B7084', width: 70 }}>{new Date(r.timestamp).toLocaleDateString('fr-FR', { day: '2-digit', month: 'short' })}</Text>
                <Text style={{ fontSize: 15, fontWeight: '800', color: '#1E1F24', flex: 1 }}>{r.weight} kg</Text>
          ...
          {!history.length && (
            <GC style={{ alignItems: 'center', padding: 32 }}>
              ...
              <Text style={{ fontSize: 15, fontWeight: '700', color: '#1E1F24', marginTop: 12 }}>Aucune pesee</Text>
              <Text style={{ fontSize: 12, color: '#6B7084', marginTop: 4, textAlign: 'center' }}>Connectez votre balance et pesez-vous pour voir vos donnees ici.</Text>
            </GC>
          )}
        </>}
      </ScrollView>
        ...
        <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'flex-end' }}>
          <View style={{ backgroundColor: '#FFFFFF', borderTopLeftRadius: 28, borderTopRightRadius: 28, padding: 24, maxHeight: '70%' }}>
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10, marginBottom: 16 }}>
              ...
              <Text style={{ fontSize: 18, fontWeight: '900', color: '#1E1F24', flex: 1 }}>Balances detectees</Text>
            ...
            {bleState === 'scanning' && (
              <View style={{ alignItems: 'center', paddingVertical: 24 }}>
                ...
                <Text style={{ fontSize: 13, color: '#6B7084', marginTop: 12 }}>Recherche de balances a proximite...</Text>
              </View>
            )}
            {bleState === 'connecting' && (
              <View style={{ alignItems: 'center', paddingVertical: 24 }}>
                <ActivityIndicator size="large" color="#4CAF50" />
                <Text style={{ fontSize: 13, color: '#6B7084', marginTop: 12 }}>Connexion en cours...</Text>
              </View>
            )}
            {foundDevices.map(d => (
              <TouchableOpacity key={d.id} testID={`ble-device-${d.id}`} onPress={() => connectDevice(d.id, d.name)}
                ...
                <View style={{ flex: 1 }}>
                  <Text style={{ fontSize: 15, fontWeight: '700', color: '#1E1F24' }}>{d.name}</Text>
                  <Text style={{ fontSize: 11, color: '#6B7084' }}>Signal : {Math.min(100, Math.max(0, 100 + d.rssi))}%</Text>
                </View>
            ...
            {foundDevices.length === 0 && bleState !== 'scanning' && bleState !== 'connecting' && (
              <View style={{ alignItems: 'center', paddingVertical: 24 }}>
                ...
                <Text style={{ fontSize: 13, color: '#6B7084', marginTop: 8, textAlign: 'center' }}>Aucune balance trouvee. Verifiez qu'elle est allumee.</Text>
