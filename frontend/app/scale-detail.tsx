      ...
      <ScrollView contentContainerStyle={{ paddingHorizontal: 16, paddingBottom: 80 }}
        ...
        {!isConnected ? (
          <GC style={{ alignItems: 'center', padding: 24, borderWidth: 2, borderColor: '#2196F3', borderStyle: 'dashed' }} data-testid="ble-connect-section">
            ...
            <Text style={{ fontSize: 16, fontWeight: '800', color: '#000', marginBottom: 4 }}>Connecter votre balance</Text>
            <Text style={{ fontSize: 12, color: '#888', textAlign: 'center', marginBottom: 16, lineHeight: 18 }}>
          ...
          <GC style={{ padding: 16, borderLeftWidth: 4, borderLeftColor: '#4CAF50' }} data-testid="ble-connected-section">
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12 }}>
              <View style={{ width: 44, height: 44, borderRadius: 22, backgroundColor: '#4CAF50' + '15', justifyContent: 'center', alignItems: 'center' }}>
                <Ionicons name="bluetooth" size={22} color="#4CAF50" />
              </View>
              <View style={{ flex: 1 }}>
                <Text style={{ fontSize: 15, fontWeight: '800', color: '#000' }}>{connectedDevice || 'Balance Lefu'}</Text>
            ...
            {bleState === 'weighing' && liveMeasurement && (
              <View style={{ alignItems: 'center', marginTop: 16 }}>
                ...
                <Text style={{ fontSize: 11, color: '#888' }}>Stabilisation en cours...</Text>
            ...
            {bleState === 'done' && (
              <TouchableOpacity onPress={() => { setBleState('connected'); setLiveMeasurement(null); }}
                style={{ backgroundColor: '#4CAF50', borderRadius: 9999, paddingVertical: 14, alignItems: 'center', marginTop: 12, flexDirection: 'row', justifyContent: 'center', gap: 8 }}>
        ...
        {latest && <>
          ...
          {!history.length && (
            <GC style={{ alignItems: 'center', padding: 32 }}>
              ...
              <Text style={{ fontSize: 15, fontWeight: '700', color: '#000', marginTop: 12 }}>Aucune pesee</Text>
              <Text style={{ fontSize: 12, color: '#888', marginTop: 4, textAlign: 'center' }}>Connectez votre balance et pesez-vous pour voir vos donnees ici.</Text>
            </GC>
          )}
        </>}
      </ScrollView>
        ...
        <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'flex-end' }}>
          <View style={{ backgroundColor: '#FFF', borderTopLeftRadius: 28, borderTopRightRadius: 28, padding: 24, maxHeight: '70%' }}>
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10, marginBottom: 16 }}>
              ...
              <Text style={{ fontSize: 18, fontWeight: '900', color: '#000', flex: 1 }}>Balances detectees</Text>
            ...
            {bleState === 'scanning' && (
              <View style={{ alignItems: 'center', paddingVertical: 24 }}>
                ...
                <Text style={{ fontSize: 13, color: '#888', marginTop: 12 }}>Recherche de balances a proximite...</Text>
              </View>
            )}
            {bleState === 'connecting' && (
              <View style={{ alignItems: 'center', paddingVertical: 24 }}>
                <ActivityIndicator size="large" color="#4CAF50" />
                <Text style={{ fontSize: 13, color: '#888', marginTop: 12 }}>Connexion en cours...</Text>
              </View>
            )}
            {foundDevices.map(d => (
              <TouchableOpacity key={d.id} testID={`ble-device-${d.id}`} onPress={() => connectDevice(d.id, d.name)}
                ...
                <View style={{ flex: 1 }}>
                  <Text style={{ fontSize: 15, fontWeight: '700', color: '#000' }}>{d.name}</Text>
                  <Text style={{ fontSize: 11, color: '#888' }}>Signal : {Math.min(100, Math.max(0, 100 + d.rssi))}%</Text>
                </View>
            ...
            {foundDevices.length === 0 && bleState !== 'scanning' && bleState !== 'connecting' && (
              <View style={{ alignItems: 'center', paddingVertical: 24 }}>
                ...
                <Text style={{ fontSize: 13, color: '#888', marginTop: 8, textAlign: 'center' }}>Aucune balance trouvee. Verifiez qu'elle est allumee.</Text>
