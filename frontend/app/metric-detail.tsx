      ...
      <div onClick={handleClick} style={{ cursor: 'crosshair' }}>
        <svg width="100%" viewBox={`0 0 ${W} ${H}`} preserveAspectRatio="xMidYMid meet" style={{ display: 'block' }}>
          {[0, 0.25, 0.5, 0.75, 1].map((p, i) => {
            ...
            return <g key={i}>
              <line x1={padL} y1={yPos} x2={W - padR} y2={yPos} stroke="rgba(0,0,0,0.04)" />
          ...
          {isBP ? (
            sliced.map((h: any, i: number) => {
              ...
              return <g key={i}>
                ...
                {isSel && <text x={toX(i)} y={sY - 6} fill="#111" fontSize="9" fontWeight="800" textAnchor="middle">{sys}/{dia}</text>}
              </g>;
            })
            ...
            sliced.map((h: any, i: number) => {
              ...
              return <g key={i}>
                ...
                {isSel && <text x={toX(i)} y={barY - 6} fill="#111" fontSize="9" fontWeight="800" textAnchor="middle">{typeof h.value === 'number' && h.value > 100 ? Math.round(h.value) : h.value}</text>}
              </g>;
            })
          ) : graphType === 'scatter' ? (
            <>
              ...
              {sliced.map((h: any, i: number) => {
                ...
                return <g key={i}>
                  ...
                  {isSel && <text x={toX(i)} y={toY(h.value) - 14} fill="#111" fontSize="10" fontWeight="800" textAnchor="middle">{h.value}</text>}
                </g>;
              })}
            </>
          ) : (
    ...
    return (
      <div data-testid="gauge-container" style={{ padding: '24px 16px 20px', borderRadius: 18, background: '#F4F4F5', marginBottom: 14, textAlign: 'center' } as any}>
        <svg width="100%" viewBox="0 0 300 145" style={{ maxWidth: 300, margin: '0 auto', display: 'block' }}>
          ...
          <text x={cx} y={cy - 18} textAnchor="middle" fill="#111" fontSize="34" fontWeight="900" fontFamily="Inter, system-ui">{typeof currentVal === 'number' ? (Number.isInteger(currentVal) ? currentVal : currentVal.toFixed(1)) : currentVal}</text>
          <text x={cx} y={cy + 2} textAnchor="middle" fill="#9CA3AF" fontSize="12" fontWeight="600">{m.unit}</text>
          <text x={cx - r - 2} y={cy + 14} textAnchor="middle" fill="#9CA3AF" fontSize="9">{zones ? zones[0].min : gMin.toFixed?.(1) || gMin}</text>
          <text x={cx + r + 2} y={cy + 14} textAnchor="middle" fill="#9CA3AF" fontSize="9">{zones ? zones[zones.length - 1].max : gMax.toFixed?.(1) || gMax}</text>
        </svg>
        {zoneLabel && (
          ...
          <div style={{ display: 'flex', justifyContent: 'center', gap: 14, marginTop: 14, flexWrap: 'wrap' } as any}>
            {zones.map((z, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 5 } as any}>
                ...
                <span style={{ fontSize: 10, color: '#6B7280' }}>{z.label} ({z.min}-{z.max})</span>
              </div>
            ))}
          </div>
        )}
    ...
    return (
      <div style={{ padding: '14px 16px', borderRadius: 18, background: '#F4F4F5', marginBottom: 14 } as any}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 4 } as any}>
          {sliced.filter((_: any, i: number) => i === 0 || i === sliced.length - 1).map((h: any, i: number) => <span key={i} style={{ fontSize: 9, color: '#9CA3AF' }}>{h.label}</span>)}
        </div>
        {stats.trend != null && (
            ...
            <div style={{ textAlign: 'center', marginTop: 12 } as any}>
              ...
              {!isGauge && (
                <>
                  ...
                  {nMin != null && (
                    <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '4px 14px', borderRadius: 999, background: isNormal ? 'rgba(16,185,129,0.2)' : 'rgba(239,68,68,0.2)', marginTop: 8 } as any}>
                      <span style={{ fontSize: 11, fontWeight: 700, color: isNormal ? '#10B981' : '#EF4444' }}>{isNormal ? 'Normal' : currentVal < nMin ? 'Bas' : 'Eleve'}</span>
                    </div>
                  )}
                  {stats.trend != null && <div style={{ fontSize: 12, fontWeight: 700, color: stats.trend <= 0 ? '#10B981' : '#F59E0B', marginTop: 6 }}>{stats.trend > 0 ? '+' : ''}{stats.trend} sur {range}</div>}
                </>
              )}
            </div>
          </div>
            ...
            <>
              ...
              <div data-testid="period-selector" style={{ display: 'flex', gap: 6, marginBottom: 14, alignItems: 'center' } as any}>
                {['24h', '7j', '30j', '90j'].map(r => (
                  <div key={r} data-testid={`period-${r}`} onClick={() => changeRange(r)} style={{ padding: '8px 14px', borderRadius: 10, background: range === r ? '#111' : '#F4F4F5', cursor: 'pointer', fontSize: 12, fontWeight: 700, color: range === r ? '#FFF' : '#9CA3AF' } as any}>{r}</div>
                ))}
                ...
                }} style={{ padding: '6px 10px', borderRadius: 8, background: '#F4F4F5', border: '1px solid #E5E7EB', color: '#111', fontSize: 11, fontFamily: 'inherit', outline: 'none' } as any} />
              </div>
              ...
              <div data-testid="chart-card" style={{ padding: '16px 12px 10px', borderRadius: 18, background: '#F4F4F5', marginBottom: 14, overflow: 'hidden' } as any}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 24px 0 36px' } as any}>
                  ...
                  }).map((h: any, i: number) => <span key={i} style={{ fontSize: 10, color: '#9CA3AF', fontWeight: 600 }}>{h.label}</span>)}
                </div>
                {isBP && (
              ...
              {/* Selected point */}
              {selData && (
                <div data-testid="selected-point" style={{ padding: '14px 18px', borderRadius: 18, background: '#F4F4F5', marginBottom: 14, borderLeft: `3px solid ${color}` } as any}>
                  <div style={{ fontSize: 11, color: '#9CA3AF', marginBottom: 4 }}>{selData.label}</div>
              ...
              <div style={{ marginTop: 12 } as any}>
                ...
                {nMin != null && (
                  <div style={{ display: 'flex', gap: 8 } as any}>
                    <div style={{ flex: 1, padding: '10px', borderRadius: 12, background: 'rgba(56,189,248,0.08)', textAlign: 'center' } as any}>
                      <div style={{ fontSize: 9, color: '#9CA3AF', textTransform: 'uppercase', marginBottom: 4 }}>Min normal</div>
                      <div style={{ fontSize: 16, fontWeight: 900, color: '#38BDF8' }}>{nMin} <span style={{ fontSize: 9, color: '#9CA3AF' }}>{m.unit}</span></div>
                    </div>
                    <div style={{ flex: 1, padding: '10px', borderRadius: 12, background: 'rgba(16,185,129,0.08)', textAlign: 'center' } as any}>
                      <div style={{ fontSize: 9, color: '#9CA3AF', textTransform: 'uppercase', marginBottom: 4 }}>Zone ideale</div>
                      <div style={{ fontSize: 16, fontWeight: 900, color: '#10B981' }}>{nMin}-{nMax}</div>
                    </div>
                    <div style={{ flex: 1, padding: '10px', borderRadius: 12, background: 'rgba(239,68,68,0.08)', textAlign: 'center' } as any}>
                      <div style={{ fontSize: 9, color: '#9CA3AF', textTransform: 'uppercase', marginBottom: 4 }}>Max normal</div>
                      <div style={{ fontSize: 16, fontWeight: 900, color: '#EF4444' }}>{nMax} <span style={{ fontSize: 9, color: '#9CA3AF' }}>{m.unit}</span></div>
                    </div>
                  </div>
                )}
              </div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 } as any}>
                  ...
                  {!thEdit && (
                    <div data-testid="configure-threshold-btn" onClick={() => {
                      setThEdit(true);
                      const smartDefaults: Record<string, { min: string; max: string }> = {
                        heart_rate: { min: '50', max: '100' }, spo2: { min: '92', max: '' },
                        blood_pressure: { min: '90', max: '140' }, temperature: { min: '35.5', max: '38.5' },
                        hrv: { min: '20', max: '' }, stress_level: { min: '', max: '70' },
                        recovery_score: { min: '40', max: '' },
                      };
                      const defaults = smartDefaults[key || ''];
                      if (defaults) { if (!thMin) setThMin(defaults.min); if (!thMax) setThMax(defaults.max); }
                      else { if (!thMin && nMin != null) setThMin(String(nMin)); if (!thMax && nMax != null) setThMax(String(nMax)); }
                    }} style={{ padding: '6px 14px', borderRadius: 999, background: '#FFF', border: '1px solid #E5E7EB', cursor: 'pointer', fontSize: 11, fontWeight: 700, color: '#111' } as any}>
                ...
                {!thEdit ? (
                  ...
                  <div>
                    <div style={{ display: 'flex', gap: 10, marginBottom: 12 } as any}>
                      <div style={{ flex: 1 } as any}>
                        <div style={{ fontSize: 9, fontWeight: 700, color: '#38BDF8', textTransform: 'uppercase', marginBottom: 4 }}>Seuil bas</div>
                        <input type="number" step="0.1" value={thMin} onChange={(e: any) => setThMin(e.target.value)} placeholder="Min" style={{ width: '100%', padding: '12px', borderRadius: 12, background: '#FFF', border: '1px solid #E5E7EB', color: '#111', fontSize: 16, fontWeight: 700, fontFamily: 'inherit', boxSizing: 'border-box', outline: 'none' } as any} />
                      </div>
                      <div style={{ flex: 1 } as any}>
                        <div style={{ fontSize: 9, fontWeight: 700, color: '#EF4444', textTransform: 'uppercase', marginBottom: 4 }}>Seuil haut</div>
                        <input type="number" step="0.1" value={thMax} onChange={(e: any) => setThMax(e.target.value)} placeholder="Max" style={{ width: '100%', padding: '12px', borderRadius: 12, background: '#FFF', border: '1px solid #E5E7EB', color: '#111', fontSize: 16, fontWeight: 700, fontFamily: 'inherit', boxSizing: 'border-box', outline: 'none' } as any} />
                      </div>
                    </div>
                    <div style={{ display: 'flex', gap: 8 } as any}>
                      ...
                      }} style={{ flex: 1, padding: '12px', borderRadius: 999, background: color, cursor: 'pointer', textAlign: 'center', fontSize: 14, fontWeight: 800, color: '#FFF' } as any}>{thSaving ? '...' : 'Sauvegarder'}</div>
                      <div onClick={() => setThEdit(false)} style={{ padding: '12px 16px', borderRadius: 999, background: '#F4F4F5', cursor: 'pointer', fontSize: 13, fontWeight: 700, color: '#9CA3AF' } as any}>Annuler</div>
                    </div>
                  </div>
                )}
