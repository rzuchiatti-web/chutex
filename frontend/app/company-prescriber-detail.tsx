...
export default function CompanyPrescriberDetail() {
  ...
  if (!data?.prescriber) return <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', backgroundColor: '#FFFFFF' }}><Text style={{ color: '#6B7280' }}>Prescripteur non trouve</Text></View>;
  ...
  return (
    <View style={{ flex: 1, backgroundColor: '#FFFFFF' }}>
      <View style={{ flexDirection: 'row', alignItems: 'center', paddingHorizontal: 16, paddingVertical: 14 }}>
        ...
        <Text style={{ flex: 1, fontSize: 18, fontWeight: '900', color: '#111827' }}>Fiche Prescripteur</Text>
      </View>
      ...
      <ScrollView contentContainerStyle={{ paddingHorizontal: 16, paddingBottom: 80 }}
        ...
        <GlassCard style={{ padding: 24 }}>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 16, marginBottom: 16 }}>
            ...
            <View style={{ flex: 1 }}>
              <Text style={{ fontSize: 22, fontWeight: '900', color: '#111827' }}>{pr.name}</Text>
        ...
        <View style={{ flexDirection: 'row', gap: 8, marginBottom: 12 }}>
          <GlassCard style={{ flex: 1, alignItems: 'center', padding: 16, marginBottom: 0 }}>
            ...
            <Text style={{ fontSize: 9, color: '#6B7280', textTransform: 'uppercase', letterSpacing: 0.5, marginTop: 2 }}>Prescriptions</Text>
          </GlassCard>
        ...
        <GlassCard>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10, marginBottom: 14 }}>
            ...
            <Text style={{ fontSize: 16, fontWeight: '800', color: '#111827' }}>Validees ({subscribed.length})</Text>
            <View style={{ flex: 1 }} />
            <Text style={{ fontSize: 14, fontWeight: '800', color: '#10B981' }}>{data.comm_validated} EUR</Text>
          </View>
          {subscribed.length === 0 ? (
            <Text style={{ fontSize: 12, color: '#6B7280', textAlign: 'center', paddingVertical: 12 }}>Aucune prescription validee</Text>
            ...
            <View key={p.id} style={{ flexDirection: 'row', alignItems: 'center', gap: 10, paddingVertical: 10, borderBottomWidth: 0.5, borderBottomColor: 'rgba(0,0,0,0.04)' }}>
              ...
              <View style={{ flex: 1 }}>
                ...
                <Text style={{ fontSize: 10, color: '#6B7280' }}>{p.subscription_type} - {p.created_at ? new Date(p.created_at).toLocaleDateString('fr-FR') : ''}</Text>
              </View>
        ...
        <GlassCard>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10, marginBottom: 14 }}>
            ...
            <Text style={{ fontSize: 16, fontWeight: '800', color: '#111827' }}>En cours ({pending.length})</Text>
            <View style={{ flex: 1 }} />
            <Text style={{ fontSize: 14, fontWeight: '800', color: '#FF9800' }}>{data.comm_pending} EUR</Text>
          </View>
          {pending.length === 0 ? (
            <Text style={{ fontSize: 12, color: '#6B7280', textAlign: 'center', paddingVertical: 12 }}>Aucune prescription en cours</Text>
          ) : pending.map((p: any) => (
            <View key={p.id} style={{ flexDirection: 'row', alignItems: 'center', gap: 10, paddingVertical: 10, borderBottomWidth: 0.5, borderBottomColor: 'rgba(0,0,0,0.04)' }}>
