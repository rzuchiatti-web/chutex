...
const RelationshipPicker = ({ value, onChange, relationships, label }: any) => {
  ...
  return (
    <View style={{ marginBottom: 16 }}>
      <Text style={{ fontSize: 12, fontWeight: '700', color: '#6B7084', marginBottom: 8, letterSpacing: 0.5, textTransform: 'uppercase' }}>{label}</Text>
      <TouchableOpacity
        ...
        <Text style={{ fontSize: 15, color: value ? '#1E1F24' : '#9CA3B0', fontWeight: value ? '600' : '400' }}>
          {selected ? selected.label : 'Choisir...'}
        </Text>
        <Icon name="chevron-down" size={18} color="#9CA3B0" />
      </TouchableOpacity>
      ...
      <Modal visible={open} transparent animationType="slide">
        <TouchableOpacity style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.4)' }} activeOpacity={1} onPress={() => setOpen(false)}>
          ...
          <View style={{ backgroundColor: '#FFF', borderTopLeftRadius: 24, borderTopRightRadius: 24, maxHeight: '60%', paddingBottom: 40 }}>
            ...
            <Text style={{ fontSize: 18, fontWeight: '800', color: '#1E1F24', paddingHorizontal: 20, marginBottom: 12 }}>{label}</Text>
            <ScrollView style={{ paddingHorizontal: 20 }}>
              {relationships.map((r: any) => (
                <TouchableOpacity
                  ...
                  <Text style={{ flex: 1, fontSize: 16, fontWeight: value === r.id ? '700' : '400', color: '#1E1F24' }}>{r.label}</Text>
    ...
    return (
      <SafeAreaView style={{ flex: 1, backgroundColor: '#F5F6F8' }}>
        <View style={{ flexDirection: 'row', alignItems: 'center', paddingHorizontal: 16, paddingVertical: 14 }}>
          ...
          <Text style={{ flex: 1, fontSize: 22, fontWeight: '900', color: '#1E1F24' }}>Mes gardiens</Text>
        </View>
        <ScrollView contentContainerStyle={{ paddingHorizontal: 16, paddingBottom: 40 }}>
          {/* Code permanent */}
          <GlassCard style={{ alignItems: 'center', padding: 28 }}>
            <Icon name="qr-code-outline" size={36} color="#1E1F24" />
            <Text style={{ fontSize: 11, fontWeight: '700', color: '#6B7084', textTransform: 'uppercase', letterSpacing: 1, marginTop: 12 }}>Votre code permanent</Text>
            {loadingCode ? <ActivityIndicator color="#1E1F24" style={{ marginTop: 12 }} /> : (
              <Text style={{ fontSize: 38, fontWeight: '900', color: '#1E1F24', letterSpacing: 6, marginTop: 6 }}>{myCode}</Text>
            )}
            <Text style={{ fontSize: 12, color: '#6B7084', marginTop: 8, textAlign: 'center', lineHeight: 18 }}>Communiquez ce code a vos proches pour qu'ils deviennent vos gardiens.</Text>
            <TouchableOpacity style={{ backgroundColor: '#1E1F24', borderRadius: 9999, paddingVertical: 14, paddingHorizontal: 32, marginTop: 16, flexDirection: 'row', gap: 8, alignItems: 'center' }} onPress={shareCode}>
          ...
          <GlassCard>
            <Text style={{ fontSize: 16, fontWeight: '800', color: '#1E1F24', marginBottom: 4 }}>Inviter par telephone</Text>
            <Text style={{ fontSize: 12, color: '#6B7084', marginBottom: 14, lineHeight: 17 }}>Envoyez une invitation et precisez qui est cette personne pour vous.</Text>
            ...
            <TouchableOpacity
              ...
              style={{ backgroundColor: '#1E1F24', borderRadius: 9999, paddingVertical: 14, alignItems: 'center' }}
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: '#F5F6F8' }}>
      <View style={{ flexDirection: 'row', alignItems: 'center', paddingHorizontal: 16, paddingVertical: 14 }}>
        ...
        <Text style={{ flex: 1, fontSize: 22, fontWeight: '900', color: '#1E1F24' }}>Ajouter un beneficiaire</Text>
      </View>
      <ScrollView contentContainerStyle={{ paddingHorizontal: 16, paddingBottom: 40 }}>
        {mode === 'choose' && (
          <>
            <Text style={{ fontSize: 14, color: '#6B7084', marginBottom: 20, lineHeight: 20 }}>Choisissez une methode. Le beneficiaire devra valider votre demande.</Text>
            {[
              ...
              <TouchableOpacity key={i} onPress={() => setMode(opt.key as any)} activeOpacity={0.7}>
                <GlassCard style={{ flexDirection: 'row', alignItems: 'center', gap: 16 }}>
                  <View style={{ width: 48, height: 48, borderRadius: 14, backgroundColor: '#F0F1F3', justifyContent: 'center', alignItems: 'center' }}>
                    <Icon name={opt.icon as any} size={24} color="#1E1F24" />
                  </View>
                  <View style={{ flex: 1 }}>
                    <Text style={{ fontSize: 16, fontWeight: '800', color: '#1E1F24' }}>{opt.title}</Text>
                    <Text style={{ fontSize: 12, color: '#6B7084', marginTop: 2 }}>{opt.desc}</Text>
                  </View>
                  <Icon name="chevron-forward" size={20} color="#9CA3B0" />
                </GlassCard>
              </TouchableOpacity>
            ))}
          </>
        ...
        {mode === 'enter_code' && (
          <GlassCard style={{ padding: 24 }}>
            <Text style={{ fontSize: 18, fontWeight: '900', color: '#1E1F24', marginBottom: 8 }}>Saisir le code</Text>
            <Text style={{ fontSize: 13, color: '#6B7084', marginBottom: 16 }}>Entrez le code a 6 caracteres du beneficiaire</Text>
            ...
            <TouchableOpacity
              ...
              style={{ backgroundColor: '#1E1F24', borderRadius: 9999, paddingVertical: 16, alignItems: 'center' }}
        ...
        {mode === 'enter_phone' && (
          <GlassCard style={{ padding: 24 }}>
            <Text style={{ fontSize: 18, fontWeight: '900', color: '#1E1F24', marginBottom: 8 }}>Numero de telephone</Text>
            <Text style={{ fontSize: 13, color: '#6B7084', marginBottom: 16 }}>Entrez le numero du beneficiaire</Text>
