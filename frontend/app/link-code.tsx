    ...
    return (
      <SafeAreaView style={{ flex: 1, backgroundColor: colors.background }}>
        <View style={{ flexDirection: 'row', alignItems: 'center', paddingHorizontal: 16, paddingVertical: 14 }}>
          ...
          <Text style={{ flex: 1, fontSize: 22, fontWeight: '900', color: '#000' }}>Mes gardiens</Text>
        </View>
        <ScrollView contentContainerStyle={{ paddingHorizontal: 16, paddingBottom: 40 }}>
          {/* My permanent code */}
          <GlassCard style={{ alignItems: 'center', padding: 28 }}>
            <Ionicons name="qr-code-outline" size={36} color="#000" />
            <Text style={{ fontSize: 11, fontWeight: '700', color: '#888', textTransform: 'uppercase', letterSpacing: 1, marginTop: 12 }}>VOTRE CODE PERMANENT</Text>
            {loadingCode ? <ActivityIndicator color="#000" style={{ marginTop: 12 }} /> : (
              <Text style={{ fontSize: 38, fontWeight: '900', color: '#000', letterSpacing: 6, marginTop: 6 }}>{myCode}</Text>
            )}
            <Text style={{ fontSize: 12, color: '#888', marginTop: 8, textAlign: 'center', lineHeight: 18 }}>Communiquez ce code a vos proches pour qu'ils deviennent vos gardiens. Ils devront le saisir dans leur application.</Text>
          ...
          {/* Invite by phone */}
          <GlassCard>
            <Text style={{ fontSize: 16, fontWeight: '800', color: '#000', marginBottom: 4 }}>Inviter par telephone</Text>
            <Text style={{ fontSize: 12, color: '#888', marginBottom: 14, lineHeight: 17 }}>Envoyez une invitation a un proche par son numero. Il recevra une notification.</Text>
            {Platform.OS === 'web' ? (
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: colors.background }}>
      ...
      <ScrollView contentContainerStyle={{ paddingHorizontal: 16, paddingBottom: 40 }}>
        {mode === 'choose' && (
          <>
            <Text style={{ fontSize: 14, color: '#888', marginBottom: 20, lineHeight: 20 }}>Choisissez une methode. Le beneficiaire devra valider votre demande.</Text>
            {[
              ...
              <TouchableOpacity key={i} onPress={() => setMode(opt.key as any)} activeOpacity={0.7}>
                <GlassCard style={{ flexDirection: 'row', alignItems: 'center', gap: 16 }}>
                  <View style={{ width: 48, height: 48, borderRadius: 14, backgroundColor: 'rgba(0,0,0,0.06)', justifyContent: 'center', alignItems: 'center' }}>
                    <Ionicons name={opt.icon as any} size={24} color="#000" />
                  </View>
                  <View style={{ flex: 1 }}>
                    <Text style={{ fontSize: 16, fontWeight: '800', color: '#000' }}>{opt.title}</Text>
