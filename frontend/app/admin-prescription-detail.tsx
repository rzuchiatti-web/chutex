...
export default function AdminPrescriptionDetail() {
  ...
  if (!data?.prescription) return <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', backgroundColor: '#F5F6F8' }}><Text style={{ color: '#5A6068' }}>Prescription non trouvee</Text></View>;
      ...
      <ScrollView contentContainerStyle={{ paddingHorizontal: 16, paddingBottom: 80 }}>
        ...
        {p.email_content && (
          <GlassCard>
            ...
            <Text style={{ fontSize: 11, fontWeight: '600', color: '#5A6068', marginBottom: 4 }}>Objet: {p.email_content.subject}</Text>
            <Text style={{ fontSize: 12, color: '#1A1D21', lineHeight: 18 }}>{p.email_content.body}</Text>
          </GlassCard>
        )}
