...
export default function GuardianDetailScreen() {
  ...
  if (!guardian) return <SafeAreaView style={{ flex: 1, backgroundColor: colors.background, justifyContent: 'center', alignItems: 'center' }}><Text style={{ color: colors.textSecondary }}>Gardien non trouve</Text></SafeAreaView>;
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: colors.background }}>
      <View style={{ flexDirection: 'row', alignItems: 'center', paddingHorizontal: 16, paddingVertical: 14 }}>
        ...
        <Text style={{ flex: 1, fontSize: 22, fontWeight: '900', color: colors.textPrimary }}>Fiche Gardien</Text>
      </View>
      ...
      <ScrollView contentContainerStyle={{ paddingHorizontal: 16, paddingBottom: 40 }}>
        ...
        <GlassCard style={{ alignItems: 'center', padding: 28 }}>
          ...
          <Text style={{ fontSize: 22, fontWeight: '900', color: colors.textPrimary, marginTop: 12 }}>{guardian.name}</Text>
          {guardian.profession && <Text style={{ fontSize: 13, color: colors.textSecondary, marginTop: 4 }}>{guardian.profession}</Text>}
          {guardian.structure_name && <Text style={{ fontSize: 13, color: colors.textSecondary, marginTop: 2 }}>{guardian.structure_name}</Text>}
        </GlassCard>
        ...
        <GlassCard>
          {infoRows.map((r, i) => (
            <View key={i} style={{ flexDirection: 'row', alignItems: 'center', gap: 12, paddingVertical: 12, borderBottomWidth: i < infoRows.length - 1 ? 0.5 : 0, borderBottomColor: 'rgba(0,0,0,0.06)' }}>
              ...
              <View style={{ flex: 1 }}>
                <Text style={{ fontSize: 10, color: colors.textSecondary, textTransform: 'uppercase', letterSpacing: 0.5 }}>{r.label}</Text>
                <Text style={{ fontSize: 16, fontWeight: '600', color: colors.textPrimary, marginTop: 2 }}>{r.val}</Text>
              </View>
            </View>
          ))}
        </GlassCard>
        ...
        {guardian.latitude && (
          <GlassCard>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12 }}>
              ...
              <View style={{ flex: 1 }}>
                <Text style={{ fontSize: 10, color: colors.textSecondary, textTransform: 'uppercase', letterSpacing: 0.5 }}>Coordonnees GPS</Text>
                <Text style={{ fontSize: 15, fontWeight: '600', color: colors.textPrimary, marginTop: 2 }}>{guardian.latitude?.toFixed(4)}, {guardian.longitude?.toFixed(4)}</Text>
              </View>
            </View>
          </GlassCard>
        )}
