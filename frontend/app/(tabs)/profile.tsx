  ...
  if (Platform.OS === 'web') {
    return rows ? (
      <div style={{ marginBottom: 10 }}><textarea value={val} onChange={(e: any) => onChange(e.target.value)} placeholder={placeholder} rows={rows}
        style={{ width: '100%', fontSize: 14, padding: '12px', borderRadius: 12, border: '1px solid rgba(255,255,255,0.10)', background: 'rgba(255,255,255,0.04)', color: '#1A1D21', fontFamily: 'system-ui', resize: 'none' as any, boxSizing: 'border-box' as any }} /></div>
    ) : (
      <div style={{ marginBottom: 10 }}><input type={type || 'text'} value={val} onChange={(e: any) => onChange(e.target.value)} placeholder={placeholder}
        style={{ width: '100%', fontSize: 15, padding: '12px 14px', borderRadius: 12, border: '1px solid rgba(255,255,255,0.10)', background: 'rgba(255,255,255,0.04)', color: '#1A1D21', fontFamily: 'system-ui', boxSizing: 'border-box' as any }} /></div>
    );
  }
  ...
  return (
    <View style={{ marginBottom: 10 }}>
      <RNTextInput value={val} onChangeText={onChange} placeholder={placeholder} placeholderTextColor="rgba(255,255,255,0.30)"
        ...
        style={{ fontSize: 15, padding: 12, borderRadius: 12, borderWidth: 1, borderColor: 'rgba(0,0,0,0.06)', backgroundColor: '#FFFFFF', color: '#1A1D21' }} />
    </View>
  );
};
  ...
  return (
    <View style={{ flex: 1, backgroundColor: '#F5F6F8' }} testID="profile-screen">
      <ScrollView contentContainerStyle={{ paddingHorizontal: 16, paddingBottom: 80 }} showsVerticalScrollIndicator={false}>
        <Text style={{ fontSize: 28, fontWeight: '900', color: '#1A1D21', marginTop: 16, marginBottom: 20 }}>{t('profile')}</Text>
        ...
        <GlassCard style={{ alignItems: 'center', padding: 28 }}>
          <TouchableOpacity testID="avatar-upload-btn" onPress={handleAvatarUpload} style={{ position: 'relative' }}>
            ...
            <View style={{ position: 'absolute', bottom: 0, right: -4, width: 28, height: 28, borderRadius: 14, backgroundColor: '#F5F6F8', justifyContent: 'center', alignItems: 'center', borderWidth: 2, borderColor: '#000' }}>
        ...
        {editMode && (
          <GlassCard>
            <Text style={{ fontSize: 16, fontWeight: '800', color: '#1A1D21', marginBottom: 14 }}>{t('modify_profile')}</Text>
            ...
            <View style={{ flexDirection: 'row', gap: 10, marginTop: 4 }}>
              <TouchableOpacity style={{ flex: 1, paddingVertical: 12, borderRadius: 9999, backgroundColor: 'rgba(0,0,0,0.06)', alignItems: 'center' }} onPress={() => setEditMode(false)}><Text style={{ fontSize: 14, fontWeight: '700', color: '#5A6068' }}>{t('cancel')}</Text></TouchableOpacity>
              <TouchableOpacity testID="save-profile-btn" style={{ flex: 1, paddingVertical: 12, borderRadius: 9999, backgroundColor: '#F5F6F8', alignItems: 'center' }} onPress={saveProfile} disabled={saving}>{saving ? <ActivityIndicator color="#1A1D21" size="small" /> : <Text style={{ fontSize: 14, fontWeight: '700', color: '#FFF' }}>ENREGISTRER</Text>}</TouchableOpacity>
            </View>
          </GlassCard>
        )}
        ...
        {showPwChange && (
          <GlassCard>
            <Text style={{ fontSize: 16, fontWeight: '800', color: '#1A1D21', marginBottom: 14 }}>{t('security')}</Text>
            ...
            <View style={{ flexDirection: 'row', gap: 10, marginTop: 4 }}>
              <TouchableOpacity style={{ flex: 1, paddingVertical: 12, borderRadius: 9999, backgroundColor: 'rgba(0,0,0,0.06)', alignItems: 'center' }} onPress={() => setShowPwChange(false)}><Text style={{ fontSize: 14, fontWeight: '700', color: '#5A6068' }}>{t('cancel')}</Text></TouchableOpacity>
        ...
        {showContact && (
          <GlassCard>
            <Text style={{ fontSize: 16, fontWeight: '800', color: '#1A1D21', marginBottom: 4 }}>{t('support')}</Text>
            <Text style={{ fontSize: 12, color: '#5A6068', marginBottom: 14 }}>contact@chutex-innovation.com</Text>
            ...
            <View style={{ flexDirection: 'row', gap: 10 }}>
              <TouchableOpacity style={{ flex: 1, paddingVertical: 12, borderRadius: 9999, backgroundColor: 'rgba(0,0,0,0.06)', alignItems: 'center' }} onPress={() => setShowContact(false)}><Text style={{ fontSize: 14, fontWeight: '700', color: '#5A6068' }}>{t('cancel')}</Text></TouchableOpacity>
              <TouchableOpacity style={{ flex: 1, paddingVertical: 12, borderRadius: 9999, backgroundColor: '#F5F6F8', alignItems: 'center' }} onPress={sendContactForm} disabled={sendingContact}>{sendingContact ? <ActivityIndicator color="#1A1D21" size="small" /> : <Text style={{ fontSize: 14, fontWeight: '700', color: '#FFF' }}>ENVOYER</Text>}</TouchableOpacity>
            </View>
          </GlassCard>
        )}
          ...
          <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'flex-end' }}>
            <View style={{ backgroundColor: '#FFFFFF', borderTopLeftRadius: 28, borderTopRightRadius: 28, maxHeight: '85%' }}>
              ...
              <ScrollView contentContainerStyle={{ paddingHorizontal: 20, paddingBottom: 40 }}>
                {notifPrefs ? (
                  <>
                    <Text style={{ fontSize: 13, fontWeight: '800', color: '#1A1D21', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 12 }}>Urgences</Text>
                    {[
                      ...
                      <View key={item.key} style={{ flexDirection: 'row', alignItems: 'center', gap: 12, paddingVertical: 14, borderBottomWidth: 0.5, borderBottomColor: 'rgba(0,0,0,0.04)' }}>
                        ...
                        <View style={{ flex: 1 }}>
                          ...
                          <Text style={{ fontSize: 11, color: '#5A6068', marginTop: 2 }}>{item.desc}</Text>
                        </View>
                        <Switch value={notifPrefs[item.key] ?? true} onValueChange={(v) => toggleNotifPref(item.key, v)} trackColor={{ true: item.color, false: '#E0E0E0' }} />
                      </View>
                    ))}

                    <Text style={{ fontSize: 13, fontWeight: '800', color: '#1A1D21', textTransform: 'uppercase', letterSpacing: 0.5, marginTop: 20, marginBottom: 12 }}>Appareils</Text>
                    {[
                    ...
                    <Text style={{ fontSize: 13, fontWeight: '800', color: '#1A1D21', textTransform: 'uppercase', letterSpacing: 0.5, marginTop: 20, marginBottom: 12 }}>Rappels quotidiens</Text>
                    {[
                    ...
                    <Text style={{ fontSize: 13, fontWeight: '800', color: '#1A1D21', textTransform: 'uppercase', letterSpacing: 0.5, marginTop: 20, marginBottom: 12 }}>Autres</Text>
                    {[
