    ...
    <ScrollView style={{ flex: 1, backgroundColor: '#FFFFFF' }} contentContainerStyle={{ paddingBottom: 80 }} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => { setRefreshing(true); fetchIvs(); }} />}>
      ...
      {user?.is_intervention_provider && (displayedIvs.length > 0 ? displayedIvs.map(iv => {
        ...
        return Platform.OS === 'web' ? (
          <div key={iv.id} data-testid={`iv-${iv.id}`} onClick={() => { window.scrollTo(0,0); document.body.style.overflow = 'hidden'; setSelectedIv(iv); }}
      ...
      {selectedIv && Platform.OS === 'web' && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 9999, display: 'flex', flexDirection: 'column', fontFamily: 'Inter, system-ui, sans-serif', overflow: 'hidden', touchAction: 'none', overscrollBehavior: 'none' } as any}
          ...
          <div style={{ position: 'relative', display: 'flex', alignItems: 'center', gap: 10, padding: '16px 16px 0', zIndex: 10 } as any}>
            <div onClick={() => { document.body.classList.remove('navbar-dark'); document.body.style.overflow = ''; setSelectedIv(null); }} style={{ width: 40, height: 40, borderRadius: 999, background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.12)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' } as any}>
            ...
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '7px 16px', borderRadius: 999, background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.12)' } as any}>
              <span style={{ width: 8, height: 8, borderRadius: '50%', background: ['completed', 'resolved'].includes(selectedIv.status) ? '#10B981' : '#F59E0B' } as any} />
              <span style={{ fontSize: 13, fontWeight: 600, color: '#FFF' }}>{['completed', 'resolved'].includes(selectedIv.status) ? 'Alerte resolue' : 'En cours'}</span>
            </div>
          </div>

          {/* Scrollable content */}
          <div data-scroll style={{ flex: 1, overflowY: 'auto', overflowX: 'hidden', position: 'relative', zIndex: 5, padding: '16px 20px 100px', WebkitOverflowScrolling: 'touch' } as any}>
            {/* Alert header */}
            ...
            {/* Beneficiary avatar + name */}
            <div style={{ textAlign: 'center', marginBottom: 16 } as any}>
              <div style={{ width: 56, height: 56, borderRadius: 999, background: 'linear-gradient(135deg, #D4845A, #E8A87C)', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', marginBottom: 8, boxShadow: '0 4px 16px rgba(0,0,0,0.2)' } as any}>
            ...
            {/* Beneficiary medical info — glass pill card */}
            {selectedIv.beneficiary_info && (
              <div style={{ padding: '14px 16px', borderRadius: 20, background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', marginBottom: 10 } as any}>
                <div style={{ fontSize: 10, fontWeight: 600, color: 'rgba(255,255,255,0.35)', letterSpacing: 1, textTransform: 'uppercase', marginBottom: 8 }}>Informations medicales</div>
                {[
                  selectedIv.beneficiary_info.date_of_birth && `Naissance : ${selectedIv.beneficiary_info.date_of_birth}`,
                  selectedIv.beneficiary_info.blood_type && `Groupe sanguin : ${selectedIv.beneficiary_info.blood_type}`,
                  selectedIv.beneficiary_info.medical_conditions && `Pathologies : ${selectedIv.beneficiary_info.medical_conditions}`,
                  selectedIv.beneficiary_info.allergies && `Allergies : ${selectedIv.beneficiary_info.allergies}`,
                  selectedIv.beneficiary_info.doctor_name && `Medecin : ${selectedIv.beneficiary_info.doctor_name}`,
                ].filter(Boolean).map((line, i) => (
                  <div key={i} style={{ fontSize: 13, color: '#FFF', lineHeight: 1.6, paddingVertical: 1 }}>{line}</div>
                ))}
              </div>
            )}

            {/* Phone — glass pill with icon */}
            {(selectedIv.beneficiary_info?.phone || selectedIv.beneficiary_phone) && (
              <div onClick={() => { window.location.href = `tel:${selectedIv.beneficiary_info?.phone || selectedIv.beneficiary_phone}`; }} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 16px', borderRadius: 999, background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', marginBottom: 10, cursor: 'pointer' } as any}>
                <div style={{ width: 36, height: 36, borderRadius: 999, background: 'rgba(255,255,255,0.12)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 } as any}>
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#FFF" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72c.127.96.361 1.903.7 2.81a2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0 1 22 16.92z"/></svg>
                </div>
                <div style={{ fontSize: 15, fontWeight: 700, color: '#FFF' }}>{selectedIv.beneficiary_info?.phone || selectedIv.beneficiary_phone}</div>
              </div>
            )}

            {/* Address — glass pill with icon */}
            {(selectedIv.beneficiary_info?.address || selectedIv.beneficiary_address || selectedIv.beneficiary_location?.address) && (
              <div onClick={() => { window.location.href = `https://maps.google.com/?q=${encodeURIComponent(selectedIv.beneficiary_info?.address || selectedIv.beneficiary_address || selectedIv.beneficiary_location?.address)}`; }} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 16px', borderRadius: 999, background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', marginBottom: 10, cursor: 'pointer' } as any}>
                <div style={{ width: 36, height: 36, borderRadius: 999, background: 'rgba(255,255,255,0.12)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 } as any}>
                ...
                <div style={{ fontSize: 14, fontWeight: 600, color: '#FFF', lineHeight: 1.4 }}>{selectedIv.beneficiary_info?.address || selectedIv.beneficiary_address || selectedIv.beneficiary_location?.address}</div>
              </div>
            )}

            {/* Emergency contact */}
            {selectedIv.beneficiary_info?.emergency_contact_name && (
              <div onClick={() => { if (selectedIv.beneficiary_info?.emergency_contact_phone) window.location.href = `tel:${selectedIv.beneficiary_info.emergency_contact_phone}`; }} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 16px', borderRadius: 999, background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', marginBottom: 16, cursor: 'pointer' } as any}>
                <div style={{ width: 36, height: 36, borderRadius: 999, background: 'rgba(239,68,68,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 } as any}>
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#F87171" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
                </div>
                <div><div style={{ fontSize: 14, fontWeight: 600, color: '#FFF' }}>{selectedIv.beneficiary_info.emergency_contact_name}</div><div style={{ fontSize: 11, color: 'rgba(255,255,255,0.4)' }}>Contact d'urgence · {selectedIv.beneficiary_info.emergency_contact_phone}</div></div>
              </div>
            )}
            ...
            {selectedIv.assigned_name ? (
              <div style={{ padding: '14px 16px', borderRadius: 20, background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', marginBottom: 10 } as any}>
                ...
                <div style={{ fontSize: 16, fontWeight: 700, color: '#FFF', marginBottom: 4 }}>{selectedIv.assigned_name}</div>
                <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.5)' }}>{selectedIv.structure_name || selectedIv.company_name || ''} {selectedIv.distance_km ? `· ${selectedIv.distance_km} km` : ''}</div>
                {selectedIv.accepted_at && <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.4)', marginTop: 4 }}>Accepte le {new Date(selectedIv.accepted_at).toLocaleString('fr-FR', { day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' })}</div>}
              </div>
            ) : (
              <div style={{ padding: '14px 16px', borderRadius: 20, background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', marginBottom: 10, textAlign: 'center' } as any}>
                <div style={{ fontSize: 14, color: 'rgba(255,255,255,0.5)' }}>Aucun intervenant assigne</div>
              </div>
            )}

            {/* Slide to intervene (if pending) */}
            {selectedIv.status === 'pending_acceptance' && (
              <div style={{ width: '100%', height: 58, borderRadius: 999, position: 'relative', overflow: 'hidden', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', marginBottom: 16, touchAction: 'none' } as any}
                onMouseDown={(e: any) => { const bar = e.currentTarget; const thumb = bar.querySelector('[data-thumb]') as HTMLElement; if (!thumb) return; const rect = bar.getBoundingClientRect(); const maxX = rect.width - 52; const startX = e.clientX; const onMove = (ev: any) => { const dx = Math.max(0, Math.min(ev.clientX - startX, maxX)); thumb.style.transform = `translateX(${dx}px)`; if (dx > maxX * 0.8) { document.removeEventListener('mousemove', onMove); document.removeEventListener('mouseup', onUp); apiFetch(`/api/interventions/${selectedIv.id}/accept`, { method: 'POST' }, token).then(() => { fetchIvs(); document.body.classList.remove('navbar-dark'); setSelectedIv(null); }).catch(() => {}); } }; const onUp = () => { thumb.style.transform = 'translateX(0)'; thumb.style.transition = 'transform 0.3s'; setTimeout(() => { if(thumb) thumb.style.transition = ''; }, 300); document.removeEventListener('mousemove', onMove); document.removeEventListener('mouseup', onUp); }; document.addEventListener('mousemove', onMove); document.addEventListener('mouseup', onUp); }}
                onTouchStart={(e: any) => { e.stopPropagation(); const bar = e.currentTarget; const thumb = bar.querySelector('[data-thumb]') as HTMLElement; if (!thumb) return; const rect = bar.getBoundingClientRect(); const maxX = rect.width - 52; const startX = e.touches[0].clientX; const onMove = (ev: any) => { ev.preventDefault(); const dx = Math.max(0, Math.min(ev.touches[0].clientX - startX, maxX)); thumb.style.transform = `translateX(${dx}px)`; if (dx > maxX * 0.8) { bar.removeEventListener('touchmove', onMove); bar.removeEventListener('touchend', onUp); apiFetch(`/api/interventions/${selectedIv.id}/accept`, { method: 'POST' }, token).then(() => { fetchIvs(); document.body.classList.remove('navbar-dark'); setSelectedIv(null); }).catch(() => {}); } }; const onUp = () => { thumb.style.transform = 'translateX(0)'; thumb.style.transition = 'transform 0.3s'; setTimeout(() => { if(thumb) thumb.style.transition = ''; }, 300); bar.removeEventListener('touchmove', onMove); bar.removeEventListener('touchend', onUp); }; bar.addEventListener('touchmove', onMove, { passive: false }); bar.addEventListener('touchend', onUp); }}>
                <div data-thumb style={{ position: 'absolute', top: 4, left: 4, width: 50, height: 50, borderRadius: 999, background: '#FFF', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 2px 10px rgba(0,0,0,0.2)', willChange: 'transform', touchAction: 'none' } as any}>
            ...
            {selectedIv.report && (
              <>
                <div style={{ fontSize: 20, fontWeight: 800, color: '#FFF', textAlign: 'center', marginTop: 8, marginBottom: 12 }}>Rapport & Detail</div>
                <div style={{ padding: '14px 16px', borderRadius: 20, background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', marginBottom: 10 } as any}>
                  {selectedIv.report.description && <><div style={{ fontSize: 10, fontWeight: 600, color: 'rgba(255,255,255,0.35)', letterSpacing: 1, textTransform: 'uppercase', marginBottom: 4 }}>Description</div><div style={{ fontSize: 13, color: '#FFF', lineHeight: 1.6, marginBottom: 10 }}>{selectedIv.report.description}</div></>}
                  {selectedIv.report.actions_taken && <><div style={{ fontSize: 10, fontWeight: 600, color: 'rgba(255,255,255,0.35)', letterSpacing: 1, textTransform: 'uppercase', marginBottom: 4 }}>Actions realisees</div><div style={{ fontSize: 13, color: '#FFF', lineHeight: 1.6, marginBottom: 10 }}>{selectedIv.report.actions_taken}</div></>}
                  {selectedIv.report.patient_condition && <><div style={{ fontSize: 10, fontWeight: 600, color: 'rgba(255,255,255,0.35)', letterSpacing: 1, textTransform: 'uppercase', marginBottom: 4 }}>Etat du patient</div><div style={{ fontSize: 13, color: '#FFF', lineHeight: 1.6, marginBottom: 10 }}>{selectedIv.report.patient_condition === 'stable' ? 'Stable' : selectedIv.report.patient_condition}</div></>}
                  {selectedIv.report.follow_up_notes && <><div style={{ fontSize: 10, fontWeight: 600, color: 'rgba(255,255,255,0.35)', letterSpacing: 1, textTransform: 'uppercase', marginBottom: 4 }}>Suivi recommande</div><div style={{ fontSize: 13, color: '#FFF', lineHeight: 1.6 }}>{selectedIv.report.follow_up_notes}</div></>}
                </div>
              </>
            )}
            ...
            {selectedIv.timeline && selectedIv.timeline.length > 0 && (
              <>
                <div style={{ fontSize: 16, fontWeight: 700, color: '#FFF', textAlign: 'center', marginTop: 8, marginBottom: 10 }}>Chronologie</div>
                {selectedIv.timeline.map((t: any, i: number) => (
                  <div key={i} style={{ display: 'flex', gap: 10, marginBottom: 8 } as any}>
                    <div style={{ width: 8, height: 8, borderRadius: 4, background: i === selectedIv.timeline.length - 1 ? '#10B981' : 'rgba(255,255,255,0.3)', marginTop: 6, flexShrink: 0 }} />
                    <div><div style={{ fontSize: 12, color: '#FFF', fontWeight: 600 }}>{t.note}</div><div style={{ fontSize: 10, color: 'rgba(255,255,255,0.4)', marginTop: 2 }}>{new Date(t.time).toLocaleString('fr-FR', { day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' })}</div></div>
                  </div>
                ))}
              </>
            )}
