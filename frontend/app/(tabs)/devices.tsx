...
function DeviceManagement({ token }: { token: string }) {
  ...
  const fetchDevices = useCallback(async () => {
    try {
      ...
      setDevices(devs); setSubscription(sub);
  ...
  const removeDevice = async (deviceId: string) => {
    setRemoving(true);
    try { await apiFetch(`/api/devices/${deviceId}/remove`, { method: 'DELETE' }, token); setSelectedDevice(null); fetchDevices(); } catch {} finally { setRemoving(false); }
  };

  const syncDevice = async (deviceType: string) => {
    try { await apiFetch('/api/devices/sync', { method: 'POST', body: JSON.stringify({ device_type: deviceType, data: {} }) }, token); fetchDevices(); window.alert('Synchronise !'); } catch (e: any) { window.alert(e.message || 'Erreur'); }
  };
  ...
  if (Platform.OS === 'web') {
    return (
      <div data-testid="devices-page" style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, display: 'flex', flexDirection: 'column', fontFamily: 'Inter, system-ui, sans-serif', overflow: 'hidden' } as any}>
              ...
              <div key={dt} data-testid={`device-card-${dt}`} style={{ borderRadius: 24, marginBottom: 16, overflow: 'hidden', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.08)', backdropFilter: 'blur(16px)', WebkitBackdropFilter: 'blur(16px)' } as any}>
                ...
                <div style={{ padding: '0 20px 20px' } as any}>
                  ...
                  {isAssociated ? (
                    <div>
                      {realBattery > 0 && <>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 } as any}><span style={{ fontSize: 12, fontWeight: 600, color: 'rgba(255,255,255,0.5)' }}><i className="ri-battery-line" style={{ fontSize: 14, marginRight: 6 }} />Batterie</span><span style={{ fontSize: 14, fontWeight: 800, color: realBattery > 50 ? '#10B981' : realBattery > 20 ? '#F59E0B' : '#EF4444' }}>{realBattery}%</span></div>
                        <div style={{ height: 14, borderRadius: 7, background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.08)', overflow: 'hidden' } as any}><div style={{ height: '100%', borderRadius: 7, width: `${Math.max(4, realBattery)}%`, background: realBattery > 50 ? 'linear-gradient(90deg, #059669, #10B981)' : realBattery > 20 ? 'linear-gradient(90deg, #D97706, #F59E0B)' : 'linear-gradient(90deg, #DC2626, #EF4444)' } as any} /></div>
                      </>}
        ...
        {/* ── Pairing Steps Popup ── */}
        {pairingDevice && bleStatus === 'idle' && (() => { const meta = DEVICE_META[pairingDevice]; const steps = meta.steps; const cur = steps[pairingStep]; const isLast = pairingStep === steps.length - 1; return (
          <GlassOverlay onClose={closePairing}><div style={{ textAlign: 'center' } as any}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, marginBottom: 28 } as any}>{steps.map((_: any, i: number) => <div key={i} style={{ height: 4, borderRadius: 2, width: i === pairingStep ? 24 : 12, background: i === pairingStep ? meta.color : i < pairingStep ? `${meta.color}66` : 'rgba(255,255,255,0.1)' } as any} />)}</div>
            <div style={{ width: 52, height: 52, borderRadius: 14, background: `${meta.color}15`, border: `1px solid ${meta.color}30`, display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 16px' } as any}><i className={cur.icon} style={{ fontSize: 24, color: meta.color }} /></div>
        ...
        {pairingDevice && bleStatus === 'scanning' && (() => { const meta = DEVICE_META[pairingDevice]; return (
          <GlassOverlay onClose={closePairing}><div style={{ textAlign: 'center' } as any}>
            ...
            <div style={{ position: 'relative', width: 80, height: 80, margin: '0 auto 24px' } as any}><div style={{ position: 'absolute', inset: 0, borderRadius: '50%', border: `2px solid ${meta.color}`, opacity: 0.3, animation: 'pulseRing 1.5s ease-out infinite' } as any} /><div style={{ position: 'absolute', inset: 8, borderRadius: '50%', border: `2px solid ${meta.color}`, opacity: 0.3, animation: 'pulseRing 1.5s ease-out infinite 0.5s' } as any} /><div style={{ position: 'absolute', inset: 16, borderRadius: '50%', background: `${meta.color}15`, border: `2px solid ${meta.color}40`, display: 'flex', alignItems: 'center', justifyContent: 'center' } as any}><i className="ri-bluetooth-connect-line" style={{ fontSize: 22, color: meta.color, animation: 'spin 2s linear infinite' }} /></div></div>
        ...
        {pairingDevice && bleStatus === 'error' && (() => { const meta = DEVICE_META[pairingDevice]; return (
          <GlassOverlay onClose={closePairing}><div style={{ textAlign: 'center' } as any}>
            ...
            <div style={{ display: 'flex', gap: 10 } as any}><div onClick={closePairing} style={{ flex: 1, padding: '14px', borderRadius: 999, background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', cursor: 'pointer', textAlign: 'center', fontSize: 14, fontWeight: 700, color: 'rgba(255,255,255,0.5)' } as any}>Fermer</div><div onClick={() => launchBleScan(pairingDevice)} style={{ flex: 1, padding: '14px', borderRadius: 999, background: '#FFF', cursor: 'pointer', textAlign: 'center', fontSize: 14, fontWeight: 700, color: '#111' } as any}>Reessayer</div></div>
          </div></GlassOverlay>); })()}
        ...
        {selectedDevice && deviceMap[selectedDevice] && (() => { const meta = DEVICE_META[selectedDevice]; const device = deviceMap[selectedDevice];
          ...
          <div style={{ textAlign: 'center', marginBottom: 20 } as any}><img src={meta.img} alt="" style={{ width: 100, height: 100, objectFit: 'contain', margin: '0 auto 14px', display: 'block' } as any} /><div style={{ fontSize: 22, fontWeight: 900, color: '#FFF', marginBottom: 8 }}>{meta.name}</div><div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '6px 16px', borderRadius: 999, background: `${detailColor}18`, border: `1px solid ${detailColor}30` } as any}><span style={{ width: 7, height: 7, borderRadius: 4, background: detailColor } as any} /><span style={{ fontSize: 12, fontWeight: 700, color: detailColor }}>{detailLabel}</span></div></div>
          <div style={{ padding: '4px 16px', borderRadius: 16, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)', marginBottom: 12 } as any}>{[['Nom', meta.name], ...(device.mac_address || device.ble_device_id ? [['ID', device.mac_address || device.ble_device_id]] : []), ['Derniere sync.', device.last_sync ? new Date(device.last_sync).toLocaleString('fr-FR',{day:'2-digit',month:'short',hour:'2-digit',minute:'2-digit'}) : '--']].map(([l,v]) => <div key={l} style={{ display:'flex',justifyContent:'space-between',alignItems:'center',padding:'10px 0',borderBottom:'1px solid rgba(255,255,255,0.04)' } as any}><span style={{ fontSize:12,color:'rgba(255,255,255,0.4)' }}>{l}</span><span style={{ fontSize:13,fontWeight:700,color:'#FFF', fontFamily: l === 'ID' ? 'monospace' : 'inherit' }}>{v}</span></div>)}</div>
          ...
          <div style={{ display:'flex',gap:8,marginBottom:12 } as any}>
            ...
            {selectedDevice === 'vest' && <div onClick={() => { setSelectedDevice(null); launchBleScan('vest'); }} style={{ flex:1,padding:'14px',borderRadius:999,background:`${meta.color}18`,border:`1px solid ${meta.color}30`,cursor:'pointer',textAlign:'center',fontSize:13,fontWeight:700,color:meta.color } as any}><i className="ri-refresh-line" style={{ marginRight:6 }} />Synchroniser</div>}
          </div>
        ...
        {showNoSubPopup && <GlassOverlay onClose={() => setShowNoSubPopup(false)}><div style={{ textAlign:'center',marginBottom:24 } as any}><div style={{ width:72,height:72,borderRadius:22,background:'rgba(245,158,11,0.12)',border:'1px solid rgba(245,158,11,0.25)',display:'inline-flex',alignItems:'center',justifyContent:'center',marginBottom:16 } as any}><i className="ri-watch-line" style={{ fontSize:36,color:'#F59E0B' }} /></div><div style={{ fontSize:22,fontWeight:900,color:'#FFF',marginBottom:8 }}>Abonnement requis</div><div style={{ fontSize:13,color:'rgba(255,255,255,0.45)',lineHeight:1.6 }}>Pour associer votre bracelet Elio, choisissez votre formule.</div></div><div onClick={() => { setShowNoSubPopup(false); window.open('https://chutex-innovation.com/products/elio-smart-health-bracelet','_blank'); }} style={{ padding:'20px',borderRadius:20,background:'rgba(255,255,255,0.06)',border:'1px solid rgba(255,255,255,0.12)',marginBottom:14,cursor:'pointer' } as any}><div style={{ display:'flex',gap:16,alignItems:'center' } as any}><div style={{ width:52,height:52,borderRadius:16,background:'rgba(59,130,246,0.15)',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0 } as any}><i className="ri-watch-line" style={{ fontSize:26,color:'#3B82F6' }} /></div><div style={{ flex:1 } as any}><div style={{ fontSize:16,fontWeight:800,color:'#FFF',marginBottom:3 }}>Bracelet Elio</div><div style={{ fontSize:12,color:'rgba(255,255,255,0.45)' }}>24,90 EUR/mois</div></div></div></div><div onClick={() => { setShowNoSubPopup(false); router.push('/subscription' as any); }} style={{ padding:'20px',borderRadius:20,background:'linear-gradient(135deg,rgba(124,92,255,0.1),rgba(167,139,250,0.04))',border:'1px solid rgba(124,92,255,0.25)',marginBottom:14,cursor:'pointer',position:'relative' } as any}><div style={{ position:'absolute',top:10,right:12,padding:'3px 10px',borderRadius:999,background:'rgba(16,185,129,0.15)',border:'1px solid rgba(16,185,129,0.25)' } as any}><span style={{ fontSize:9,fontWeight:700,color:'#10B981',textTransform:'uppercase' }}>Recommande</span></div><div style={{ display:'flex',gap:16,alignItems:'center' } as any}><div style={{ width:52,height:52,borderRadius:16,background:'rgba(124,92,255,0.15)',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0 } as any}><i className="ri-shield-star-line" style={{ fontSize:26,color:'#A78BFA' }} /></div><div style={{ flex:1 } as any}><div style={{ fontSize:16,fontWeight:800,color:'#FFF',marginBottom:3 }}>Chutex Care</div><div style={{ fontSize:12,color:'rgba(255,255,255,0.45)' }}>39,90 EUR/mois</div></div></div></div></GlassOverlay>}

        {/* ── WeighingFlow (balance) ── */}
        {showWeighing && <WeighingFlow onClose={() => { setShowWeighing(false); fetchDevices(); }} d={dashData?.scale || {}} weighings={weighings} />}
      </div>
    );
  }
  return (<ScrollView style={d.sv} contentContainerStyle={d.sc} showsVerticalScrollIndicator={false}>{allTypes.map(dt => { const meta = DEVICE_META[dt]; const device = deviceMap[dt]; const isA = device && (device.connected || device.battery > 0); return (<View key={dt} style={d.deviceCard} testID={`device-card-${dt}`}><View style={d.deviceHeader}><View style={d.deviceIconBg}><MCIcon name={(dt==='bracelet'?'watch':dt==='scale'?'scale-bathroom':'tshirt-crew') as any} size={24} color={Colors.textPrimary} /></View><View style={d.deviceInfo}><Text style={d.deviceName}>{meta.name}</Text></View>{isA && <Text style={d.batteryT}>{device?.battery||0}%</Text>}</View><TouchableOpacity style={d.syncBtn} onPress={() => startPairing(dt)}><Icon name="bluetooth" size={16} color={isA?Colors.success:Colors.primary} /><Text style={d.syncBtnText}>{isA?(dt==='scale'?'Nouvelle pesee':'Connexion'):(dt==='scale'?'Nouvelle pesee':'Associer')}</Text></TouchableOpacity></View>); })}</ScrollView>);
}
