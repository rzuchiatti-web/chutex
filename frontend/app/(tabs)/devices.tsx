    ...
    return (
      <div data-testid="devices-page" style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, display: 'flex', flexDirection: 'column', fontFamily: 'Inter, system-ui, sans-serif', overflow: 'hidden' } as any}>
        ...
        {selectedDevice && deviceMap[selectedDevice] && (() => { const meta = DEVICE_META[selectedDevice]; const device = deviceMap[selectedDevice]; return (
          ...
          <div style={{ textAlign: 'center', marginBottom: 20 } as any}><img src={meta.img} alt="" style={{ width: 100, height: 100, objectFit: 'contain', margin: '0 auto 14px', display: 'block' } as any} /><div style={{ fontSize: 22, fontWeight: 900, color: '#FFF', marginBottom: 8 }}>{meta.name}</div><div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '6px 16px', borderRadius: 999, background: device.connected ? 'rgba(16,185,129,0.12)' : 'rgba(239,68,68,0.12)', border: `1px solid ${device.connected ? 'rgba(16,185,129,0.2)' : 'rgba(239,68,68,0.2)'}` } as any}><span style={{ width: 7, height: 7, borderRadius: 4, background: device.connected ? '#10B981' : '#EF4444' } as any} /><span style={{ fontSize: 12, fontWeight: 700, color: device.connected ? '#10B981' : '#EF4444' }}>{device.connected ? 'Actif' : 'Inactif'}</span></div></div>
          <div style={{ padding: '4px 16px', borderRadius: 16, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)', marginBottom: 12 } as any}>{[['Nom', meta.name],['Derniere sync.', device.last_sync ? new Date(device.last_sync).toLocaleString('fr-FR',{day:'2-digit',month:'short',hour:'2-digit',minute:'2-digit'}) : '--']].map(([l,v]) => <div key={l} style={{ display:'flex',justifyContent:'space-between',alignItems:'center',padding:'10px 0',borderBottom:'1px solid rgba(255,255,255,0.04)' } as any}><span style={{ fontSize:12,color:'rgba(255,255,255,0.4)' }}>{l}</span><span style={{ fontSize:13,fontWeight:700,color:'#FFF' }}>{v}</span></div>)}</div>
