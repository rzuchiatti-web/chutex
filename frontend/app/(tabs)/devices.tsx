    ...
    return (
      <div style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, display: 'flex', flexDirection: 'column', fontFamily: 'Inter, system-ui, sans-serif', overflow: 'hidden' } as any}>
        ...
        {showPrescriberPopup && <div onClick={() => setShowPrescriberPopup(false)} style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, zIndex: 9999, backdropFilter: 'blur(32px)', WebkitBackdropFilter: 'blur(32px)', background: 'rgba(0,0,0,0.2)', overflowY: 'scroll', WebkitOverflowScrolling: 'touch' } as any}><div onClick={(e: any) => e.stopPropagation()} style={{ width: '100%', maxWidth: 400, margin: '0 auto', padding: '40px 28px 120px', boxSizing: 'border-box' } as any}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 24 } as any}><div style={{ width: 64, height: 64, borderRadius: 20, background: 'rgba(212,132,90,0.2)', border: '1px solid rgba(212,132,90,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 } as any}><span style={{ fontSize: 26, fontWeight: 800, color: '#D4845A' }}>{(selectedPresc.guardian_name || selectedPresc.prescriber_name || '?').charAt(0)}</span></div><div><div style={{ fontSize: 22, fontWeight: 800, color: '#FFF' }}>{selectedPresc.guardian_name || selectedPresc.prescriber_name}</div>{selectedPresc.structure_name && <div style={{ fontSize: 13, color: 'rgba(255,255,255,0.5)', marginTop: 2 }}>{selectedPresc.structure_name}</div>}</div></div>
          {[selectedPresc.guardian_phone && { icon: 'ri-phone-line', label: 'Telephone', value: selectedPresc.guardian_phone, phone: true }, selectedPresc.guardian_email && { icon: 'ri-mail-line', label: 'Email', value: selectedPresc.guardian_email }, selectedPresc.structure_name && { icon: 'ri-building-line', label: 'Structure', value: selectedPresc.structure_name }, selectedPresc.created_at && { icon: 'ri-calendar-line', label: 'Prescription du', value: new Date(selectedPresc.created_at).toLocaleString('fr-FR', { day: '2-digit', month: 'long', year: 'numeric' }) }, { icon: 'ri-vip-crown-line', label: 'Type', value: selectedPresc.subscription_type || 'Standard' }, { icon: 'ri-money-euro-circle-line', label: 'Commission', value: `${selectedPresc.commission || 0} EUR` }].filter(Boolean).map((item: any, i: number, arr: any[]) => (<div key={i}><div onClick={() => item.phone && (window.location.href = `tel:${item.value}`)} style={{ display: 'flex', alignItems: 'center', gap: 14, cursor: item.phone ? 'pointer' : 'default', padding: '13px 0' } as any}><div style={{ width: 34, height: 34, borderRadius: 10, background: item.phone ? 'rgba(16,185,129,0.1)' : 'rgba(255,255,255,0.08)', border: `1px solid ${item.phone ? 'rgba(16,185,129,0.2)' : 'rgba(255,255,255,0.1)'}`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 } as any}><i className={item.icon} style={{ fontSize: 15, color: item.phone ? '#10B981' : 'rgba(255,255,255,0.5)' }} /></div><div style={{ flex: 1 } as any}><div style={{ fontSize: 9, fontWeight: 600, color: 'rgba(255,255,255,0.3)', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 2 }}>{item.label}</div><div style={{ fontSize: 15, color: '#FFF', fontWeight: item.phone ? 700 : 500 }}>{item.value}</div></div></div>{i < arr.length - 1 && <div style={{ height: 1, background: 'rgba(255,255,255,0.08)' } as any} />}</div>))}
        </div></div>}
        {/* BENEFICIARY POPUP */}
