      ...
      <TouchableOpacity onPress={() => setShowDetail(true)} activeOpacity={0.8}>
        <View style={{ backgroundColor: '#FFF8E1', borderRadius: 20, padding: 16, marginBottom: 12, borderWidth: 1.5, borderColor: '#FFD54F', flexDirection: 'row', alignItems: 'center', gap: 12 }}>
          <View style={{ width: 48, height: 48, borderRadius: 24, backgroundColor: '#FFD700', justifyContent: 'center', alignItems: 'center' }}>
            <Icon name="trophy" size={24} color="#1C1917" />
          </View>
          <View style={{ flex: 1 }}>
            <Text style={{ fontSize: 14, fontWeight: '900', color: '#1C1917' }}>Challenge du mois</Text>
            <Text style={{ fontSize: 12, color: '#78716C' }}>1er: {data.prizes?.['1']}EUR - 2e: {data.prizes?.['2']}EUR - 3e: {data.prizes?.['3']}EUR</Text>
          </View>
          <View style={{ alignItems: 'center' }}>
            <Text style={{ fontSize: 24, fontWeight: '900', color: posColor }}>{data.my_position}<Text style={{ fontSize: 11 }}>e</Text></Text>
            <Text style={{ fontSize: 9, color: '#78716C' }}>position</Text>
          </View>
        ...
        <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'flex-end' }}>
          <View style={{ backgroundColor: '#FFFFFF', borderTopLeftRadius: 28, borderTopRightRadius: 28, padding: 24, maxHeight: '90%' }}>
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              <Text style={{ fontSize: 20, fontWeight: '900', color: '#1C1917' }}>Challenge Prescripteurs</Text>
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
                  ...
                  {[{pos: '1er', prize: data.prizes?.['1'], color: '#FFD700', icon: 'trophy'}, {pos: '2e', prize: data.prizes?.['2'], color: '#C0C0C0', icon: 'medal'}, {pos: '3e', prize: data.prizes?.['3'], color: '#CD7F32', icon: 'ribbon'}].map(t => (
                    <View key={t.pos} style={{ alignItems: 'center' }}>
                      <View style={{ width: 44, height: 44, borderRadius: 24, backgroundColor: t.color, justifyContent: 'center', alignItems: 'center', marginBottom: 6 }}>
                        <Icon name={t.icon as any} size={22} color="#1C1917" />
                      </View>
                      <Text style={{ fontSize: 16, fontWeight: '900', color: '#1C1917' }}>{t.prize}EUR</Text>
                      <Text style={{ fontSize: 11, color: '#78716C' }}>{t.pos}</Text>
                    </View>
                  ))}
                </View>
              </View>
              <View style={{ backgroundColor: posColor + '15', borderRadius: 16, padding: 16, marginBottom: 16, borderWidth: 2, borderColor: posColor }}>
                <Text style={{ fontSize: 11, fontWeight: '700', color: '#78716C', textAlign: 'center', marginBottom: 4 }}>VOTRE POSITION</Text>
                ...
                <Text style={{ fontSize: 14, color: '#1C1917', textAlign: 'center', fontWeight: '600' }}>{data.my_prescriptions} prescription{data.my_prescriptions !== 1 ? 's' : ''} ce mois</Text>
              ...
              <Text style={{ fontSize: 14, fontWeight: '800', color: '#1C1917', marginBottom: 10 }}>Classement anonyme</Text>
              {(data.ranking || []).map((r: any) => (
                <View key={r.position} style={[{ flexDirection: 'row', alignItems: 'center', gap: 10, paddingVertical: 10, borderBottomWidth: 0.5, borderBottomColor: 'rgba(0,0,0,0.04)' }, r.is_me && { backgroundColor: '#FFF8E1', borderRadius: 10, paddingHorizontal: 8 }]}>
                  ...
                  <Text style={{ fontSize: 14, fontWeight: r.is_me ? '900' : '600', color: '#1C1917', flex: 1 }}>{r.is_me ? 'Vous' : `Prescripteur #${r.position}`}</Text>
              ...
              <View style={{ backgroundColor: 'rgba(0,0,0,0.03)', borderRadius: 14, padding: 14, marginTop: 16 }}>
                <Text style={{ fontSize: 13, fontWeight: '800', color: '#1C1917', marginBottom: 6 }}>Regles du programme</Text>
              ...
              {data.history?.length > 0 && (
                <View style={{ marginTop: 16 }}>
                  <Text style={{ fontSize: 14, fontWeight: '800', color: '#1C1917', marginBottom: 10 }}>Historique</Text>
                  {data.history.map((h: any, i: number) => (
                    <View key={i} style={{ flexDirection: 'row', alignItems: 'center', gap: 8, paddingVertical: 8 }}>
                      <Text style={{ fontSize: 12, color: '#78716C', width: 60 }}>{h.month}</Text>
                      <Icon name="trophy" size={14} color="#FFD700" />
                      <Text style={{ fontSize: 12, fontWeight: '700', color: '#1C1917' }}>{h.winner_name || 'Prescripteur #1'}</Text>
                    </View>
                  ))}
                </View>
              )}
    ...
    <ScrollView style={d.sv} contentContainerStyle={[d.sc, { paddingBottom: 80 }]} showsVerticalScrollIndicator={false}>
      {!user?.is_prescriber ? (
        <View style={{ backgroundColor: '#FFFFFF', borderRadius: 24, borderWidth: 1, borderColor: 'rgba(0,0,0,0.06)', padding: 28, alignItems: 'center', ...glass }}>
          ...
          <Text style={{ fontSize: 20, fontWeight: '800', color: '#1C1917', marginTop: 12 }}>Espace Prescripteur</Text>
          <Text style={{ fontSize: 13, color: '#78716C', textAlign: 'center', lineHeight: 19, marginTop: 8, marginBottom: 20 }}>
            Activez votre espace prescripteur avec le code fourni par votre structure partenaire Chutex.
          </Text>
          <TextInput testID="prescriber-code-input" style={{ width: '100%', backgroundColor: 'rgba(255,255,255,0.6)', borderRadius: 12, padding: 14, fontSize: 16, fontWeight: '700', color: '#1C1917', borderWidth: 1.5, borderColor: 'rgba(0,0,0,0.06)', textAlign: 'center', letterSpacing: 3, marginBottom: 10 }}
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10, width: '100%', marginVertical: 16 }}>
            ...
            <Text style={{ fontSize: 12, color: '#A8A29E' }}>ou</Text>
            <View style={{ flex: 1, height: 1, backgroundColor: 'rgba(0,0,0,0.08)' }} />
          </View>
        ...
        <>
              ...
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 14 }}>
                ...
                <View style={{ flex: 1 }}>
                  <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                    <Text style={{ fontSize: 17, fontWeight: '800', color: '#1C1917' }}>Prescripteur</Text>
            ...
            <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'flex-end' }}>
              <View style={{ backgroundColor: '#FFFFFF', borderTopLeftRadius: 24, borderTopRightRadius: 24, padding: 24, maxHeight: '85%' }}>
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
                  ...
                  <TouchableOpacity onPress={() => setShowPrescModal(false)}><Icon name="close" size={24} color="#1C1917" /></TouchableOpacity>
                </View>
                <ScrollView showsVerticalScrollIndicator={false}>
                  <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12, marginBottom: 16 }}>
                    ...
                    <View style={{ flex: 1 }}>
                      ...
                      <Text style={{ fontSize: 12, color: '#78716C', marginTop: 2 }}>{user.prescriber_structure}</Text>
                    </View>
                  </View>
                  {[
                    ...
                    <View key={label} style={{ flexDirection: 'row', alignItems: 'center', gap: 10, paddingVertical: 10, borderBottomWidth: 0.5, borderBottomColor: 'rgba(0,0,0,0.04)' }}>
                      ...
                      <Text style={{ fontSize: 12, color: '#78716C', width: 90 }}>{label}</Text>
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingVertical: 12, paddingHorizontal: 4, marginBottom: 4 }}>
            <Text style={{ fontSize: 12, color: '#78716C' }}>Total commissions {prescTab === 'pending' ? 'en attente' : 'validees'}</Text>
          ...
          <TouchableOpacity testID="new-prescription-btn" style={{ backgroundColor: '#FAF8F5', borderRadius: 14, paddingVertical: 14, alignItems: 'center', marginBottom: 16, flexDirection: 'row', justifyContent: 'center', gap: 8 }} onPress={() => setShowForm(true)}>
          ...
          {loading ? <ActivityIndicator size="large" color={Colors.primary} /> : displayedPresc.length > 0 ? (
              ...
              <TouchableOpacity key={p.id} testID={`prescription-${p.id}`} activeOpacity={0.7}
                ...
                <View style={{ backgroundColor: '#FFFFFF', borderRadius: 18, borderWidth: 1, borderColor: 'rgba(0,0,0,0.06)', padding: 16, marginBottom: 10, borderLeftWidth: 4, borderLeftColor: p.status === 'subscribed' ? '#4CAF50' : '#FF9800', ...glass }}>
                  <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10, marginBottom: 8 }}>
                    ...
                    <View style={{ flex: 1 }}>
                      <Text style={{ fontSize: 15, fontWeight: '700', color: '#1C1917' }}>{p.beneficiary_name}</Text>
                      <Text style={{ fontSize: 11, color: '#78716C' }}>{p.beneficiary_email}</Text>
                    </View>
                  ...
                  <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, paddingTop: 8, borderTopWidth: 0.5, borderTopColor: 'rgba(0,0,0,0.04)' }}>
                    ...
                    {p.beneficiary_phone && <><Icon name="call-outline" size={12} color="#888" /><Text style={{ fontSize: 11, color: '#78716C' }}>{p.beneficiary_phone}</Text></>}
                  </View>
                </View>
              </TouchableOpacity>
            ))
            ...
            <View style={{ backgroundColor: 'rgba(255,255,255,0.3)', borderRadius: 18, padding: 32, alignItems: 'center' }}>
              ...
              <Text style={{ fontSize: 14, fontWeight: '600', color: '#78716C', marginTop: 10 }}>{prescTab === 'pending' ? 'Aucune prescription en cours' : 'Aucune prescription validee'}</Text>
            </View>
          )}
            ...
            <View style={d.modalO}><View style={d.modalC}>
              ...
              <View style={d.modalBtns}>
                ...
                <TouchableOpacity testID="presc-submit-btn" style={d.submitBtn} onPress={submitPrescription} disabled={submitting}>
                  {submitting ? <ActivityIndicator color="#1C1917" /> : <Text style={d.submitBtnT}>Creer</Text>}</TouchableOpacity>
              </View>
            </View></View>
          </Modal>
                  ...
                  <ScrollView contentContainerStyle={{ paddingHorizontal: 16, paddingBottom: 40 }} showsVerticalScrollIndicator={false}>
                    ...
                    <View style={{ flexDirection: 'row', gap: 8, marginBottom: 12 }}>
                      ...
                      <View style={{ flex: 1, backgroundColor: '#FFFFFF', borderRadius: 24, borderWidth: 1, borderColor: 'rgba(0,0,0,0.06)', padding: 16, alignItems: 'center', ...glass }}>
                        <Text style={{ fontSize: 18, fontWeight: '900', color: '#1C1917' }}>{selectedPresc.subscription_type === 'standard' ? '15' : '25'}</Text>
                        <Text style={{ fontSize: 10, color: '#78716C', fontWeight: '700', letterSpacing: 0.5, marginTop: 2 }}>EUR/MOIS</Text>
                      </View>
                    </View>
        ...
        <View style={d.modalO}><View style={d.modalC}>
          ...
          <View style={d.modalBtns}>
            ...
            <TouchableOpacity style={d.submitBtn} onPress={saveCode} disabled={saving}>
              {saving ? <ActivityIndicator color="#1C1917" /> : <Text style={d.submitBtnT}>{editCode ? 'Modifier' : 'Créer'}</Text>}
            </TouchableOpacity>
          </View>
        </View></View>
      </Modal>
...
function CompanyPrescriptionsTab({ token }: { token: string }) {
  ...
  if (loading) return <View style={d.center}><ActivityIndicator size="large" color="#1C1917" /></View>;
      ...
      <ScrollView contentContainerStyle={{ paddingHorizontal: 16, paddingBottom: 80 }}
        ...
        {displayedPrescs.length === 0 && (
          <View style={{ alignItems: 'center', paddingVertical: 40 }}>
            <Icon name={prescTab === 'pending' ? 'time-outline' : 'checkmark-circle-outline'} size={36} color="#CCC" />
            <Text style={{ fontSize: 14, color: '#78716C', marginTop: 8 }}>Aucune prescription {prescTab === 'pending' ? 'en cours' : 'validee'}</Text>
          </View>
        )}
      </ScrollView>
...
export default function DevicesScreen() {
  ...
  return (
    <View key={r} style={[d.safeArea, { backgroundColor: '#FAF8F5' }]} testID="devices-screen">
      <View style={d.header}>
        <Text style={[d.title, { color: '#1C1917' }]}>{r === 'admin' ? 'Prescripteurs' : r === 'prescriber_company' ? 'Prescriptions' : r === 'guardian' ? 'Prescriptions' : r === 'teleassistance' ? 'Abonnes' : 'Mes Appareils'}</Text>
      </View>
