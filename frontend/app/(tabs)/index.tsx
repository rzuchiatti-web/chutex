    ...
    return (
      <div data-testid="company-dashboard" style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, display: 'flex', flexDirection: 'column', fontFamily: 'Inter, system-ui, sans-serif', overflow: 'hidden' } as any}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', position: 'relative', zIndex: 5, padding: '20px 20px 100px', WebkitOverflowScrolling: 'touch' } as any}>
          ...
          {/* Active interventions — summary violet card */}
          {activeIvs.length > 0 && (<div onClick={() => router.push('/(tabs)/teleconsult' as any)} style={{ borderRadius: 20, overflow: 'hidden', position: 'relative', padding: '24px 20px', marginBottom: 10, cursor: 'pointer', textAlign: 'center' } as any}><img src={BG_VIOLET} alt="" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover', zIndex: 0 } as any} /><div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.25)', zIndex: 1 } as any} /><div style={{ position: 'relative', zIndex: 2 } as any}><div style={{ fontSize: 48, fontWeight: 900, color: '#FFF', letterSpacing: -2 }}>{activeIvs.length}</div><div style={{ fontSize: 14, fontWeight: 700, color: 'rgba(255,255,255,0.8)', marginTop: 2 }}>intervention{activeIvs.length > 1 ? 's' : ''} en cours</div><div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '6px 14px', borderRadius: 999, background: 'rgba(255,255,255,0.12)', border: '1px solid rgba(255,255,255,0.2)', marginTop: 10 } as any}><i className="ri-eye-line" style={{ fontSize: 14, color: '#FFF' }} /><span style={{ fontSize: 12, fontWeight: 600, color: '#FFF' }}>Voir les interventions</span></div></div></div>)}
          {/* Ranking */}
