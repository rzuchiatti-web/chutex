    ...
    return (
      <div data-testid="guardian-dashboard" style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, display: 'flex', flexDirection: 'column', fontFamily: 'Inter, system-ui, sans-serif', overflow: 'hidden' } as any}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', position: 'relative', zIndex: 5, padding: '20px 20px 100px', WebkitOverflowScrolling: 'touch' } as any}>
          ...
          <div style={{ display: 'flex', gap: 8, marginBottom: 14 } as any}>{[{ val: bens.length, label: 'Beneficiaires', icon: 'ri-user-heart-line' }, { val: activeAlerts.length, label: 'Alertes', icon: 'ri-alarm-warning-line' }, { val: pendingInterventions.length, label: 'Interventions', icon: 'ri-map-pin-range-line' }].map((s, i) => (<div key={i} style={{ flex: 1, padding: '14px 10px', borderRadius: 18, background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', backdropFilter: 'blur(12px)', WebkitBackdropFilter: 'blur(12px)', textAlign: 'center' } as any}><i className={s.icon} style={{ fontSize: 18, color: 'rgba(255,255,255,0.4)', display: 'block', marginBottom: 4 }} /><div style={{ fontSize: 24, fontWeight: 900, color: '#FFF' }}>{s.val}</div><div style={{ fontSize: 9, color: 'rgba(255,255,255,0.35)', textTransform: 'uppercase', letterSpacing: 0.5 }}>{s.label}</div></div>))}</div>
          {activeAlertsG.map((a: any) => (<div key={a.id} onClick={() => router.push({ pathname: '/alert-detail', params: { alertId: a.id } })} style={{ padding: '14px 16px', borderRadius: 18, background: 'rgba(239,68,68,0.12)', border: '1px solid rgba(239,68,68,0.2)', marginBottom: 10, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 12 } as any}><div style={{ width: 40, height: 40, borderRadius: 12, background: '#EF4444', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 } as any}><i className="ri-alarm-warning-line" style={{ fontSize: 20, color: '#FFF' }} /></div><div style={{ flex: 1 } as any}><div style={{ fontSize: 13, fontWeight: 800, color: '#EF4444' }}>ALERTE - {a.beneficiary_name}</div><div style={{ fontSize: 11, color: 'rgba(255,255,255,0.5)' }}>{a.message}</div></div><i className="ri-arrow-right-s-line" style={{ fontSize: 18, color: 'rgba(255,255,255,0.3)' }} /></div>))}
    ...
    return (
      <div data-testid="company-dashboard" style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, display: 'flex', flexDirection: 'column', fontFamily: 'Inter, system-ui, sans-serif', overflow: 'hidden' } as any}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', position: 'relative', zIndex: 5, padding: '20px 20px 100px', WebkitOverflowScrolling: 'touch' } as any}>
          {/* Header */}
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 } as any}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 } as any}><div style={{ width: 46, height: 46, borderRadius: 999, background: 'rgba(212,132,90,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', border: '2px solid rgba(212,132,90,0.3)' } as any}><i className="ri-building-line" style={{ fontSize: 20, color: '#D4845A' }} /></div><div><div style={{ fontSize: 16, fontWeight: 800, color: '#FFF' }}>{user.company_name || user.name}</div><span style={{ fontSize: 11, color: 'rgba(255,255,255,0.5)' }}>Structure SAAD</span></div></div>
          </div>
          {/* Stats */}
          ...
          {/* Quick actions */}
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8, marginBottom: 14 } as any}>{[{ icon: 'ri-group-line', label: 'Intervenants', route: '/(tabs)/teleconsult' }, { icon: 'ri-file-text-line', label: 'Prescriptions', route: '/(tabs)/devices' }, { icon: 'ri-alarm-warning-line', label: 'Alertes', route: '/(tabs)/alerts' }].map((a, i) => (<div key={i} onClick={() => router.push(a.route as any)} style={{ padding: '14px 8px', borderRadius: 16, background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.08)', textAlign: 'center', cursor: 'pointer' } as any}><i className={a.icon} style={{ fontSize: 18, color: 'rgba(255,255,255,0.5)', display: 'block', marginBottom: 4 }} /><div style={{ fontSize: 11, fontWeight: 600, color: 'rgba(255,255,255,0.5)' }}>{a.label}</div></div>))}</div>
          {/* Active alerts — red cards */}
          {activeAlerts.map((a: any) => (<div key={a.id} onClick={() => router.push({ pathname: '/alert-detail', params: { alertId: a.id } })} style={{ borderRadius: 20, overflow: 'hidden', position: 'relative', padding: '16px', marginBottom: 10, cursor: 'pointer' } as any}><img src={BG_RED} alt="" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover', zIndex: 0 } as any} /><div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.2)', zIndex: 1 } as any} /><div style={{ position: 'relative', zIndex: 2, display: 'flex', alignItems: 'center', gap: 12 } as any}><div style={{ width: 38, height: 38, borderRadius: 12, background: 'rgba(239,68,68,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center' } as any}><i className="ri-alarm-warning-line" style={{ fontSize: 18, color: '#FFF' }} /></div><div style={{ flex: 1 } as any}><div style={{ fontSize: 14, fontWeight: 800, color: '#FFF' }}>{a.beneficiary_name}</div><div style={{ fontSize: 11, color: 'rgba(255,255,255,0.6)' }}>{a.message}</div></div><div style={{ padding: '3px 8px', borderRadius: 999, background: 'rgba(239,68,68,0.25)' } as any}><span style={{ fontSize: 9, fontWeight: 600, color: '#FFF' }}>Active</span></div></div></div>))}
          {/* Active interventions — violet cards */}
          {activeIvs.map((iv: any) => (<div key={iv.id} onClick={() => router.push({ pathname: '/company-intervention-detail', params: { interventionId: iv.id } })} style={{ borderRadius: 20, overflow: 'hidden', position: 'relative', padding: '16px', marginBottom: 10, cursor: 'pointer' } as any}><img src={BG_VIOLET} alt="" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover', zIndex: 0 } as any} /><div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.2)', zIndex: 1 } as any} /><div style={{ position: 'relative', zIndex: 2, display: 'flex', alignItems: 'center', gap: 12 } as any}><div style={{ width: 38, height: 38, borderRadius: 12, background: 'rgba(124,92,255,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center' } as any}><i className="ri-map-pin-range-line" style={{ fontSize: 18, color: '#FFF' }} /></div><div style={{ flex: 1 } as any}><div style={{ fontSize: 14, fontWeight: 800, color: '#FFF' }}>{iv.beneficiary_name || 'Intervention'}</div><div style={{ fontSize: 11, color: 'rgba(255,255,255,0.6)' }}>{iv.assigned_name || 'En attente'}{iv.distance_km ? ` · ${iv.distance_km}km` : ''}</div></div><div style={{ padding: '3px 8px', borderRadius: 999, background: 'rgba(124,92,255,0.25)' } as any}><span style={{ fontSize: 9, fontWeight: 600, color: '#FFF' }}>En cours</span></div></div></div>))}
          {/* Ranking */}
          ...
          {/* Recent prescriptions — orange cards */}
          {prescriptions.length > 0 && (<><div style={{ fontSize: 11, fontWeight: 700, color: 'rgba(255,255,255,0.25)', letterSpacing: 1.2, textTransform: 'uppercase', marginBottom: 10, marginTop: 4 }}>Prescriptions recentes</div>{prescriptions.slice(0, 3).map((p: any) => (<div key={p.id} style={{ borderRadius: 18, overflow: 'hidden', position: 'relative', padding: '14px 16px', marginBottom: 8 } as any}><img src={BG_ORANGE} alt="" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover', zIndex: 0 } as any} /><div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.2)', zIndex: 1 } as any} /><div style={{ position: 'relative', zIndex: 2, display: 'flex', alignItems: 'center', gap: 12 } as any}><div style={{ flex: 1 } as any}><div style={{ fontSize: 13, fontWeight: 700, color: '#FFF' }}>{p.beneficiary_name || 'Prescription'}</div><div style={{ fontSize: 11, color: 'rgba(255,255,255,0.6)' }}>{p.prescriber_name} · {p.status === 'validated' ? 'Validee' : 'En attente'}</div></div><div style={{ padding: '3px 8px', borderRadius: 999, background: p.status === 'validated' ? 'rgba(16,185,129,0.25)' : 'rgba(245,158,11,0.25)' } as any}><span style={{ fontSize: 9, fontWeight: 600, color: '#FFF' }}>{p.status === 'validated' ? 'Validee' : 'Pending'}</span></div></div></div>))}</>)}
        </div>
      </div>
    );
  }
