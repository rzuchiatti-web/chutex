    ...
    return (
      <div data-testid="company-dashboard" style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, display: 'flex', flexDirection: 'column', fontFamily: 'Inter, system-ui, sans-serif', overflow: 'hidden' } as any}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', position: 'relative', zIndex: 5, padding: '20px 20px 100px', WebkitOverflowScrolling: 'touch' } as any}>
          ...
          {/* Active alerts — red cards (only if active) */}
          ...
          <div onClick={() => router.push('/(tabs)/teleconsult' as any)} style={{ borderRadius: 22, overflow: 'hidden', position: 'relative', padding: '20px', marginBottom: 12, cursor: 'pointer' } as any}>
            ...
            <div style={{ position: 'relative', zIndex: 2 } as any}>
              ...
              {activeIvs.length > 0 && <div style={{ height: 1, background: 'rgba(255,255,255,0.1)', marginBottom: 10 } as any} />}
              {activeIvs.slice(0, 2).map((iv: any) => (<div key={iv.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 0' } as any}><div style={{ width: 28, height: 28, borderRadius: 999, background: 'rgba(255,255,255,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center' } as any}><span style={{ fontSize: 11, fontWeight: 800, color: '#FFF' }}>{(iv.assigned_name || '?').charAt(0)}</span></div><div style={{ flex: 1 } as any}><span style={{ fontSize: 12, fontWeight: 600, color: '#FFF' }}>{iv.assigned_name || 'En attente'}</span><span style={{ fontSize: 11, color: 'rgba(255,255,255,0.4)', marginLeft: 8 }}>{iv.beneficiary_name}</span></div></div>))}
            </div>
          </div>
          ...
          <div onClick={() => router.push('/(tabs)/devices' as any)} style={{ borderRadius: 22, overflow: 'hidden', position: 'relative', padding: '20px', marginBottom: 12, cursor: 'pointer' } as any}>
            ...
            <div style={{ position: 'relative', zIndex: 2 } as any}>
              ...
              <div style={{ display: 'flex', gap: 10, marginBottom: 12 } as any}>
                {[{ val: prescriptions.filter((p: any) => p.status === 'validated').length, label: 'Validees', color: '#10B981' }, { val: prescriptions.filter((p: any) => p.status !== 'validated').length, label: 'En attente', color: '#F59E0B' }, { val: prescribers.length, label: 'Prescripteurs', color: 'rgba(255,255,255,0.4)' }].map((s, i) => (
                  <div key={i} style={{ flex: 1, textAlign: 'center' } as any}><div style={{ fontSize: 28, fontWeight: 900, color: s.color }}>{s.val}</div><div style={{ fontSize: 9, color: 'rgba(255,255,255,0.4)', textTransform: 'uppercase', letterSpacing: 0.5 }}>{s.label}</div></div>
                ))}
              </div>
              {prescriptions.length > 0 && <div style={{ height: 1, background: 'rgba(255,255,255,0.1)', marginBottom: 10 } as any} />}
              {prescriptions.slice(0, 2).map((p: any) => (<div key={p.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '6px 0' } as any}><div style={{ flex: 1 } as any}><span style={{ fontSize: 12, fontWeight: 600, color: '#FFF' }}>{p.beneficiary_name || 'Souscription'}</span><span style={{ fontSize: 11, color: 'rgba(255,255,255,0.4)', marginLeft: 8 }}>{p.prescriber_name}</span></div><div style={{ padding: '2px 8px', borderRadius: 999, background: p.status === 'validated' ? 'rgba(16,185,129,0.2)' : 'rgba(245,158,11,0.2)' } as any}><span style={{ fontSize: 9, fontWeight: 600, color: p.status === 'validated' ? '#10B981' : '#F59E0B' }}>{p.status === 'validated' ? 'Validee' : 'Pending'}</span></div></div>))}
            </div>
          </div>
