    ...
    return (
      <div data-testid="beneficiary-dashboard" style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, display: 'flex', flexDirection: 'column', fontFamily: "'Inter', system-ui, -apple-system, sans-serif", overflow: 'hidden' } as any}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', position: 'relative', zIndex: 5, padding: '20px 20px 100px', WebkitOverflowScrolling: 'touch' } as any}>
          ...
          {/* ── Language GLASS POPUP — horizontal scroll picker ── */}
          {langOpen && (
            <div onClick={() => setLangOpen(false)} style={{ position: 'fixed', inset: 0, zIndex: 9999, backdropFilter: 'blur(32px)', WebkitBackdropFilter: 'blur(32px)', background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center' } as any}>
              <div onClick={(e: any) => e.stopPropagation()} style={{ width: '92%', maxWidth: 380, padding: '28px 20px', borderRadius: 28, background: 'rgba(15,23,42,0.92)', border: '1px solid rgba(255,255,255,0.08)', backdropFilter: 'blur(24px)', WebkitBackdropFilter: 'blur(24px)', boxShadow: '0 24px 48px rgba(0,0,0,0.4)' } as any}>
                <div style={{ textAlign: 'center', marginBottom: 24 } as any}>
                  <div style={{ fontSize: 20, fontWeight: 900, color: '#FFF', marginBottom: 4 }}>Langue</div>
                ...
                <div style={{ display: 'flex', gap: 10, overflowX: 'auto', padding: '8px 0 16px', justifyContent: 'center', scrollbarWidth: 'none' } as any}>
                  {[
                    ...
                    <div key={l.code} onClick={() => { setLang(l.code); setLangOpen(false); }} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6, cursor: 'pointer', flexShrink: 0, padding: '10px 8px', borderRadius: 16, background: lang === l.code ? 'rgba(255,255,255,0.1)' : 'transparent', border: lang === l.code ? '1px solid rgba(255,255,255,0.15)' : '1px solid transparent', minWidth: 62, transition: 'all 0.2s' } as any}>
                      <span style={{ fontSize: 32, lineHeight: 1 }}>{l.flag}</span>
                      <span style={{ fontSize: 10, fontWeight: lang === l.code ? 700 : 500, color: lang === l.code ? '#FFF' : 'rgba(255,255,255,0.4)', textAlign: 'center' }}>{l.name}</span>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <div data-testid="programs-section" style={{ marginBottom: 16 } as any}>
            ...
            {activeProgram?.active ? (
              <>
              {/* Active program card */}
              <div data-testid="active-program-card" onClick={() => router.push('/programs' as any)} style={{ borderRadius: 20, background: 'rgba(255,255,255,0.04)', border: `1px solid ${activeProgram.program.color}25`, padding: '18px', marginBottom: 10, cursor: 'pointer', transition: 'transform 0.2s' } as any}
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 14 } as any}>
                  ...
                  <div style={{ flex: 1 } as any}>
                    ...
                    <div style={{ fontSize: 11, color: activeProgram.program.color, fontWeight: 600 }}>Jour {activeProgram.current_day}/{activeProgram.program.duration_days} {activeProgram.current_phase ? `· ${activeProgram.current_phase.name}` : ''}</div>
                  </div>
                  <div style={{ fontSize: 22, fontWeight: 900, color: activeProgram.program.color }}>{activeProgram.progress_pct}%</div>
                </div>
                {/* Progress bar */}
                <div style={{ height: 6, borderRadius: 3, background: 'rgba(255,255,255,0.06)', overflow: 'hidden', marginBottom: 14 } as any}>
                  <div style={{ height: 6, borderRadius: 3, width: `${activeProgram.progress_pct}%`, background: `linear-gradient(90deg, ${activeProgram.program.color}80, ${activeProgram.program.color})`, transition: 'width 0.5s ease' } as any} />
                </div>
                {/* Today's focus */}
                {activeProgram.today_tasks && (
                  <div style={{ padding: '12px 14px', borderRadius: 14, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)' } as any}>
                    <div style={{ fontSize: 12, fontWeight: 700, color: '#FFF', marginBottom: 8 }}>{activeProgram.today_tasks.focus}</div>
                    {activeProgram.today_tasks.tasks?.map((task: string, ti: number) => (
                      <div key={ti} style={{ display: 'flex', alignItems: 'flex-start', gap: 8, marginBottom: 4 } as any}>
                        <div style={{ width: 16, height: 16, borderRadius: 4, border: '1.5px solid rgba(255,255,255,0.15)', flexShrink: 0, marginTop: 1 } as any} />
                        <span style={{ fontSize: 12, color: 'rgba(255,255,255,0.5)', lineHeight: 1.4 }}>{task}</span>
                      </div>
                    ))}
                    {activeProgram.today_tasks.tip && (
                      <div style={{ marginTop: 10, padding: '8px 10px', borderRadius: 10, background: `${activeProgram.program.color}08`, border: `1px solid ${activeProgram.program.color}15`, fontSize: 11, color: 'rgba(255,255,255,0.4)', fontStyle: 'italic' } as any}>
                        <i className="ri-lightbulb-line" style={{ marginRight: 4, color: activeProgram.program.color }} />{activeProgram.today_tasks.tip}
                      </div>
                    )}
                  </div>
                )}
              ...
              <div style={{ display: 'flex', gap: 10, overflowX: 'auto', paddingBottom: 4 } as any}>
                {programCatalog.map((p: any) => (
                  <div key={p.id} data-testid={`program-${p.id}`} onClick={async () => {
                    try {
                      await apiFetch(`/api/programs/start/${p.id}`, { method: 'POST' }, token);
                      fetchData();
                    } catch (e: any) { Alert.alert('Erreur', e.message); }
                  }} style={{ minWidth: 160, padding: '16px', borderRadius: 18, background: 'rgba(255,255,255,0.04)', border: `1px solid ${p.color}20`, cursor: 'pointer', flexShrink: 0, transition: 'transform 0.2s' } as any}
                    onMouseEnter={(e: any) => e.currentTarget.style.transform = 'translateY(-2px)'}
                    onMouseLeave={(e: any) => e.currentTarget.style.transform = ''}>
                    <div style={{ width: 36, height: 36, borderRadius: 10, background: `${p.color}15`, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 10 } as any}>
                      <i className={p.icon} style={{ fontSize: 18, color: p.color }} />
                    </div>
                    <div style={{ fontSize: 13, fontWeight: 800, color: '#FFF', marginBottom: 4, lineHeight: 1.3 }}>{p.title}</div>
                    <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.3)' }}>{p.duration_days} jours · {p.difficulty}</div>
                  </div>
                ))}
              </div>
            )}
