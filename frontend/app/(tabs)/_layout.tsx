...
export default function TabLayout() {
  ...
  return (
    <Tabs key={r} sceneContainerStyle={{ backgroundColor: 'transparent' }} screenOptions={{
      ...
      <Tabs.Screen name="health" options={{ tabBarIcon: ({ color, size }) => isAdmin ? <Icon name="people-outline" size={size} color={color} /> : isCompany ? <Icon name="business-outline" size={size} color={color} /> : <MCIcon name="heart-pulse" size={size} color={color} />, href: isCompany ? '/company-agency' : (!isBen && !isAdmin && !isCompany) ? null : undefined }} />
