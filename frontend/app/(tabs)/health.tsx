              ...
              <div data-testid="health-active-program" onClick={() => router.push('/programs' as any)} style={{ borderRadius: 22, background: `${pg.color}08`, border: `1px solid ${pg.color}20`, padding: '18px', marginBottom: 14, cursor: 'pointer', transition: 'transform 0.2s' } as any}
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12 } as any}>
                  ...
                  <div style={{ textAlign: 'right' } as any}>
                    <div style={{ fontSize: 22, fontWeight: 900, color: pg.color }}>{healthProgData.progress_pct}%</div>
                    <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.3)' }}>J{healthProgData.current_day}/{pg.duration_days}</div>
                  </div>
                </div>
                <div style={{ height: 6, borderRadius: 3, background: 'rgba(255,255,255,0.06)', overflow: 'hidden', marginBottom: 12 } as any}><div style={{ height: 6, borderRadius: 3, width: `${healthProgData.progress_pct}%`, background: `linear-gradient(90deg, ${pg.color}80, ${pg.color})` } as any} /></div>
                {healthProgData.today_tasks && (
                  <div style={{ padding: '12px 14px', borderRadius: 14, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.05)' } as any}>
                    ...
                    <div style={{ fontSize: 14, fontWeight: 800, color: '#FFF', marginBottom: 6 }}>{healthProgData.today_tasks.focus}</div>
                    {healthProgData.today_tasks.tasks?.slice(0, 2).map((t: string, i: number) => (
                ...
                {healthTeamData?.has_team && (
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 10, padding: '6px 10px', borderRadius: 10, background: 'rgba(167,139,250,0.08)', border: '1px solid rgba(167,139,250,0.15)' } as any}>
                    ...
                    <span style={{ fontSize: 11, fontWeight: 600, color: 'rgba(255,255,255,0.5)' }}>En equipe · {healthTeamData.members?.length} membres</span>
                  </div>
                )}
              </div>
            );
