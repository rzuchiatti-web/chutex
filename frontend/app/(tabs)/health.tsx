...
export default function HealthScreen() {
  ...
  const metrics = [
    ...
    { id: 'spo2', label: 'Saturation O2', value: simVitals.spo2, unit: '%', icon: 'ri-drop-line', color: '#38BDF8', range: '95-100', status: simVitals.spo2 >= 95 ? 'normal' : 'alerte' },
    { id: 'blood_pressure', label: 'Tension arterielle', value: `${simVitals.systolic}/${simVitals.diastolic}`, unit: 'mmHg', icon: 'ri-pulse-line', color: '#A78BFA', range: '120/80', status: 'normal' },
  ...
  const hrHistory = br.heart_rate_history || [];
  const maxHR = hrHistory.length > 0 ? Math.max(...hrHistory.map((h: any) => h.value)) : 82;
  const weightHist = sc.weight_history || [];
  const maxW = weightHist.length > 0 ? Math.max(...weightHist.map((w: any) => w.value)) : 73;
  const minW = weightHist.length > 0 ? Math.min(...weightHist.map((w: any) => w.value)) : 72;

  /* Glass card helper */
  const GCard = ({ children, style, onClick, testId }: any) => (
    <div data-testid={testId} onClick={onClick} style={{ padding: '16px', borderRadius: 22, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)', marginBottom: 12, cursor: onClick ? 'pointer' : 'default', transition: 'transform 0.2s', ...style } as any}
      onMouseEnter={(e: any) => { if (onClick) e.currentTarget.style.transform = 'translateY(-2px)'; }}
      onMouseLeave={(e: any) => { if (onClick) e.currentTarget.style.transform = ''; }}>
      {children}
    </div>
  );
  ...
  const SectionTitle = ({ icon, label, color }: any) => (
    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10, marginTop: 4 } as any}>
      ...
      <span style={{ fontSize: 10, fontWeight: 700, color: `${color}99`, letterSpacing: 1.5, textTransform: 'uppercase' }}>{label}</span>
      <div style={{ flex: 1, height: 1, background: `${color}20`, marginLeft: 4 } as any} />
    </div>
  );

  /* ─── WEB: Redesigned health page ─── */
  if (Platform.OS === 'web' && effectiveRole === 'beneficiary') {
    return (
      <div data-testid="health-screen" style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, display: 'flex', flexDirection: 'column', fontFamily: "'Inter', system-ui, -apple-system, sans-serif", overflow: 'hidden' } as any}>
        <img src={BG_HEALTH} alt="" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover', zIndex: 0 } as any} />
        <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(180deg, rgba(4,14,26,0.3) 0%, rgba(4,14,26,0.55) 100%)', zIndex: 1 } as any} />
        ...
        <div style={{ flex: 1, overflowY: 'auto', position: 'relative', zIndex: 5, padding: '20px 20px 100px', WebkitOverflowScrolling: 'touch' } as any}>
          ...
          <div style={{ textAlign: 'center', marginBottom: 20 } as any}>
            <div style={{ fontSize: 24, fontWeight: 800, color: '#FFF', letterSpacing: -0.5 }}>Ma sante</div>
            <div style={{ fontSize: 12, color: 'rgba(79,195,247,0.6)', marginTop: 4 }}>Suivi complet en temps reel</div>
          </div>

          {/* ── BRACELET SECTION ── */}
          <SectionTitle icon="ri-pulse-line" label="Bracelet Elio" color="#22D3EE" />

          {/* Vitals grid */}
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 12 } as any}>
            {metrics.map(m => (
              <GCard key={m.id} testId={`health-metric-${m.id}`} onClick={() => router.push({ pathname: '/health-detail', params: { metricId: m.id } })}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 } as any}>
                  <div style={{ width: 34, height: 34, borderRadius: 10, background: `${m.color}18`, display: 'flex', alignItems: 'center', justifyContent: 'center' } as any}>
                    <i className={m.icon} style={{ fontSize: 16, color: m.color }} />
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '2px 8px', borderRadius: 999, background: m.status === 'normal' ? 'rgba(16,185,129,0.12)' : 'rgba(239,68,68,0.12)' } as any}>
                ...
                <div style={{ fontSize: 26, fontWeight: 900, color: '#FFF', letterSpacing: -1 }}>{m.value}<span style={{ fontSize: 12, fontWeight: 600, color: 'rgba(255,255,255,0.35)', marginLeft: 4 }}>{m.unit}</span></div>
                <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.35)', marginTop: 2 }}>{m.label}</div>
                <div style={{ fontSize: 9, color: 'rgba(255,255,255,0.2)', marginTop: 4 }}>Normal: {m.range}</div>
              </GCard>
            ))}
          </div>
            ...
            <GCard>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 } as any}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 } as any}>
                  <i className="ri-heart-pulse-line" style={{ fontSize: 14, color: '#EF4444' }} />
                  <span style={{ fontSize: 10, fontWeight: 700, color: 'rgba(255,255,255,0.3)', textTransform: 'uppercase', letterSpacing: 0.5 }}>Frequence cardiaque</span>
                </div>
                <span style={{ fontSize: 10, color: 'rgba(255,255,255,0.25)' }}>Aujourd'hui</span>
              </div>
              <div style={{ display: 'flex', alignItems: 'flex-end', gap: 6, height: 70 } as any}>
                {hrHistory.map((h: any, i: number) => (
                  <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3 } as any}>
                    <div style={{ fontSize: 8, fontWeight: 700, color: 'rgba(255,255,255,0.45)' }}>{h.value}</div>
                    <div style={{ width: '100%', borderRadius: 3, background: i === hrHistory.length - 1 ? 'linear-gradient(180deg, #EF4444, #991B1B)' : 'rgba(239,68,68,0.2)', height: `${Math.max(8, (h.value / maxHR) * 50)}px`, transition: 'height 0.5s', boxShadow: i === hrHistory.length - 1 ? '0 0 10px rgba(239,68,68,0.4)' : 'none' } as any} />
                    <div style={{ fontSize: 7, color: 'rgba(255,255,255,0.25)' }}>{h.hour}</div>
                  </div>
                ))}
              </div>
            </GCard>
          ...
          <GCard>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 } as any}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 } as any}>
                ...
                <span style={{ fontSize: 10, fontWeight: 700, color: 'rgba(255,255,255,0.3)', textTransform: 'uppercase', letterSpacing: 0.5 }}>Activite du jour</span>
              </div>
            </div>
            <div style={{ display: 'flex', gap: 12 } as any}>
              {[
                { val: br.steps?.toLocaleString(), label: 'Pas', color: '#10B981', pct: Math.min(100, (br.steps / 8000) * 100) },
                { val: br.calories, label: 'Kcal', color: '#F59E0B', pct: Math.min(100, (br.calories / 400) * 100) },
                { val: `${br.distance_km}`, label: 'Km', color: '#38BDF8', pct: Math.min(100, (br.distance_km / 5) * 100) },
              ].map((s, i) => (
                <div key={i} style={{ flex: 1, textAlign: 'center' } as any}>
                  <div style={{ fontSize: 20, fontWeight: 900, color: '#FFF' }}>{s.val}</div>
                  <div style={{ fontSize: 9, color: 'rgba(255,255,255,0.35)', marginBottom: 6 }}>{s.label}</div>
                  <div style={{ height: 4, borderRadius: 2, background: 'rgba(255,255,255,0.06)', overflow: 'hidden' } as any}>
                    <div style={{ height: 4, borderRadius: 2, width: `${s.pct}%`, background: s.color, transition: 'width 1s', boxShadow: `0 0 6px ${s.color}40` } as any} />
                  </div>
                </div>
          ...
          {/* ── BALANCE SECTION ── */}
          <SectionTitle icon="ri-scales-3-line" label="Balance Lefu" color="#A78BFA" />

          {/* Weight + BMI + Body composition */}
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 12 } as any}>
            <GCard testId="scale-weight" onClick={() => router.push('/scale-detail' as any)}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 } as any}>
                <div style={{ width: 34, height: 34, borderRadius: 10, background: 'rgba(167,139,250,0.12)', display: 'flex', alignItems: 'center', justifyContent: 'center' } as any}>
                  <i className="ri-scales-3-line" style={{ fontSize: 16, color: '#A78BFA' }} />
                </div>
              </div>
              <div style={{ fontSize: 28, fontWeight: 900, color: '#FFF' }}>{sc.weight}<span style={{ fontSize: 12, color: 'rgba(255,255,255,0.35)', marginLeft: 4 }}>kg</span></div>
              <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.35)' }}>Poids</div>
            </GCard>
            <GCard testId="scale-bmi">
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 } as any}>
                <div style={{ width: 34, height: 34, borderRadius: 10, background: 'rgba(56,189,248,0.12)', display: 'flex', alignItems: 'center', justifyContent: 'center' } as any}>
                  <i className="ri-bar-chart-box-line" style={{ fontSize: 16, color: '#38BDF8' }} />
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '2px 8px', borderRadius: 999, background: sc.bmi < 25 ? 'rgba(16,185,129,0.12)' : 'rgba(245,158,11,0.12)' } as any}>
                  <span style={{ fontSize: 9, fontWeight: 600, color: sc.bmi < 25 ? '#10B981' : '#F59E0B' }}>{sc.bmi < 25 ? 'Normal' : 'Surpoids'}</span>
                </div>
              </div>
              <div style={{ fontSize: 28, fontWeight: 900, color: '#FFF' }}>{sc.bmi}</div>
              <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.35)' }}>IMC</div>
            </GCard>
          </div>

          {/* Body composition details */}
          <GCard testId="body-composition">
            <div style={{ fontSize: 10, fontWeight: 700, color: 'rgba(255,255,255,0.3)', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 12 }}>Composition corporelle</div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8 } as any}>
              {[
                { label: 'Graisse', value: `${sc.body_fat}%`, color: '#F59E0B', icon: 'ri-droplet-line' },
                { label: 'Muscle', value: `${sc.muscle_mass}%`, color: '#10B981', icon: 'ri-heart-pulse-line' },
                { label: 'Eau', value: `${sc.water_pct}%`, color: '#38BDF8', icon: 'ri-drop-line' },
                { label: 'Os', value: `${sc.bone_mass}kg`, color: '#A78BFA', icon: 'ri-shield-line' },
                { label: 'Graisse visc.', value: `${sc.visceral_fat}`, color: '#EF4444', icon: 'ri-fire-line' },
                { label: 'Age metab.', value: `${sc.metabolic_age} ans`, color: '#22D3EE', icon: 'ri-timer-line' },
              ].map((c, i) => (
                <div key={i} style={{ padding: '10px 8px', borderRadius: 12, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.05)', textAlign: 'center' } as any}>
                  <i className={c.icon} style={{ fontSize: 14, color: c.color, marginBottom: 4, display: 'block' }} />
                  <div style={{ fontSize: 14, fontWeight: 800, color: '#FFF' }}>{c.value}</div>
                  <div style={{ fontSize: 8, color: 'rgba(255,255,255,0.3)', marginTop: 2 }}>{c.label}</div>
                </div>
              ))}
            </div>
          </GCard>
          ...
          {weightHist.length > 0 && (
            <GCard testId="weight-history">
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 } as any}>
                <span style={{ fontSize: 10, fontWeight: 700, color: 'rgba(255,255,255,0.3)', textTransform: 'uppercase', letterSpacing: 0.5 }}>Evolution du poids</span>
                <span style={{ fontSize: 10, color: 'rgba(255,255,255,0.25)' }}>7 derniers jours</span>
              </div>
              <div style={{ display: 'flex', alignItems: 'flex-end', gap: 6, height: 60 } as any}>
                {weightHist.map((w: any, i: number) => {
                  const range = maxW - minW || 1;
                  const pct = ((w.value - minW) / range) * 40 + 20;
                  return (
                    <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3 } as any}>
                      <div style={{ fontSize: 8, fontWeight: 700, color: 'rgba(255,255,255,0.4)' }}>{w.value}</div>
                      <div style={{ width: '100%', borderRadius: 3, background: i === weightHist.length - 1 ? 'linear-gradient(180deg, #A78BFA, #6D28D9)' : 'rgba(167,139,250,0.2)', height: `${pct}px`, transition: 'height 0.5s' } as any} />
                      <div style={{ fontSize: 7, color: 'rgba(255,255,255,0.2)' }}>{w.date}</div>
                    </div>
                  );
                })}
              </div>
          ...
          {/* ── GILET SECTION ── */}
          <SectionTitle icon="ri-shield-check-line" label="Gilet CareWatch" color="#10B981" />

          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 12 } as any}>
            <GCard testId="vest-posture" onClick={() => router.push('/vest-connect' as any)}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 } as any}>
                <div style={{ width: 34, height: 34, borderRadius: 10, background: 'rgba(16,185,129,0.12)', display: 'flex', alignItems: 'center', justifyContent: 'center' } as any}>
                  <i className="ri-body-scan-line" style={{ fontSize: 16, color: '#10B981' }} />
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '2px 8px', borderRadius: 999, background: vs.posture_score >= 80 ? 'rgba(16,185,129,0.12)' : 'rgba(245,158,11,0.12)' } as any}>
                  <span style={{ fontSize: 9, fontWeight: 600, color: vs.posture_score >= 80 ? '#10B981' : '#F59E0B' }}>{vs.posture_score >= 80 ? 'Bon' : 'A ameliorer'}</span>
                </div>
              </div>
              <div style={{ fontSize: 28, fontWeight: 900, color: '#FFF' }}>{vs.posture_score}<span style={{ fontSize: 12, color: 'rgba(255,255,255,0.35)', marginLeft: 2 }}>%</span></div>
              <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.35)' }}>Score posture</div>
            </GCard>
            <GCard testId="vest-fall">
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 } as any}>
                <div style={{ width: 34, height: 34, borderRadius: 10, background: vs.fall_detected ? 'rgba(239,68,68,0.15)' : 'rgba(16,185,129,0.12)', display: 'flex', alignItems: 'center', justifyContent: 'center' } as any}>
                  <i className={vs.fall_detected ? 'ri-alarm-warning-line' : 'ri-shield-check-line'} style={{ fontSize: 16, color: vs.fall_detected ? '#EF4444' : '#10B981' }} />
                </div>
              </div>
              <div style={{ fontSize: 20, fontWeight: 900, color: '#FFF' }}>{vs.fall_detected ? 'Detectee !' : 'Aucune'}</div>
              <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.35)' }}>Chute detectee</div>
            </GCard>
          </div>

          <GCard testId="vest-details">
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8 } as any}>
              {[
                { label: 'Temp. thorax', value: `${vs.chest_temp}°C`, icon: 'ri-temp-hot-line', color: '#F59E0B' },
                { label: 'Port (h)', value: `${vs.wearing_hours_today}h`, icon: 'ri-time-line', color: '#38BDF8' },
                { label: 'Impacts', value: `${vs.impact_events_today}`, icon: 'ri-flashlight-line', color: '#A78BFA' },
              ].map((v, i) => (
                <div key={i} style={{ padding: '10px 8px', borderRadius: 12, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.05)', textAlign: 'center' } as any}>
                  <i className={v.icon} style={{ fontSize: 14, color: v.color, marginBottom: 4, display: 'block' }} />
                  <div style={{ fontSize: 14, fontWeight: 800, color: '#FFF' }}>{v.value}</div>
                  <div style={{ fontSize: 8, color: 'rgba(255,255,255,0.3)', marginTop: 2 }}>{v.label}</div>
                </div>
              ))}
            </div>
          </GCard>
          ...
          {/* ── SLEEP SECTION ── */}
          <SectionTitle icon="ri-moon-line" label="Sommeil" color="#A78BFA" />

          <GCard testId="sleep-detail" onClick={() => router.push('/sleep' as any)}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 } as any}>
              <div>
                <div style={{ fontSize: 24, fontWeight: 900, color: '#FFF' }}>{sl.duration}</div>
                <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.35)' }}>{sl.bedtime} - {sl.wakeup}</div>
              </div>
              <div style={{ padding: '6px 12px', borderRadius: 999, background: 'rgba(167,139,250,0.12)' } as any}>
                <span style={{ fontSize: 14, fontWeight: 800, color: '#A78BFA' }}>{sl.quality}%</span>
              </div>
            </div>
            <div style={{ display: 'flex', gap: 3, height: 8, borderRadius: 4, overflow: 'hidden', marginBottom: 12 } as any}>
              <div style={{ flex: 30, background: '#6D28D9', borderRadius: '4px 0 0 4px' } as any} />
              <div style={{ flex: 55, background: '#A78BFA' } as any} />
              <div style={{ flex: 15, background: '#C4B5FD', borderRadius: '0 4px 4px 0' } as any} />
            </div>
            <div style={{ display: 'flex', gap: 10 } as any}>
              {[
                { l: 'Profond', v: sl.deep, c: '#6D28D9' },
                { l: 'Leger', v: sl.light, c: '#A78BFA' },
                { l: 'REM', v: sl.rem, c: '#C4B5FD' },
              ].map((s, i) => (
                <div key={i} style={{ flex: 1 } as any}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 4 } as any}><span style={{ width: 5, height: 5, borderRadius: '50%', background: s.c } as any} /><span style={{ fontSize: 9, color: 'rgba(255,255,255,0.35)' }}>{s.l}</span></div>
                  <div style={{ fontSize: 12, fontWeight: 700, color: '#FFF', marginTop: 2 }}>{s.v}</div>
                </div>
              ))}
            </div>
          </GCard>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 14 } as any}>
            {[
              { label: 'ECG', icon: 'ri-pulse-line', route: '/ecg', color: '#EF4444' },
              { label: 'Seuils d\'alerte', icon: 'ri-alarm-warning-line', route: '/edit-thresholds', color: '#F59E0B' },
              { label: 'Balance details', icon: 'ri-scales-3-line', route: '/scale-detail', color: '#A78BFA' },
              { label: 'Sommeil details', icon: 'ri-moon-line', route: '/sleep', color: '#38BDF8' },
            ].map((a, i) => (
              <div key={i} data-testid={`health-action-${i}`} onClick={() => router.push(a.route as any)} style={{
                padding: '14px', borderRadius: 14, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)',
