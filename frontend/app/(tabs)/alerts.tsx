  ...
  return (
    <View style={{ flex: 1, backgroundColor: '#F5F6F8' }}>
      <View style={{ paddingHorizontal: 16, paddingTop: 16, paddingBottom: 8 }}>
        <Text style={{ fontSize: 22, fontWeight: '900', color: '#1E1F24', letterSpacing: -0.5 }}>Prescripteurs</Text>
        <Text style={{ fontSize: 12, color: '#6B7084' }}>{allPrescribers.length} prescripteurs actifs</Text>
      </View>
      {/* Search */}
      ...
      <ScrollView contentContainerStyle={{ paddingHorizontal: 16, paddingBottom: 80 }}
        ...
        {prescribers.map((pr: any, idx: number) => (
          <TouchableOpacity key={pr.id} activeOpacity={0.7} data-testid={`prescriber-card-${pr.id}`}
            ...
            <GlassCard style={{ flexDirection: 'row', alignItems: 'center', gap: 12, padding: 14 }}>
              ...
              <View style={{ flex: 1 }}>
                <Text style={{ fontSize: 15, fontWeight: '700', color: '#1E1F24' }}>{pr.name}</Text>
                <Text style={{ fontSize: 11, color: '#6B7084' }}>{pr.email}</Text>
        ...
        {prescribers.length === 0 && (
          <View style={{ alignItems: 'center', paddingVertical: 40 }}>
            ...
            <Text style={{ fontSize: 14, color: '#6B7084', marginTop: 8 }}>{search ? 'Aucun resultat' : 'Aucun prescripteur'}</Text>
          </View>
        )}
      </ScrollView>
    </View>
...
const SEV = { critical: { label: 'CRITIQUE', color: '#E53935' }, high: { label: 'ELEVE', color: '#FF6F00' }, medium: { label: 'MOYEN', color: '#FF9800' }, low: { label: 'FAIBLE', color: '#9CA3B0' } };
...
export default function AlertsScreen() {
      ...
      <TouchableOpacity testID={`alert-card-${item.id}`}
        ...
        <View style={{ padding: 16, paddingBottom: 10 }}>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }}>
            ...
            <View style={{ flex: 1 }}>
              <Text style={{ fontSize: 15, fontWeight: '800', color: '#1E1F24' }}>{cfg.label}</Text>
              <Text style={{ fontSize: 11, color: '#6B7084', marginTop: 1 }}>
          ...
          <Text style={{ fontSize: 13, color: '#1E1F24', lineHeight: 18, marginTop: 8 }}>{item.message}</Text>
        </View>
        ...
        {isActive && (
          <View style={{ flexDirection: 'row', gap: 8, padding: 12, borderTopWidth: 0.5, borderTopColor: 'rgba(0,0,0,0.04)' }}>
            <TouchableOpacity testID={`resolve-${item.id}`} style={{ flex: 1, backgroundColor: '#F5F6F8', borderRadius: 9999, paddingVertical: 10, alignItems: 'center' }}
  ...
  return (
    <View key={effectiveRole} style={{ flex: 1, backgroundColor: '#F5F6F8' }} testID="alerts-screen">
      <View style={{ paddingHorizontal: 16, paddingTop: 16, paddingBottom: 12 }}>
        <Text style={{ fontSize: 28, fontWeight: '900', color: '#1E1F24', letterSpacing: -0.5 }}>Alertes</Text>
      </View>
      ...
      <View style={{ flexDirection: 'row', marginHorizontal: 16, marginBottom: 12, backgroundColor: '#FFFFFF', borderRadius: 14, padding: 4, borderWidth: 1, borderColor: 'rgba(0,0,0,0.06)', ...glass }}>
        <TouchableOpacity style={[{ flex: 1, paddingVertical: 10, alignItems: 'center', borderRadius: 11 }, tab === 'active' && { backgroundColor: '#F5F6F8' }]} onPress={() => setTab('active')}>
          <Text style={{ fontSize: 13, fontWeight: '700', color: tab === 'active' ? '#FFF' : '#888' }}>Actives ({activeAlerts.length})</Text>
        </TouchableOpacity>
        <TouchableOpacity style={[{ flex: 1, paddingVertical: 10, alignItems: 'center', borderRadius: 11 }, tab === 'resolved' && { backgroundColor: '#F5F6F8' }]} onPress={() => setTab('resolved')}>
          <Text style={{ fontSize: 13, fontWeight: '700', color: tab === 'resolved' ? '#FFF' : '#888' }}>Resolues ({resolved.length})</Text>
        </TouchableOpacity>
      </View>
      {loading ? (
        <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}><ActivityIndicator size="large" color="#1E1F24" /></View>
      ) : (
        <FlatList data={filtered} renderItem={renderAlert} keyExtractor={(item) => item.id}
          contentContainerStyle={{ paddingHorizontal: 16, paddingBottom: 80 }}
          refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => { setRefreshing(true); fetchAlerts(); }} tintColor="#1E1F24" />}
          ListEmptyComponent={
            <View style={[{ alignItems: 'center', paddingVertical: 48, backgroundColor: '#FFFFFF', borderRadius: 24, borderWidth: 1, borderColor: 'rgba(0,0,0,0.06)', ...glass }]}>
              ...
              <Text style={{ fontSize: 16, fontWeight: '800', color: '#1E1F24' }}>{tab === 'active' ? 'Tout va bien !' : 'Aucun historique'}</Text>
              <Text style={{ fontSize: 12, color: '#6B7084', marginTop: 4, textAlign: 'center', paddingHorizontal: 20, lineHeight: 18 }}>{tab === 'active' ? 'Aucune alerte active. Notre systeme veille sur vous 24h/24. Si une anomalie est detectee, elle apparaitra ici instantanement.' : 'Vos alertes resolues s\'afficheront ici avec leur rapport d\'intervention, pour un suivi medical complet.'}</Text>
            </View>
          }
        />
      )}
