...
export default function InterventionDetailScreen() {
  ...
  if (!iv) return <SafeAreaView style={{ flex: 1, backgroundColor: colors.background, justifyContent: 'center', alignItems: 'center' }}><Text style={{ color: colors.textSecondary }}>Intervention non trouvee</Text></SafeAreaView>;
    ...
    return (
      <SafeAreaView style={{ flex: 1, backgroundColor: colors.background }}>
        <View style={{ flexDirection: 'row', alignItems: 'center', paddingHorizontal: 16, paddingVertical: 14 }}>
          ...
          <Text style={{ flex: 1, fontSize: 20, fontWeight: '900', color: colors.textPrimary }}>Compte-rendu</Text>
        </View>
        <ScrollView contentContainerStyle={{ paddingHorizontal: 16, paddingBottom: 40 }}>
          {qcmQuestions.map((q, qi) => (
            <GlassCard key={q.id}>
              <Text style={{ fontSize: 15, fontWeight: '800', color: colors.textPrimary, marginBottom: 10 }}>{qi + 1}. {q.question}</Text>
          ...
          <GlassCard>
            <Text style={{ fontSize: 15, fontWeight: '800', color: colors.textPrimary, marginBottom: 10 }}>Notes personnalisees</Text>
            {Platform.OS === 'web' ? (
              ...
              <TextInput value={closeNotes} onChangeText={setCloseNotes} placeholder="Decrivez ce qui s'est passe..." multiline numberOfLines={4}
                style={{ backgroundColor: colors.surfaceHighlight, borderRadius: 14, padding: 12, fontSize: 14, color: colors.textPrimary, minHeight: 100, textAlignVertical: 'top', borderWidth: 1, borderColor: colors.border }} placeholderTextColor="#999" />
            )}
          </GlassCard>

          <TouchableOpacity style={{ backgroundColor: colors.background, borderRadius: 9999, paddingVertical: 16, alignItems: 'center', marginTop: 8, ...(Platform.OS === 'web' ? { boxShadow: '0 4px 16px rgba(0,0,0,0.15)' } : {}) }} onPress={closeIntervention} disabled={closing}>
            {closing ? <ActivityIndicator color={colors.textPrimary} /> : <Text style={{ color: '#FFF', fontSize: 16, fontWeight: '800', textTransform: 'uppercase', letterSpacing: 1 }}>FINALISER L'INTERVENTION</Text>}
          </TouchableOpacity>
        </ScrollView>
      </SafeAreaView>
    );
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: colors.background }}>
      <View style={{ flexDirection: 'row', alignItems: 'center', paddingHorizontal: 16, paddingVertical: 14 }}>
        ...
        <Text style={{ flex: 1, fontSize: 20, fontWeight: '900', color: colors.textPrimary }}>Intervention</Text>
      ...
      <ScrollView contentContainerStyle={{ paddingHorizontal: 16, paddingBottom: 40 }}>
        ...
        <GlassCard style={{ borderLeftWidth: 4, borderLeftColor: '#E53935' }}>
          ...
          <Text style={{ fontSize: 18, fontWeight: '900', color: colors.textPrimary, marginTop: 4 }}>{iv.alert_message || iv.notes || 'SOS'}</Text>
          <Text style={{ fontSize: 12, color: colors.textSecondary, marginTop: 4 }}>{new Date(iv.created_at).toLocaleString('fr-FR')}</Text>
          {iv.distance_km && <Text style={{ fontSize: 13, fontWeight: '700', color: colors.textPrimary, marginTop: 6 }}>Distance : {iv.distance_km} km</Text>}
        </GlassCard>

        {/* Beneficiary Info */}
        <GlassCard>
          <Text style={{ fontSize: 11, fontWeight: '700', color: colors.textSecondary, textTransform: 'uppercase', letterSpacing: 1, marginBottom: 10 }}>BENEFICIAIRE</Text>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12, marginBottom: 12 }}>
            ...
            <View style={{ flex: 1 }}>
              <Text style={{ fontSize: 18, fontWeight: '800', color: colors.textPrimary }}>{ben.name || iv.beneficiary_name}</Text>
              {ben.phone ? <Text style={{ fontSize: 13, color: colors.textSecondary, marginTop: 2 }}>{ben.phone}</Text> : null}
            </View>
          </View>
          {ben.address ? <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 8 }}><Ionicons name="location-outline" size={16} color={colors.textPrimary} /><Text style={{ fontSize: 14, fontWeight: '600', color: colors.textPrimary, flex: 1 }}>{ben.address}</Text></View> : null}
          ...
          {ben.emergency_contact_name ? <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}><Ionicons name="call-outline" size={16} color={colors.textPrimary} /><Text style={{ fontSize: 13, color: '#555' }}>Contact urgence: {ben.emergency_contact_name} ({ben.emergency_contact_phone})</Text></View> : null}
        </GlassCard>

        {/* Coordinates */}
        {iv.beneficiary_location && (
          <GlassCard>
            <Text style={{ fontSize: 11, fontWeight: '700', color: colors.textSecondary, textTransform: 'uppercase', letterSpacing: 1, marginBottom: 6 }}>COORDONNEES GPS</Text>
            <Text style={{ fontSize: 14, fontWeight: '600', color: colors.textPrimary }}>{iv.beneficiary_location.latitude?.toFixed(4)}, {iv.beneficiary_location.longitude?.toFixed(4)}</Text>
        ...
        {iv.assigned_name && (
          <GlassCard>
            <Text style={{ fontSize: 11, fontWeight: '700', color: colors.textSecondary, textTransform: 'uppercase', letterSpacing: 1, marginBottom: 10 }}>INTERVENANT</Text>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12 }}>
              ...
              <View style={{ flex: 1 }}>
                <Text style={{ fontSize: 16, fontWeight: '800', color: colors.textPrimary }}>{iv.assigned_name}</Text>
                {iv.structure_name ? <Text style={{ fontSize: 12, color: colors.textSecondary }}>{iv.structure_name}</Text> : null}
              </View>
            </View>
            {iv.intervener_location?.address ? <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, marginTop: 8 }}><Ionicons name="location-outline" size={16} color={colors.textPrimary} /><Text style={{ fontSize: 13, color: '#555', flex: 1 }}>{iv.intervener_location.address}</Text></View> : null}
            {iv.distance_km ? <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, marginTop: 4 }}><Ionicons name="navigate-outline" size={16} color={colors.textPrimary} /><Text style={{ fontSize: 13, color: '#555' }}>Distance : {iv.distance_km} km</Text></View> : null}
          </GlassCard>
        )}

        {/* Timeline */}
        <GlassCard>
          <Text style={{ fontSize: 11, fontWeight: '700', color: colors.textSecondary, textTransform: 'uppercase', letterSpacing: 1, marginBottom: 10 }}>TIMELINE</Text>
          {(iv.timeline || []).map((t: any, i: number) => (
            <View key={i} style={{ flexDirection: 'row', gap: 10, marginBottom: 8 }}>
              <View style={{ width: 8, height: 8, borderRadius: 4, backgroundColor: colors.background, marginTop: 6 }} />
              <View style={{ flex: 1 }}>
                <Text style={{ fontSize: 13, fontWeight: '600', color: colors.textPrimary }}>{t.note}</Text>
                <Text style={{ fontSize: 11, color: colors.textSecondary }}>{new Date(t.time).toLocaleString('fr-FR')}</Text>
              </View>
            </View>
          ))}
        </GlassCard>
        ...
        {iv.report && (
          <GlassCard style={{ backgroundColor: 'rgba(200,230,201,0.5)' }}>
            ...
            <Text style={{ fontSize: 13, fontWeight: '700', color: colors.textPrimary, marginBottom: 6 }}>Par {iv.report.closed_by_name}</Text>
            {(iv.report.answers || iv.report_answers || []).map((a: any, i: number) => (
              <View key={i} style={{ marginBottom: 6 }}>
                <Text style={{ fontSize: 12, color: colors.textSecondary }}>{a.question || `Question ${i + 1}`}</Text>
                <Text style={{ fontSize: 14, fontWeight: '700', color: colors.textPrimary }}>{a.answer}</Text>
              </View>
            ))}
            {iv.report.notes ? <View style={{ marginTop: 8, borderTopWidth: 1, borderTopColor: 'rgba(0,0,0,0.08)', paddingTop: 8 }}><Text style={{ fontSize: 12, fontWeight: '700', color: colors.textPrimary }}>Notes:</Text><Text style={{ fontSize: 13, color: '#555', marginTop: 2 }}>{iv.report.notes}</Text></View> : null}
          </GlassCard>
        )}
        ...
        {iv.status === 'pending_acceptance' && isRecipient && (
          <TouchableOpacity style={{ backgroundColor: '#10B981', borderRadius: 9999, paddingVertical: 18, alignItems: 'center', marginTop: 8, ...(Platform.OS === 'web' ? { boxShadow: '0 4px 16px rgba(76,175,80,0.3)' } : {}) }} onPress={acceptIntervention} disabled={accepting}>
            {accepting ? <ActivityIndicator color={colors.textPrimary} /> : <Text style={{ color: '#FFF', fontSize: 17, fontWeight: '900', textTransform: 'uppercase', letterSpacing: 2 }}>J'INTERVIENS</Text>}
          </TouchableOpacity>
        )}

        {iv.status === 'in_progress' && isAssigned && (
          <TouchableOpacity style={{ backgroundColor: colors.background, borderRadius: 9999, paddingVertical: 16, alignItems: 'center', marginTop: 8, ...(Platform.OS === 'web' ? { boxShadow: '0 4px 16px rgba(0,0,0,0.15)' } : {}) }} onPress={openCloseQCM}>
        ...
        {iv.status === 'in_progress' && !isAssigned && iv.assigned_name && (
          <GlassCard style={{ alignItems: 'center' }}>
            ...
            <Text style={{ fontSize: 15, fontWeight: '700', color: colors.textPrimary, marginTop: 6 }}>{iv.assigned_name} intervient</Text>
            <Text style={{ fontSize: 12, color: colors.textSecondary, marginTop: 2 }}>Intervention en cours...</Text>
          </GlassCard>
        )}
      </ScrollView>
    </SafeAreaView>
