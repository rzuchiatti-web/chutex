...
export default function InterventionDetailScreen() {
  ...
  if (!iv) return <SafeAreaView style={{ flex: 1, backgroundColor: '#F5F6F8', justifyContent: 'center', alignItems: 'center' }}><Text style={{ color: '#5A6068' }}>Intervention non trouvee</Text></SafeAreaView>;
      ...
      <SafeAreaView style={{ flex: 1, backgroundColor: '#F5F6F8' }}>
        ...
        <ScrollView contentContainerStyle={{ paddingHorizontal: 16, paddingBottom: 40 }}>
          {qcmQuestions.map((q, qi) => (
            <GlassCard key={q.id}>
              <Text style={{ fontSize: 15, fontWeight: '800', color: '#1A1D21', marginBottom: 10 }}>{qi + 1}. {q.question}</Text>
          ...
          <GlassCard>
            <Text style={{ fontSize: 15, fontWeight: '800', color: '#1A1D21', marginBottom: 10 }}>Notes personnalisees</Text>
            {Platform.OS === 'web' ? (
          ...
          <TouchableOpacity style={{ backgroundColor: '#F5F6F8', borderRadius: 9999, paddingVertical: 16, alignItems: 'center', marginTop: 8, ...(Platform.OS === 'web' ? { boxShadow: '0 4px 16px rgba(0,0,0,0.15)' } : {}) }} onPress={closeIntervention} disabled={closing}>
            {closing ? <ActivityIndicator color="#1A1D21" /> : <Text style={{ color: '#FFF', fontSize: 16, fontWeight: '800', textTransform: 'uppercase', letterSpacing: 1 }}>FINALISER L'INTERVENTION</Text>}
          </TouchableOpacity>
        </ScrollView>
      </SafeAreaView>
    );
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: '#F5F6F8' }}>
      ...
      <ScrollView contentContainerStyle={{ paddingHorizontal: 16, paddingBottom: 40 }}>
        ...
        <GlassCard style={{ borderLeftWidth: 4, borderLeftColor: '#E53935' }}>
          ...
          <Text style={{ fontSize: 18, fontWeight: '900', color: '#1A1D21', marginTop: 4 }}>{iv.alert_message || iv.notes || 'SOS'}</Text>
          <Text style={{ fontSize: 12, color: '#5A6068', marginTop: 4 }}>{new Date(iv.created_at).toLocaleString('fr-FR')}</Text>
          {iv.distance_km && <Text style={{ fontSize: 13, fontWeight: '700', color: '#1A1D21', marginTop: 6 }}>Distance : {iv.distance_km} km</Text>}
        </GlassCard>

        {/* Beneficiary Info */}
        <GlassCard>
          <Text style={{ fontSize: 11, fontWeight: '700', color: '#5A6068', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 10 }}>BENEFICIAIRE</Text>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12, marginBottom: 12 }}>
            ...
            <View style={{ flex: 1 }}>
              <Text style={{ fontSize: 18, fontWeight: '800', color: '#1A1D21' }}>{ben.name || iv.beneficiary_name}</Text>
        ...
        {iv.beneficiary_location && (
          <GlassCard>
            <Text style={{ fontSize: 11, fontWeight: '700', color: '#5A6068', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 6 }}>COORDONNEES GPS</Text>
            <Text style={{ fontSize: 14, fontWeight: '600', color: '#1A1D21' }}>{iv.beneficiary_location.latitude?.toFixed(4)}, {iv.beneficiary_location.longitude?.toFixed(4)}</Text>
        ...
        {iv.assigned_name && (
          <GlassCard>
            <Text style={{ fontSize: 11, fontWeight: '700', color: '#5A6068', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 10 }}>INTERVENANT</Text>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12 }}>
              ...
              <View style={{ flex: 1 }}>
                <Text style={{ fontSize: 16, fontWeight: '800', color: '#1A1D21' }}>{iv.assigned_name}</Text>
        ...
        {/* Timeline */}
        <GlassCard>
          <Text style={{ fontSize: 11, fontWeight: '700', color: '#5A6068', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 10 }}>TIMELINE</Text>
          {(iv.timeline || []).map((t: any, i: number) => (
            <View key={i} style={{ flexDirection: 'row', gap: 10, marginBottom: 8 }}>
              <View style={{ width: 8, height: 8, borderRadius: 4, backgroundColor: '#F5F6F8', marginTop: 6 }} />
              <View style={{ flex: 1 }}>
                <Text style={{ fontSize: 13, fontWeight: '600', color: '#1A1D21' }}>{t.note}</Text>
                <Text style={{ fontSize: 11, color: '#5A6068' }}>{new Date(t.time).toLocaleString('fr-FR')}</Text>
              </View>
            </View>
          ))}
        </GlassCard>
        ...
        {iv.report && (
          <GlassCard style={{ backgroundColor: 'rgba(200,230,201,0.5)' }}>
            ...
            <Text style={{ fontSize: 13, fontWeight: '700', color: '#1A1D21', marginBottom: 6 }}>Par {iv.report.closed_by_name}</Text>
            {(iv.report.answers || iv.report_answers || []).map((a: any, i: number) => (
              <View key={i} style={{ marginBottom: 6 }}>
                <Text style={{ fontSize: 12, color: '#5A6068' }}>{a.question || `Question ${i + 1}`}</Text>
                <Text style={{ fontSize: 14, fontWeight: '700', color: '#1A1D21' }}>{a.answer}</Text>
              </View>
            ))}
            {iv.report.notes ? <View style={{ marginTop: 8, borderTopWidth: 1, borderTopColor: 'rgba(0,0,0,0.08)', paddingTop: 8 }}><Text style={{ fontSize: 12, fontWeight: '700', color: '#1A1D21' }}>Notes:</Text><Text style={{ fontSize: 13, color: '#555', marginTop: 2 }}>{iv.report.notes}</Text></View> : null}
          </GlassCard>
        )}
        ...
        {iv.status === 'pending_acceptance' && isRecipient && (
          <TouchableOpacity style={{ backgroundColor: '#10B981', borderRadius: 9999, paddingVertical: 18, alignItems: 'center', marginTop: 8, ...(Platform.OS === 'web' ? { boxShadow: '0 4px 16px rgba(76,175,80,0.3)' } : {}) }} onPress={acceptIntervention} disabled={accepting}>
            {accepting ? <ActivityIndicator color="#1A1D21" /> : <Text style={{ color: '#FFF', fontSize: 17, fontWeight: '900', textTransform: 'uppercase', letterSpacing: 2 }}>J'INTERVIENS</Text>}
          </TouchableOpacity>
        )}
