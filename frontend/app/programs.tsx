...
export default function ProgramsScreen() {
  ...
  return (
    <div data-testid="programs-screen" style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, fontFamily: "'Inter', system-ui, sans-serif", overflow: 'hidden' } as any}>
      {/* Full background — same as dashboard */}
      ...
      <div style={{ position: 'relative', zIndex: 5, height: '100%', overflowY: 'auto', WebkitOverflowScrolling: 'touch' } as any}>
        <div style={{ maxWidth: 420, margin: '0 auto', padding: 'calc(env(safe-area-inset-top, 44px) + 10px) 20px 120px' } as any}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 24 } as any}>
            <div onClick={() => router.back()} style={{ width: 40, height: 40, borderRadius: 12, background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.12)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', ...glass } as any}>
            ...
            <div style={{ flex: 1 } as any}>
              ...
              <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.4)' }}>Prevention personnalisee par Nora</div>
            </div>
          </div>
          ...
          {activeProgram?.active && prog && (
            <>
              ...
              {/* Stop program */}
              {!showStopConfirm ? (
                <div onClick={() => setShowStopConfirm(true)} style={{ padding: '12px', borderRadius: 14, textAlign: 'center', cursor: 'pointer', fontSize: 12, fontWeight: 600, color: 'rgba(239,68,68,0.4)', marginBottom: 20 } as any}>Arreter le programme</div>
              ) : (
                <div style={{ padding: '18px', borderRadius: 18, background: 'rgba(239,68,68,0.08)', border: '1px solid rgba(239,68,68,0.2)', marginBottom: 20, ...glass } as any}>
                  ...
                  <div style={{ display: 'flex', gap: 10 } as any}>
                    ...
                    <div onClick={stopProgram} style={{ flex: 1, padding: '12px', borderRadius: 999, background: '#EF4444', cursor: 'pointer', textAlign: 'center', fontSize: 13, fontWeight: 700, color: '#FFF' } as any}>{stopping ? '...' : 'Arreter'}</div>
                  </div>
                </div>
              )}
            </>
          ...
          <div style={{ marginBottom: 20 } as any}>
            ...
            {catalog.filter((p: any) => !activeProgram?.active || p.id !== activeProgram?.program?.id).map((p: any) => (
              <div key={p.id} data-testid={`catalog-${p.id}`}
                ...
                style={{ padding: '18px', borderRadius: 22, background: 'rgba(255,255,255,0.06)', border: `1px solid ${activeProgram?.active ? 'rgba(255,255,255,0.04)' : p.color + '20'}`, marginBottom: 10, cursor: activeProgram?.active || !hasDevices ? 'default' : 'pointer', opacity: activeProgram?.active ? 0.5 : 1, transition: 'transform 0.15s', ...glass } as any}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 10 } as any}>
                  ...
                  {!activeProgram?.active && hasDevices && <i className="ri-arrow-right-s-line" style={{ fontSize: 20, color: 'rgba(255,255,255,0.2)' }} />}
                </div>
