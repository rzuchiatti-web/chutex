...
export default function BeneficiaryDetailScreen() {
  ...
  /* clinic palette — flat, no nesting */
  const C = isDark
    ? { text: '#FFF', sub: 'rgba(255,255,255,0.5)', muted: 'rgba(255,255,255,0.28)', faint: 'rgba(255,255,255,0.18)', sep: 'rgba(255,255,255,0.08)', row: 'rgba(255,255,255,0.03)', toggleBg: 'rgba(255,255,255,0.08)', contentBg: 'linear-gradient(180deg, #0C0C14 0%, #1C1C24 100%)', border: 'rgba(255,255,255,0.08)' }
    : { text: '#111827', sub: 'rgba(0,0,0,0.5)', muted: 'rgba(0,0,0,0.28)', faint: 'rgba(0,0,0,0.1)', sep: 'rgba(0,0,0,0.06)', row: 'rgba(0,0,0,0.02)', toggleBg: 'rgba(0,0,0,0.06)', contentBg: '#FAFAFA', border: 'rgba(0,0,0,0.06)' };
  ...
  /* 4 vitals — always show all 4, with placeholder */
  ...
  /* style helpers */
  const sectionLabel = { fontSize: 11, fontWeight: 700 as const, color: C.muted, textTransform: 'uppercase' as const, letterSpacing: 1.2, padding: '20px 0 10px' };
  const rowStyle = { display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '11px 0', borderBottom: `1px solid ${C.sep}` } as any;
  const labelStyle = { fontSize: 12, color: C.sub, fontWeight: 500 as const };
  const valueStyle = { fontSize: 13, color: C.text, fontWeight: 600 as const, textAlign: 'right' as const };

  return (
    <div style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, fontFamily: "'Inter', system-ui, sans-serif", overflow: 'hidden' } as any}>
      <style>{`@keyframes bd-fade{from{opacity:0;transform:translateY(6px)}to{opacity:1;transform:translateY(0)}}@keyframes bd-pop{from{opacity:0;transform:translateY(10px) scale(.97)}to{opacity:1;transform:translateY(0) scale(1)}}@keyframes bd-pulse{0%,100%{transform:scale(1)}50%{transform:scale(1.03)}}@keyframes bd-slide{from{transform:translateX(-100%)}to{transform:translateX(0)}}`}</style>
      ...
      <div style={{ flex: 1, overflowY: 'auto', position: 'relative', zIndex: 5, WebkitOverflowScrolling: 'touch', height: '100%' } as any}>
        ...
        <div style={{ position: 'relative', zIndex: 1 } as any}>
          ...
          <div style={{ position: 'relative', zIndex: 2, padding: '18px 20px 44px' } as any}>
            ...
            {/* Call slide button full width */}
            {data.phone && (
              <div data-testid="beneficiary-call-btn" onClick={() => window.open(`tel:${data.phone}`, '_self')} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, width: '100%', padding: '13px 0', borderRadius: 14, background: 'rgba(255,255,255,0.18)', backdropFilter: 'blur(12px)', WebkitBackdropFilter: 'blur(12px)', cursor: 'pointer', transition: 'background 0.15s' } as any}
                onMouseEnter={(e: any) => e.currentTarget.style.background = 'rgba(255,255,255,0.28)'} onMouseLeave={(e: any) => e.currentTarget.style.background = 'rgba(255,255,255,0.18)'}>
                <i className="ri-phone-fill" style={{ fontSize: 16, color: '#FFF' }} />
                <span style={{ fontSize: 14, fontWeight: 700, color: '#FFF', letterSpacing: 0.3 }}>Appeler {firstName}</span>
              </div>
            )}
            {/* Alert */}
        ...
        <div style={{ padding: '0 20px 120px', marginTop: -16, borderRadius: '22px 22px 0 0', background: C.contentBg, position: 'relative', zIndex: 10, borderTop: `1px solid ${C.border}` } as any}>
          ...
          <div style={sectionLabel}>Informations personnelles</div>
          <div style={rowStyle}><span style={labelStyle}>Prenom</span><span style={valueStyle}>{firstName}</span></div>
          <div style={rowStyle}><span style={labelStyle}>Nom</span><span style={valueStyle}>{lastName}</span></div>
          {ageYears && <div style={rowStyle}><span style={labelStyle}>Age</span><span style={valueStyle}>{ageYears} ans</span></div>}
          <div style={rowStyle}><span style={labelStyle}>Genre</span><span style={valueStyle}>{genderLabel}</span></div>
          {data.date_of_birth && <div style={rowStyle}><span style={labelStyle}>Date de naissance</span><span style={valueStyle}>{new Date(data.date_of_birth).toLocaleDateString('fr-FR')}</span></div>}
          {data.phone && <div style={rowStyle}><span style={labelStyle}>Telephone</span><span style={valueStyle}>{data.phone}</span></div>}
          <div style={{ ...rowStyle, flexDirection: 'column', alignItems: 'flex-start', gap: 2 } as any}><span style={labelStyle}>Adresse</span><span data-testid="beneficiary-profile-address-value" style={{ fontSize: 13, color: C.text, fontWeight: 600, marginTop: 2 }}>{data.address || '-'}{(data.postal_code || data.city) ? `, ${[data.postal_code, data.city].filter(Boolean).join(' ')}` : ''}</span></div>
          {(data.height_cm || profileWeight) && (<>
            {data.height_cm && <div style={rowStyle}><span style={labelStyle}>Taille</span><span data-testid="beneficiary-profile-height-value" style={valueStyle}>{data.height_cm} cm</span></div>}
            {profileWeight && <div style={rowStyle}><span style={labelStyle}>Poids</span><span data-testid="beneficiary-profile-weight-value" style={valueStyle}>{profileWeight} kg</span></div>}
            {data.height_cm && profileWeight && <div style={rowStyle}><span style={labelStyle}>IMC</span><span style={valueStyle}>{(profileWeight / Math.pow(data.height_cm / 100, 2)).toFixed(1)}</span></div>}
          </>)}

          {/* ── 2. DOSSIER MEDICAL ── */}
          <div style={sectionLabel}>Dossier medical</div>
          <div style={rowStyle}><span style={labelStyle}>Groupe sanguin</span><span style={valueStyle}>{data.blood_type || '--'}</span></div>
          <div style={{ ...rowStyle, flexDirection: 'column', alignItems: 'flex-start', gap: 2 } as any}><span style={labelStyle}>Pathologies</span><span style={{ fontSize: 13, color: C.text, fontWeight: 600, marginTop: 2 }}>{data.medical_conditions || 'Aucune'}</span></div>
          <div style={{ ...rowStyle, flexDirection: 'column', alignItems: 'flex-start', gap: 2 } as any}><span style={labelStyle}>Allergies</span><span style={{ fontSize: 13, color: C.text, fontWeight: 600, marginTop: 2 }}>{data.allergies || 'Aucune'}</span></div>

          {/* ── 3. DONNEES DE SANTE (always 4) ── */}
          <div style={sectionLabel}>Donnees de sante</div>
          ...
          {/* CTA: voir page sante */}
          <div data-testid="view-health-page-btn" onClick={() => router.push({ pathname: '/health-readonly' as any, params: { beneficiaryId: activeBid } })} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, padding: '13px 0', borderRadius: 12, border: `1px solid ${C.sep}`, background: C.row, cursor: 'pointer', marginBottom: 4, transition: 'background 0.15s' } as any}
            onMouseEnter={(e: any) => e.currentTarget.style.background = isDark ? 'rgba(255,255,255,0.06)' : 'rgba(0,0,0,0.04)'} onMouseLeave={(e: any) => e.currentTarget.style.background = C.row}>
            <i className="ri-heart-pulse-line" style={{ fontSize: 15, color: '#10B981' }} />
            <span style={{ fontSize: 13, fontWeight: 700, color: C.text }}>Voir la page sante</span>
            <i className="ri-arrow-right-s-line" style={{ fontSize: 15, color: C.muted }} />
          </div>

          {/* Nora */}
          {noraAnalysis && <div style={{ marginTop: 16, marginBottom: 4 } as any}><NoraCard title="Analyse de Nora" text={noraAnalysis} /></div>}

          {/* ── 4. ACTIVITE PHYSIQUE ── */}
          {activityMetrics.length > 0 && (<>
            <div style={sectionLabel}>Activite physique</div>
            <div style={{ display: 'grid', gridTemplateColumns: `repeat(${activityMetrics.length}, 1fr)`, gap: 1, borderRadius: 14, overflow: 'hidden', border: `1px solid ${C.sep}`, marginBottom: 4 } as any}>
          ...
          {/* ── 5. DISPOSITIFS ── */}
          <div style={sectionLabel}>Dispositifs</div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 1, borderRadius: 14, overflow: 'hidden', border: `1px solid ${C.sep}`, marginBottom: 4 } as any}>
            {devList.map((dev, i) => {
              ...
              return (
                <div key={i} data-testid={`beneficiary-device-card-${dev.type}`} style={{ padding: '16px 8px 14px', background: C.row, textAlign: 'center', borderRight: i < 2 ? `1px solid ${C.sep}` : 'none' } as any}>
                  ...
                  <div style={{ display: 'inline-flex', alignItems: 'center', gap: 3 } as any}><span style={{ width: 5, height: 5, borderRadius: 99, background: sColor } as any} /><span style={{ fontSize: 9, fontWeight: 600, color: sColor }}>{sLabel}</span></div>
          ...
          {/* ── 6. PREFERENCES ── */}
          {guardianPerms && (<>
            <div style={sectionLabel}>Mes preferences</div>
            <div style={{ fontSize: 11, color: C.sub, marginBottom: 8, lineHeight: 1.5 }}>Notifications pour {firstName}.</div>
            {[
              ...
              <div key={pref.key} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 0', borderBottom: pi < 2 ? `1px solid ${C.sep}` : 'none' } as any}>
                ...
                <div onClick={() => { const n = !(guardianPerms as any)[pref.key]; setGuardianPerms((p: any) => ({ ...p, [pref.key]: n })); apiFetch(`/api/guardian-permissions/${user?.id}/${activeBid}/guardian`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ [pref.key]: n }) }, token).catch(() => {}); }} style={{ width: 44, height: 24, borderRadius: 12, background: (guardianPerms as any)[pref.key] ? '#10B981' : C.toggleBg, cursor: 'pointer', position: 'relative', transition: 'background 0.2s' } as any}><div style={{ width: 18, height: 18, borderRadius: 9, background: '#FFF', position: 'absolute', top: 3, left: (guardianPerms as any)[pref.key] ? 23 : 3, transition: 'left 0.2s', boxShadow: '0 1px 3px rgba(0,0,0,0.15)' } as any} /></div>
              </div>
            ))}
            {guardianPerms.guardian_alerts_enabled && guardianPerms.alerts_enabled && (<>
              ...
              {expandedPerm === 'alerts' && <div>{Object.entries(guardianPerms.guardian_alert_types || {}).map(([k, val]: [string, any]) => { const lbl: Record<string, string> = { fall: 'Chute', heart_rate: 'Freq. cardiaque', inactivity: 'Inactivite', sos_manual: 'SOS manuel', temperature: 'Temperature', spo2: 'SpO2', blood_pressure: 'Tension', weight: 'Poids', pulse: 'Pouls' }; const bg = guardianPerms.alert_types?.[k]; return (<div key={k} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '7px 0', borderBottom: `1px solid ${C.sep}` } as any}><span style={{ flex: 1, fontSize: 12, color: bg ? C.text : C.muted, fontWeight: 500 }}>{lbl[k] || k}{!bg ? ' (non partage)' : ''}</span><div onClick={() => { if (!bg) return; const n2 = { ...guardianPerms.guardian_alert_types, [k]: !val }; setGuardianPerms((p: any) => ({ ...p, guardian_alert_types: n2 })); apiFetch(`/api/guardian-permissions/${user?.id}/${activeBid}/guardian`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ guardian_alert_types: n2 }) }, token).catch(() => {}); }} style={{ width: 36, height: 20, borderRadius: 10, background: (val && bg) ? '#10B981' : C.toggleBg, cursor: bg ? 'pointer' : 'default', position: 'relative', transition: 'background 0.2s', opacity: bg ? 1 : 0.3 } as any}><div style={{ width: 14, height: 14, borderRadius: 7, background: '#FFF', position: 'absolute', top: 3, left: (val && bg) ? 19 : 3, transition: 'left 0.2s' } as any} /></div></div>); })}</div>}
            </>)}
            {guardianPerms.location_mode !== 'never' && guardianPerms.guardian_location_accepted && <div style={{ fontSize: 10, color: C.faint, padding: '6px 0' }}>Mode: {guardianPerms.location_mode === 'always' ? 'Tout le temps' : "En cas d'alerte uniquement"}</div>}
          </>)}

          {/* ── 7. GARDIENS ── */}
          <div style={sectionLabel}>Gardiens ({guardiansList.length})</div>
          {guardiansList.length > 0 ? guardiansList.map((g: any, i: number) => (
            <div key={g.id || i} data-testid={`beneficiary-guardian-card-${g.id || i}`} onClick={() => setSelectedGuardian(g)}
              style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 0', borderBottom: i < guardiansList.length - 1 ? `1px solid ${C.sep}` : 'none', cursor: 'pointer', transition: 'opacity 0.15s' } as any}
              onMouseEnter={(e: any) => e.currentTarget.style.opacity = '0.7'} onMouseLeave={(e: any) => e.currentTarget.style.opacity = '1'}>
              <div style={{ width: 36, height: 36, borderRadius: 10, background: C.row, border: `1px solid ${C.sep}`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 } as any}><span style={{ fontSize: 14, fontWeight: 700, color: C.text }}>{g.name?.charAt(0)}</span></div>
          ...
          )) : <div data-testid="beneficiary-guardians-empty" style={{ fontSize: 12, color: C.sub, padding: '14px 0' }}>Aucun gardien associe</div>}

          {/* Historique */}
          {historyAlerts.length > 0 && (<>
            <div style={sectionLabel}>Historique ({historyAlerts.length})</div>
            {historyAlerts.slice(0, 6).map((al: any, i: number) => (
              <div key={al.id} onClick={() => router.push({ pathname: '/(tabs)/alerts', params: { preselect: al.id } })} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 0', borderBottom: i < Math.min(historyAlerts.length, 6) - 1 ? `1px solid ${C.sep}` : 'none', cursor: 'pointer' } as any}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' } as any}>
            <div style={sectionLabel}>Gestion des zones</div>
            <div data-testid="zone-help-btn" onClick={() => setShowZoneHelp(true)} style={{ width: 22, height: 22, borderRadius: 11, border: `1px solid ${C.sep}`, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', marginTop: 10 } as any}><i className="ri-question-line" style={{ fontSize: 12, color: C.muted }} /></div>
          </div>

          {/* Position */}
          <div style={{ padding: '10px 0', borderBottom: `1px solid ${C.sep}` } as any}>
            <div style={{ fontSize: 10, fontWeight: 600, color: C.muted, textTransform: 'uppercase', marginBottom: 3 }}>Position</div>
            <div style={{ fontSize: 12, color: C.text }} data-testid="beneficiary-safezone-location-value">{geoLocation?.latitude != null && geoLocation?.longitude != null ? `${Number(geoLocation.latitude).toFixed(5)}, ${Number(geoLocation.longitude).toFixed(5)}${geoLocation?.updated_at ? ` \u00b7 ${new Date(geoLocation.updated_at).toLocaleString('fr-FR')}` : ''}` : 'Non disponible'}</div>
          </div>

          {/* Map */}
          <div data-testid="beneficiary-safezone-map" style={{ borderRadius: 14, overflow: 'hidden', border: `1px solid ${C.sep}`, marginTop: 12, marginBottom: 12, minHeight: 180 } as any}>
            {geoLocation?.latitude != null && geoLocation?.longitude != null ? <iframe title="map" src={`https://www.openstreetmap.org/export/embed.html?bbox=${Number(geoLocation.longitude) - 0.01},${Number(geoLocation.latitude) - 0.01},${Number(geoLocation.longitude) + 0.01},${Number(geoLocation.latitude) + 0.01}&layer=mapnik&marker=${Number(geoLocation.latitude)},${Number(geoLocation.longitude)}`} style={{ width: '100%', height: 180, border: 'none' } as any} /> : <div style={{ minHeight: 180, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', color: C.muted, gap: 6 } as any}><i className="ri-map-pin-off-line" style={{ fontSize: 22 }} /><div style={{ fontSize: 12 }} data-testid="beneficiary-safezone-map-empty">Carte indisponible</div></div>}
          </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 } as any}>
            ...
            <div data-testid="beneficiary-safezone-open-create-popup-btn" onClick={openCreateZonePopup} style={{ padding: '4px 12px', borderRadius: 99, cursor: geoLocation?.latitude != null ? 'pointer' : 'not-allowed', background: geoLocation?.latitude != null ? 'rgba(16,185,129,0.1)' : 'transparent', border: geoLocation?.latitude != null ? '1px solid rgba(16,185,129,0.2)' : `1px solid ${C.sep}`, fontSize: 10, fontWeight: 600, color: geoLocation?.latitude != null ? '#10B981' : C.muted } as any}>+ Ajouter</div>
          </div>
          {geoZones.map((zone: any, zi: number) => (
            <div key={zone.id} data-testid={`beneficiary-safezone-row-${zone.id}`} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 0', borderBottom: zi < geoZones.length - 1 ? `1px solid ${C.sep}` : 'none' } as any}>
              ...
              <div data-testid={`beneficiary-safezone-edit-btn-${zone.id}`} onClick={() => startGeoEdit(zone)} style={{ padding: '3px 10px', borderRadius: 99, cursor: 'pointer', border: `1px solid ${C.sep}`, fontSize: 10, fontWeight: 600, color: C.sub } as any}>Modifier</div>
              <div data-testid={`beneficiary-safezone-delete-btn-${zone.id}`} onClick={() => deleteGeoZone(zone.id)} style={{ padding: '3px 8px', borderRadius: 99, cursor: geoBusyId === zone.id ? 'wait' : 'pointer', border: '1px solid rgba(239,68,68,0.15)', fontSize: 10, fontWeight: 600, color: '#EF4444', opacity: geoBusyId === zone.id ? 0.5 : 1 } as any}>Suppr.</div>
            </div>
          ))}
          {geoZones.length === 0 && !geoLoading && <div style={{ fontSize: 12, color: C.muted, padding: '12px 0' }}>Aucune safe zone.</div>}
      ...
      {showZoneHelp && (
        <div data-testid="zone-help-modal" onClick={() => setShowZoneHelp(false)} style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, zIndex: 1200, backdropFilter: 'blur(24px)', WebkitBackdropFilter: 'blur(24px)', background: isDark ? 'rgba(0,0,0,0.55)' : 'rgba(0,0,0,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 } as any}>
          <div onClick={(e: any) => e.stopPropagation()} style={{ width: '100%', maxWidth: 380, borderRadius: 20, background: isDark ? 'rgba(30,30,38,0.95)' : 'rgba(255,255,255,0.97)', backdropFilter: 'blur(32px)', WebkitBackdropFilter: 'blur(32px)', border: `1px solid ${C.border}`, padding: '24px 20px', animation: 'bd-pop 220ms ease both' } as any}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 } as any}><div style={{ width: 38, height: 38, borderRadius: 10, background: 'rgba(16,185,129,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center' } as any}><i className="ri-shield-check-line" style={{ fontSize: 18, color: '#10B981' }} /></div><div style={{ fontSize: 17, fontWeight: 800, color: C.text }}>Zones de securite</div></div>
            <div style={{ fontSize: 13, color: C.sub, lineHeight: 1.7, marginBottom: 16 }}>Les <strong style={{ color: C.text }}>safe zones</strong> sont des perimetres geographiques definis autour de lieux importants.</div>
            {[
              { icon: 'ri-map-pin-add-line', c: '#3B82F6', t: 'Creer une zone', d: 'Cliquez "+ Ajouter" pour definir un perimetre autour de la position actuelle.' },
              { icon: 'ri-alarm-warning-line', c: '#F59E0B', t: 'Alertes automatiques', d: 'Recevez une notification si le beneficiaire sort d\'une zone.' },
              { icon: 'ri-edit-line', c: '#10B981', t: 'Modifier le rayon', d: 'Ajustez la taille de chaque zone (300m a 2km recommande).' },
            ].map((item, i) => (
            ...
            <div data-testid="zone-help-close-btn" onClick={() => setShowZoneHelp(false)} style={{ width: '100%', padding: '12px', borderRadius: 12, textAlign: 'center', cursor: 'pointer', background: 'rgba(16,185,129,0.1)', border: '1px solid rgba(16,185,129,0.2)', fontSize: 13, fontWeight: 700, color: '#10B981', marginTop: 10 } as any}>Compris</div>
          </div>
        </div>
      )}
      ...
      {selectedGuardian && (() => {
        ...
        return (
          <div data-testid="guardian-detail-modal" onClick={() => setSelectedGuardian(null)} style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, zIndex: 1190, backdropFilter: 'blur(24px)', WebkitBackdropFilter: 'blur(24px)', background: isDark ? 'rgba(0,0,0,0.55)' : 'rgba(0,0,0,0.3)', overflowY: 'auto', WebkitOverflowScrolling: 'touch', animation: 'bd-fade 200ms ease' } as any}>
            <div onClick={(e: any) => e.stopPropagation()} style={{ width: '100%', maxWidth: 420, margin: '0 auto', padding: '40px 24px 120px', boxSizing: 'border-box' } as any}>
              ...
              <div style={{ borderRadius: 18, background: isDark ? 'rgba(30,30,38,0.95)' : 'rgba(255,255,255,0.97)', border: `1px solid ${C.border}`, padding: '24px 20px', animation: 'bd-pop 220ms ease both' } as any}>
                <div style={{ textAlign: 'center', marginBottom: 16 } as any}>
                  <div style={{ width: 52, height: 52, borderRadius: 14, background: C.row, display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 10px' } as any}><span style={{ fontSize: 19, fontWeight: 800, color: C.text }}>{gInitials}</span></div>
                  <div data-testid="guardian-detail-name" style={{ fontSize: 19, fontWeight: 800, color: C.text, marginBottom: 4 }}>{gName}</div>
                  <div data-testid="guardian-detail-role" style={{ fontSize: 12, color: C.sub, fontWeight: 500 }}>{gRelation}</div>
                </div>
                {/* flat rows */}
                {gPhone && <div data-testid="guardian-detail-phone" style={{ ...rowStyle, borderBottom: `1px solid ${C.sep}` }}><span style={labelStyle}>Telephone</span><span style={valueStyle}>{gPhone}</span></div>}
                {gEmail && <div data-testid="guardian-detail-email" style={{ ...rowStyle, borderBottom: `1px solid ${C.sep}`, cursor: 'pointer' }} onClick={() => window.open(`mailto:${gEmail}`, '_blank')}><span style={labelStyle}>Email</span><span style={valueStyle}>{gEmail}</span></div>}
                {gAddress && <div data-testid="guardian-detail-address" style={{ ...rowStyle, flexDirection: 'column', alignItems: 'flex-start', gap: 2, borderBottom: `1px solid ${C.sep}` } as any}><span style={labelStyle}>Adresse</span><span style={{ fontSize: 13, color: C.text, fontWeight: 600, marginTop: 2 }}>{gAddress}</span></div>}
                <div data-testid="guardian-detail-type" style={rowStyle}><span style={labelStyle}>Type</span><span style={valueStyle}>{selectedGuardian.guardian_type === 'saad' ? 'SAAD' : selectedGuardian.guardian_type === 'company' ? 'Entreprise' : 'Particulier'}</span></div>
                {/* actions */}
                <div style={{ display: 'flex', gap: 8, margin: '16px 0 14px' } as any}>
                  {gPhone && <div data-testid="guardian-call-btn" onClick={() => window.open(`tel:${gPhone}`, '_self')} style={{ flex: 1, padding: '12px', borderRadius: 12, cursor: 'pointer', background: 'rgba(16,185,129,0.08)', border: '1px solid rgba(16,185,129,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, fontSize: 13, fontWeight: 700, color: '#10B981' } as any}><i className="ri-phone-fill" style={{ fontSize: 14 }} />Appeler</div>}
                  {gPhone && <div data-testid="guardian-sms-btn" onClick={() => window.open(`sms:${gPhone}`, '_self')} style={{ flex: 1, padding: '12px', borderRadius: 12, cursor: 'pointer', border: `1px solid ${C.sep}`, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, fontSize: 13, fontWeight: 700, color: C.text } as any}><i className="ri-message-3-line" style={{ fontSize: 14 }} />SMS</div>}
                </div>
                {/* activity */}
                <div style={{ fontSize: 10, fontWeight: 700, color: C.muted, textTransform: 'uppercase', letterSpacing: 1, marginBottom: 8 }}>Activite</div>
                <div style={rowStyle}><span style={labelStyle}>Actions</span><span data-testid="guardian-detail-activity-count" style={valueStyle}>{activity.count}</span></div>
                <div style={rowStyle}><span style={labelStyle}>Derniere action</span><span data-testid="guardian-detail-last-action" style={valueStyle}>{activity.lastActionAt ? new Date(activity.lastActionAt).toLocaleString('fr-FR') : 'Aucune'}</span></div>
                {activity.recentAlerts.length > 0 ? activity.recentAlerts.map((a: any) => (
                  <div key={a.id} data-testid={`guardian-detail-activity-item-${a.id}`} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '7px 0', borderBottom: `1px solid ${C.sep}` } as any}><div style={{ width: 4, height: 4, borderRadius: 2, background: C.muted, flexShrink: 0 } as any} /><span style={{ fontSize: 11, color: C.sub, flex: 1 }}>{a.message || a.alert_type}</span><span style={{ fontSize: 9, color: C.muted }}>{new Date(a.created_at).toLocaleDateString('fr-FR')}</span></div>
                )) : <div data-testid="guardian-detail-activity-empty" style={{ fontSize: 11, color: C.faint, padding: '8px 0' }}>Aucune action enregistree</div>}
              </div>
            </div>
          </div>
        );
      ...
      {/* Safe zone form */}
      {geoFormOpen && (
        <div data-testid="safezone-form-modal" style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, zIndex: 1200, backdropFilter: 'blur(24px)', WebkitBackdropFilter: 'blur(24px)', background: isDark ? 'rgba(0,0,0,0.55)' : 'rgba(0,0,0,0.3)', overflowY: 'auto', WebkitOverflowScrolling: 'touch' } as any}>
          <div style={{ width: '100%', maxWidth: 420, margin: '0 auto', padding: '40px 24px 120px', boxSizing: 'border-box' } as any}>
            ...
            <div style={{ borderRadius: 18, background: isDark ? 'rgba(30,30,38,0.95)' : 'rgba(255,255,255,0.97)', border: `1px solid ${C.border}`, padding: '24px 20px', animation: 'bd-pop 220ms ease both' } as any}>
              <div style={{ fontSize: 18, fontWeight: 800, color: C.text, marginBottom: 4 }} data-testid="safezone-form-modal-title">{geoEditingId ? 'Modifier la safe zone' : 'Nouvelle safe zone'}</div>
              <div style={{ fontSize: 11, color: C.sub, marginBottom: 16 }} data-testid="safezone-form-center-info">Centre: {Number(geoFormLat || 0).toFixed(5)}, {Number(geoFormLng || 0).toFixed(5)}</div>
              <div style={{ marginBottom: 8 } as any}><div style={{ fontSize: 10, fontWeight: 600, color: C.muted, textTransform: 'uppercase', marginBottom: 4 }}>Nom</div><input data-testid="safezone-form-name-input" value={geoFormName} onChange={(e: any) => setGeoFormName(e.target.value)} style={{ width: '100%', padding: '10px 12px', borderRadius: 10, border: `1px solid ${C.sep}`, background: C.row, color: C.text, fontSize: 13, outline: 'none', boxSizing: 'border-box' } as any} /></div>
              <div style={{ marginBottom: 14 } as any}><div style={{ fontSize: 10, fontWeight: 600, color: C.muted, textTransform: 'uppercase', marginBottom: 4 }}>Rayon (metres)</div><input data-testid="safezone-form-radius-input" value={geoFormRadius} onChange={(e: any) => setGeoFormRadius(e.target.value)} style={{ width: '100%', padding: '10px 12px', borderRadius: 10, border: `1px solid ${C.sep}`, background: C.row, color: C.text, fontSize: 13, outline: 'none', boxSizing: 'border-box' } as any} /></div>
              <div style={{ display: 'flex', gap: 8 } as any}>
                ...
                <div data-testid="safezone-form-save-btn" onClick={saveGeoForm} style={{ flex: 1, padding: '11px', borderRadius: 12, cursor: geoFormSaving ? 'wait' : 'pointer', textAlign: 'center', background: 'rgba(16,185,129,0.1)', border: '1px solid rgba(16,185,129,0.2)', fontSize: 13, fontWeight: 700, color: '#10B981', opacity: geoFormSaving ? 0.6 : 1 } as any}>{geoFormSaving ? '...' : (geoEditingId ? 'Enregistrer' : 'Creer')}</div>
              </div>
            </div>
          </div>
        </div>
