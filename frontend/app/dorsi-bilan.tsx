...
const DIRECTIONS = [
  { key: 'forward', label: 'Anteversion', desc: 'Inclinez le bassin vers l\'avant', icon: 'ri-arrow-up-line', angle: 0 },
  { key: 'backward', label: 'Retroversion', desc: 'Inclinez le bassin vers l\'arriere', icon: 'ri-arrow-down-line', angle: 180 },
  { key: 'left', label: 'Flexion gauche', desc: 'Inclinez le bassin vers la gauche', icon: 'ri-arrow-left-line', angle: 270 },
  { key: 'right', label: 'Flexion droite', desc: 'Inclinez le bassin vers la droite', icon: 'ri-arrow-right-line', angle: 90 },
];
...
function RadarChart({ measurements }: { measurements: Record<string, { mobility: number; pain: number }> }) {
  ...
  const mobilityPoints = keys.map((k, i) => getPoint(angles[i], measurements[k]?.mobility || 0));
  const painPoints = keys.map((k, i) => getPoint(angles[i], measurements[k]?.pain * 10 || 0));
  const mobilityPath = mobilityPoints.map((p, i) => `${i === 0 ? 'M' : 'L'}${p.x},${p.y}`).join(' ') + 'Z';
  const painPath = painPoints.map((p, i) => `${i === 0 ? 'M' : 'L'}${p.x},${p.y}`).join(' ') + 'Z';

  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} style={{ display: 'block', margin: '0 auto' }}>
      {[20, 40, 60, 80, 100].map(v => (
        <polygon key={v} points={angles.map(a => `${cx + Math.cos(a) * (v / 100) * maxR},${cy + Math.sin(a) * (v / 100) * maxR}`).join(' ')} fill="none" stroke="rgba(255,255,255,0.08)" strokeWidth={1} />
      ))}
      {angles.map((a, i) => (
        <g key={i}>
          <line x1={cx} y1={cy} x2={cx + Math.cos(a) * maxR} y2={cy + Math.sin(a) * maxR} stroke="rgba(255,255,255,0.1)" strokeWidth={1} />
          <text x={cx + Math.cos(a) * (maxR + 20)} y={cy + Math.sin(a) * (maxR + 20)} fill="rgba(255,255,255,0.5)" fontSize={11} fontWeight={600} textAnchor="middle" dominantBaseline="middle">{labels[i]}</text>
        </g>
      ))}
      <path d={mobilityPath} fill={`${ACCENT}25`} stroke={ACCENT} strokeWidth={2.5} />
      {mobilityPoints.map((p, i) => <circle key={`m${i}`} cx={p.x} cy={p.y} r={4} fill={ACCENT} stroke="#FFF" strokeWidth={1.5} />)}
      <path d={painPath} fill="rgba(239,68,68,0.15)" stroke="#EF4444" strokeWidth={2} strokeDasharray="6,3" />
      {painPoints.map((p, i) => <circle key={`p${i}`} cx={p.x} cy={p.y} r={3.5} fill="#EF4444" stroke="#FFF" strokeWidth={1.5} />)}
    </svg>
  );
}

function SimulatedGyro({ direction, onCapture }: { direction: typeof DIRECTIONS[0]; onCapture: (val: number) => void }) {
  ...
  const startCapture = () => {
    setCapturing(true);
    setCaptured(false);
    // Simulate gyroscope reading ramping up
    let v = 0;
  ...
  return (
    <div style={{ textAlign: 'center' } as any}>
      <div style={{ width: 160, height: 160, borderRadius: '50%', border: `3px solid ${capturing ? ACCENT : 'rgba(255,255,255,0.15)'}`, margin: '0 auto 20px', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative', background: `conic-gradient(${ACCENT}40 ${value * 3.6}deg, rgba(255,255,255,0.03) ${value * 3.6}deg)`, transition: 'all 0.3s' } as any}>
        <div style={{ textAlign: 'center' } as any}>
          <i className={direction.icon} style={{ fontSize: 32, color: ACCENT, display: 'block', marginBottom: 4 }} />
          <div style={{ fontSize: 28, fontWeight: 900, color: '#FFF' }}>{value}%</div>
          <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.4)' }}>mobilite</div>
        </div>
      </div>
      {!captured ? (
        <div data-testid={`capture-${direction.key}`} onClick={startCapture} style={{ padding: '14px 32px', borderRadius: 999, background: capturing ? 'rgba(249,115,22,0.15)' : ACCENT, cursor: capturing ? 'default' : 'pointer', color: capturing ? ACCENT : '#FFF', fontSize: 14, fontWeight: 700, display: 'inline-block', border: capturing ? `1px solid ${ACCENT}40` : 'none' } as any}>
          {capturing ? 'Mesure en cours...' : 'Mesurer'}
        </div>
      ) : (
        <div onClick={() => onCapture(value)} data-testid={`confirm-${direction.key}`} style={{ padding: '14px 32px', borderRadius: 999, background: '#10B981', cursor: 'pointer', color: '#FFF', fontSize: 14, fontWeight: 700, display: 'inline-block' } as any}>
          Valider ({value}%)
        </div>
      )}
    </div>
...
export default function DorsiBilanPage() {
  ...
  const [step, setStep] = useState(0); // 0=intro, 1=tare, 2-5=directions, 6=pain, 7=results
  ...
  const handleMobilityCapture = (dirKey: string, val: number) => {
    ...
    if (dirIdx < DIRECTIONS.length - 1) {
      setTimeout(() => setStep(dirIdx + 3), 500);
    } else {
      setTimeout(() => setStep(6), 500);
    }
  };

  const handlePainChange = (dirKey: string, val: number) => {
    setPainValues(prev => ({ ...prev, [dirKey]: val }));
  };
  ...
  const submitBilan = useCallback(async () => {
    ...
    DIRECTIONS.forEach(d => {
      m[d.key] = { mobility: currentMobility[d.key] || 50, pain: painValues[d.key] || 0 };
    });
    setMeasurements(m);
    try {
      const result = await apiFetch('/api/dorsi/bilan', {
        method: 'POST',
  ...
  const createProgram = useCallback(async () => {
    ...
    try {
      await apiFetch('/api/dorsi/program', {
        method: 'POST',
        body: JSON.stringify({ bilan_id: bilanResult.id }),
      }, token);
  ...
  const renderStep = () => {
      ...
      return (
        <div style={{ textAlign: 'center', maxWidth: 420, margin: '0 auto' } as any}>
          ...
          <p style={{ fontSize: 14, color: 'rgba(255,255,255,0.45)', lineHeight: 1.7, margin: '0 0 32px' }}>
            Ce bilan evalue votre mobilite lombaire dans 4 directions. Asseyez-vous sur le coussin Dorsi, le dos droit. Vous devrez incliner votre bassin dans chaque direction demandee.
          </p>
          <div style={{ background: 'rgba(249,115,22,0.08)', border: `1px solid ${ACCENT}25`, borderRadius: 16, padding: 20, marginBottom: 32, textAlign: 'left' } as any}>
      ...
      return (
        <div style={{ textAlign: 'center', maxWidth: 420, margin: '0 auto' } as any}>
          ...
          <p style={{ fontSize: 14, color: 'rgba(255,255,255,0.45)', lineHeight: 1.7, margin: '0 0 24px' }}>
            Restez immobile dans votre position de depart. Le capteur gyroscopique du coussin va se calibrer automatiquement.
          </p>
          <div style={{ position: 'relative', width: 120, height: 120, margin: '0 auto 28px' } as any}>
            ...
            <div style={{ position: 'absolute', inset: 15, borderRadius: '50%', border: '2px solid rgba(16,185,129,0.2)', animation: 'pulseRing 2s ease-out infinite 0.7s' } as any} />
          ...
          <div data-testid="tare-done-btn" onClick={() => setStep(2)} style={{ padding: '16px 40px', borderRadius: 999, background: '#10B981', cursor: 'pointer', color: '#FFF', fontSize: 15, fontWeight: 800, display: 'inline-block' } as any}>
    ...
    // Steps 2-5: Measure each direction
    if (step >= 2 && step <= 5) {
      const dirIdx = step - 2;
      ...
      return (
        <div style={{ textAlign: 'center', maxWidth: 420, margin: '0 auto' } as any}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, marginBottom: 24 } as any}>
            {DIRECTIONS.map((_, i) => (
              <div key={i} style={{ height: 4, borderRadius: 2, width: i === dirIdx ? 28 : 12, background: i === dirIdx ? ACCENT : i < dirIdx ? `${ACCENT}66` : 'rgba(255,255,255,0.1)', transition: 'all 0.3s' } as any} />
            ))}
          </div>
          ...
          <SimulatedGyro direction={dir} onCapture={(val) => handleMobilityCapture(dir.key, val)} />
          {currentMobility[dir.key] && (
      ...
      return (
        <div style={{ maxWidth: 420, margin: '0 auto' } as any}>
          <div style={{ textAlign: 'center', marginBottom: 28 } as any}>
            ...
            <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.4)', margin: 0 }}>Indiquez votre niveau de douleur (0 = aucune, 10 = maximale) pour chaque direction.</p>
          </div>
          {DIRECTIONS.map(dir => (
            <div key={dir.key} style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 16, padding: '16px 20px', marginBottom: 12 } as any}>
              ...
              <input
                data-testid={`pain-slider-${dir.key}`}
                type="range" min={0} max={10} step={1} value={painValues[dir.key]}
                onChange={(e: any) => handlePainChange(dir.key, parseInt(e.target.value))}
                style={{ width: '100%', accentColor: ACCENT, height: 6 }}
              />
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 10, color: 'rgba(255,255,255,0.25)', marginTop: 4 } as any}>
    ...
    // Step 7: Results with radar chart
    if (step === 7) {
      return (
        <div style={{ maxWidth: 480, margin: '0 auto', textAlign: 'center' } as any}>
          ...
          <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 20, padding: 24, marginBottom: 20 } as any}>
            ...
            <div style={{ display: 'flex', justifyContent: 'center', gap: 24, marginTop: 16 } as any}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 6 } as any}>
                <div style={{ width: 12, height: 3, borderRadius: 2, background: ACCENT } as any} />
                <span style={{ fontSize: 11, color: 'rgba(255,255,255,0.5)' }}>Mobilite</span>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 6 } as any}>
                <div style={{ width: 12, height: 3, borderRadius: 2, background: '#EF4444', borderStyle: 'dashed' } as any} />
                <span style={{ fontSize: 11, color: 'rgba(255,255,255,0.5)' }}>Douleur</span>
              </div>
            </div>
          </div>
              ...
              return (
                <div key={dir.key} style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 14, padding: 14, textAlign: 'left' } as any}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 8 } as any}>
                    <i className={dir.icon} style={{ fontSize: 14, color: ACCENT }} />
                    <span style={{ fontSize: 12, fontWeight: 700, color: '#FFF' }}>{dir.label}</span>
                  </div>
          ...
          {!programCreated ? (
            <div data-testid="create-program-btn" onClick={createProgram} style={{ padding: '16px 32px', borderRadius: 999, background: ACCENT, cursor: 'pointer', color: '#FFF', fontSize: 15, fontWeight: 800, display: 'inline-block' } as any}>
              <i className="ri-calendar-check-line" style={{ marginRight: 8 }} />Generer mon programme 10 jours
            </div>
          ) : (
            <div style={{ padding: '16px 32px', borderRadius: 999, background: 'rgba(16,185,129,0.15)', border: '1px solid rgba(16,185,129,0.3)', color: '#10B981', fontSize: 15, fontWeight: 800, display: 'inline-block' } as any}>
              <i className="ri-check-line" style={{ marginRight: 8 }} />Programme cree ! Redirection...
            </div>
          )}
        </div>
      );
  ...
  return (
    <div data-testid="dorsi-bilan-page" style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, background: BG, fontFamily: 'Inter, system-ui, sans-serif', overflow: 'hidden' } as any}>
      <div style={{ position: 'absolute', inset: 0, overflowY: 'auto', padding: '0 20px 120px', WebkitOverflowScrolling: 'touch' } as any}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', padding: '20px 0 24px', gap: 12 } as any}>
          ...
          <div style={{ flex: 1 } as any}>
            ...
            <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.35)' }}>Evaluation de mobilite lombaire</div>
          </div>
        </div>
      ...
      <style dangerouslySetInnerHTML={{ __html: `
        @keyframes pulseRing{0%{transform:scale(1);opacity:0.4}100%{transform:scale(2);opacity:0}}
        input[type="range"]{-webkit-appearance:none;appearance:none;background:rgba(255,255,255,0.08);border-radius:4px;outline:none}
        input[type="range"]::-webkit-slider-thumb{-webkit-appearance:none;appearance:none;width:18px;height:18px;border-radius:50%;background:${ACCENT};cursor:pointer;border:2px solid #FFF}
      `}} />
    </div>
  );
}
