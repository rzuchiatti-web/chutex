...
export default function RemindersScreen() {
    ...
    return (
      <SafeAreaView style={{ flex: 1, backgroundColor: '#000' }}>
        <View style={{ flexDirection: 'row', alignItems: 'center', paddingHorizontal: 16, paddingVertical: 14 }}>
          ...
          <Text style={{ flex: 1, fontSize: 22, fontWeight: '900', color: 'rgba(255,255,255,0.92)' }}>Mes rappels</Text>
        </View>
            ...
            return (
              <TouchableOpacity key={c.key} onPress={() => setActiveCat(c.key)}>
                <GlassCard style={{ flexDirection: 'row', alignItems: 'center', padding: 20 }}>
                  ...
                  <View style={{ flex: 1 }}>
                    <Text style={{ fontSize: 18, fontWeight: '800', color: 'rgba(255,255,255,0.92)' }}>{c.label}</Text>
                    <Text style={{ fontSize: 12, color: 'rgba(255,255,255,0.50)', marginTop: 2 }}>{items.length} rappel{items.length !== 1 ? 's' : ''}</Text>
                    <Text style={{ fontSize: 12, fontWeight: '800', color: 'rgba(255,255,255,0.92)', marginTop: 6 }}>Prochain dans {getTimeRemaining(items.filter(r => r.active))}</Text>
                  </View>
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: '#000' }}>
      <View style={{ flexDirection: 'row', alignItems: 'center', paddingHorizontal: 16, paddingVertical: 14 }}>
        <TouchableOpacity onPress={() => setActiveCat(null)} style={{ padding: 4, marginRight: 12 }}><Ionicons name="chevron-back" size={24} color="#FFF" /></TouchableOpacity>
        <Text style={{ flex: 1, fontSize: 22, fontWeight: '900', color: 'rgba(255,255,255,0.92)' }}>{cat?.label}</Text>
      </View>
      <ScrollView contentContainerStyle={{ paddingHorizontal: 16, paddingBottom: 40 }}>
          ...
          <GlassCard key={r.id} style={{ opacity: r.active ? 1 : 0.5 }}>
            <View style={{ flexDirection: 'row', alignItems: 'flex-start', justifyContent: 'space-between' }}>
              <TouchableOpacity style={{ flex: 1 }} onPress={() => openEdit(r)}>
                <Text style={{ fontSize: 30, fontWeight: '900', color: 'rgba(255,255,255,0.92)' }}>{r.time}</Text>
                <Text style={{ fontSize: 13, fontWeight: '600', color: 'rgba(255,255,255,0.92)', marginTop: 4 }}>{r.title}</Text>
                {r.dosage ? <Text style={{ fontSize: 12, color: 'rgba(255,255,255,0.50)' }}>{r.dosage}</Text> : null}
                {/* Day chips */}
      ...
      {confirmDelete && (
        <View style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'center', padding: 32, zIndex: 100 }}>
          <View style={{ backgroundColor: 'rgba(255,255,255,0.04)', borderRadius: 24, padding: 24, alignItems: 'center' }}>
            <Ionicons name="trash-outline" size={32} color="#E53935" />
            <Text style={{ fontSize: 18, fontWeight: '800', color: 'rgba(255,255,255,0.92)', marginTop: 12 }}>Supprimer ce rappel ?</Text>
            <Text style={{ fontSize: 13, color: 'rgba(255,255,255,0.50)', marginTop: 4 }}>Cette action est irreversible</Text>
      ...
      <Modal visible={showModal} transparent animationType="slide">
        <View style={{ flex: 1, justifyContent: 'flex-end', backgroundColor: 'rgba(0,0,0,0.5)' }}>
          <View style={{ backgroundColor: 'rgba(255,255,255,0.04)', borderTopLeftRadius: 24, borderTopRightRadius: 24, padding: 24 }}>
            <Text style={{ fontSize: 20, fontWeight: '900', color: 'rgba(255,255,255,0.92)', marginBottom: 16 }}>{editR ? 'Modifier' : 'Nouveau rappel'}</Text>
            {Platform.OS === 'web' && (<>
              <div style={{ marginBottom: 12 }}><div style={{ fontSize: 11, fontWeight: '700', color: 'rgba(255,255,255,0.50)', marginBottom: 6, textTransform: 'uppercase' as any, letterSpacing: 1 }}>Titre</div><input type="text" placeholder="Ex: Boire 1 verre" value={fTitle} onChange={(e: any) => setFTitle(e.target.value)} style={{ width: '100%', fontSize: 15, padding: '12px', borderRadius: 12, border: '1.5px solid rgba(0,0,0,0.08)', background: 'rgba(245,245,245,0.8)', fontFamily: 'system-ui', boxSizing: 'border-box' as any }} /></div>
              <div style={{ marginBottom: 12 }}><div style={{ fontSize: 11, fontWeight: '700', color: 'rgba(255,255,255,0.50)', marginBottom: 6, textTransform: 'uppercase' as any, letterSpacing: 1 }}>Heure</div><input type="time" value={fTime} onChange={(e: any) => setFTime(e.target.value)} style={{ width: '100%', fontSize: 18, fontWeight: '800', padding: '12px', borderRadius: 12, border: '1.5px solid rgba(0,0,0,0.08)', background: 'rgba(245,245,245,0.8)', fontFamily: 'system-ui', boxSizing: 'border-box' as any }} /></div>
              {activeCat === 'medication' && <div style={{ marginBottom: 12 }}><div style={{ fontSize: 11, fontWeight: '700', color: 'rgba(255,255,255,0.50)', marginBottom: 6, textTransform: 'uppercase' as any, letterSpacing: 1 }}>Dosage</div><input type="text" placeholder="1 comprime" value={fDosage} onChange={(e: any) => setFDosage(e.target.value)} style={{ width: '100%', fontSize: 15, padding: '12px', borderRadius: 12, border: '1.5px solid rgba(0,0,0,0.08)', background: 'rgba(245,245,245,0.8)', fontFamily: 'system-ui', boxSizing: 'border-box' as any }} /></div>}
            </>)}
            <View style={{ flexDirection: 'row', gap: 6, marginBottom: 16, justifyContent: 'center' }}>
              {DAYS.map(d => (
                <TouchableOpacity key={d.key} style={[{ width: 38, height: 38, borderRadius: 19, justifyContent: 'center', alignItems: 'center', borderWidth: 1.5, borderColor: fDays.includes(d.key) ? '#000' : '#DDD' }, fDays.includes(d.key) && { backgroundColor: '#000' }]}
