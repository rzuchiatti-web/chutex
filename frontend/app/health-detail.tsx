  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: colors.background }}>
      <View style={{ flexDirection: 'row', alignItems: 'center', paddingHorizontal: 16, paddingVertical: 10 }}>
        ...
        <Text style={{ flex: 1, fontSize: 22, fontWeight: '900', color: colors.textPrimary, textAlign: 'center', marginRight: 36 }}>{cfg.title}</Text>
      </View>
      ...
      <ScrollView contentContainerStyle={{ paddingHorizontal: 16, paddingBottom: 40 }}>
        ...
        <GlassCard style={{ paddingTop: 70, zIndex: 2 }}>
          <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
            <Text style={{ fontSize: 38, fontWeight: '900', color: colors.textPrimary }}>{currentVal || '--'}<Text style={{ fontSize: 14, color: colors.textSecondary }}> {cfg.unit}</Text></Text>
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
            <View style={{ flexDirection: 'row', gap: 6 }}>
              {[{ k: '7', l: '7J' }, { k: '14', l: '14J' }, { k: '30', l: '30J' }].map(p => (
                <TouchableOpacity key={p.k} style={[{ paddingHorizontal: 12, paddingVertical: 6, borderRadius: 9999, borderWidth: 1.5, borderColor: period === p.k ? '#000' : 'rgba(0,0,0,0.08)' }, period === p.k && { backgroundColor: colors.background }]} onPress={() => setPeriod(p.k)}>
            ...
            {Platform.OS === 'web' && (
              <div><input type="date" value={selectedDate} onChange={(e: any) => setSelectedDate(e.target.value)} max={new Date().toISOString().split('T')[0]}
                style={{ fontSize: 12, padding: '6px 10px', borderRadius: 10, border: '1px solid rgba(0,0,0,0.08)', background: 'transparent', fontFamily: 'system-ui', cursor: 'pointer', color: colors.textSecondary }} /></div>
            )}
          </View>

          {history.length > 1 && <SimpleChart data={history} color={cfg.color} width={screenW} />}
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginTop: 4 }}>
            {history.map((_, i) => { const d = new Date(); d.setDate(d.getDate() - (history.length - 1 - i)); return <Text key={i} style={{ fontSize: 8, color: colors.textMuted }}>{d.getDate()}/{d.getMonth() + 1}</Text>; })}
          </View>
        </GlassCard>
        ...
        <GlassCard style={{ flexDirection: 'row', padding: 14 }}>
          {[{ label: 'Plus bas', val: minVal }, { label: 'Moyenne', val: avgVal }, { label: 'Plus haut', val: maxVal }].map((s, i) => (
            <View key={i} style={{ flex: 1, alignItems: 'center', borderRightWidth: i < 2 ? 1 : 0, borderRightColor: 'rgba(0,0,0,0.06)' }}>
              <Text style={{ fontSize: 22, fontWeight: '900', color: colors.textPrimary }}>{s.val}</Text>
        ...
        <GlassCard>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 8 }}>
            ...
            <Text style={{ fontSize: 14, fontWeight: '800', color: colors.textPrimary }}>Analyse IA</Text>
          </View>
        ...
        <GlassCard>
          ...
          <Text style={{ fontSize: 14, fontWeight: '900', color: colors.textPrimary, textAlign: 'center', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 10 }}>Seuils d'alertes</Text>
          ...
          <TouchableOpacity style={{ backgroundColor: colors.background, borderRadius: 9999, paddingVertical: 12, alignItems: 'center' }} onPress={() => router.push({ pathname: '/edit-thresholds', params: { metricId: metricId || 'heart_rate' } })}>
