  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: '#F5F6F8' }}>
      <View style={{ flexDirection: 'row', alignItems: 'center', paddingHorizontal: 16, paddingVertical: 10 }}>
        ...
        <Text style={{ flex: 1, fontSize: 22, fontWeight: '900', color: '#1E1F24', textAlign: 'center', marginRight: 36 }}>{cfg.title}</Text>
      </View>
      ...
      <ScrollView contentContainerStyle={{ paddingHorizontal: 16, paddingBottom: 40 }}>
        ...
        <GlassCard style={{ paddingTop: 70, zIndex: 2 }}>
          <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
            <Text style={{ fontSize: 38, fontWeight: '900', color: '#1E1F24' }}>{currentVal || '--'}<Text style={{ fontSize: 14, color: '#6B7084' }}> {cfg.unit}</Text></Text>
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
            ...
            {Platform.OS === 'web' && (
              <div><input type="date" value={selectedDate} onChange={(e: any) => setSelectedDate(e.target.value)} max={new Date().toISOString().split('T')[0]}
                style={{ fontSize: 12, padding: '6px 10px', borderRadius: 10, border: '1px solid rgba(0,0,0,0.08)', background: 'transparent', fontFamily: 'system-ui', cursor: 'pointer', color: '#6B7084' }} /></div>
            )}
          </View>

          {history.length > 1 && <SimpleChart data={history} color={cfg.color} width={screenW} />}
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginTop: 4 }}>
            {history.map((_, i) => { const d = new Date(); d.setDate(d.getDate() - (history.length - 1 - i)); return <Text key={i} style={{ fontSize: 8, color: '#9CA3B0' }}>{d.getDate()}/{d.getMonth() + 1}</Text>; })}
          </View>
        </GlassCard>
        ...
        <GlassCard style={{ flexDirection: 'row', padding: 14 }}>
          {[{ label: 'Plus bas', val: minVal }, { label: 'Moyenne', val: avgVal }, { label: 'Plus haut', val: maxVal }].map((s, i) => (
            <View key={i} style={{ flex: 1, alignItems: 'center', borderRightWidth: i < 2 ? 1 : 0, borderRightColor: 'rgba(0,0,0,0.06)' }}>
              <Text style={{ fontSize: 22, fontWeight: '900', color: '#1E1F24' }}>{s.val}</Text>
              <Text style={{ fontSize: 10, color: '#6B7084' }}>{s.label}</Text>
            </View>
          ))}
        </GlassCard>
        ...
        <GlassCard>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 8 }}>
            <Icon name="sparkles" size={18} color="#1E1F24" />
            <Text style={{ fontSize: 14, fontWeight: '800', color: '#1E1F24' }}>Analyse IA</Text>
          </View>
        ...
        <GlassCard>
          ...
          <Text style={{ fontSize: 14, fontWeight: '900', color: '#1E1F24', textAlign: 'center', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 10 }}>Seuils d'alertes</Text>
          ...
          <TouchableOpacity style={{ backgroundColor: '#F5F6F8', borderRadius: 9999, paddingVertical: 12, alignItems: 'center' }} onPress={() => router.push({ pathname: '/edit-thresholds', params: { metricId: metricId || 'heart_rate' } })}>
            <Text style={{ color: '#1E1F24', fontSize: 13, fontWeight: '800', textTransform: 'uppercase' }}>MODIFIER LES SEUILS</Text>
          </TouchableOpacity>
        </GlassCard>
      </ScrollView>
    </SafeAreaView>
