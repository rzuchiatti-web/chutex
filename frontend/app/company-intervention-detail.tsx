...
export default function CompanyInterventionDetail() {
  ...
  if (loading) return <View style={{ flex: 1, backgroundColor: '#F5F6F8', justifyContent: 'center', alignItems: 'center' }}><ActivityIndicator size="large" color="#1E1F24" /></View>;
  if (!data?.intervention) return <View style={{ flex: 1, backgroundColor: '#F5F6F8', justifyContent: 'center', alignItems: 'center' }}><Text style={{ color: '#6B7084' }}>Intervention non trouvee</Text></View>;
  ...
  if (isCompleted) {
    ...
    const GC = ({ children, style: s2 }: any) => <View style={[{ backgroundColor: '#FFFFFF', borderRadius: 24, borderWidth: 1, borderColor: 'rgba(0,0,0,0.06)', padding: 18, marginBottom: 12, ...glass }, s2]}>{children}</View>;
    const IR = ({ icon, label, value, color: c }: any) => value ? (
      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10, paddingVertical: 8, borderBottomWidth: 0.5, borderBottomColor: 'rgba(0,0,0,0.04)' }}>
        <Icon name={icon} size={16} color={c || '#888'} /><Text style={{ fontSize: 12, color: '#6B7084', width: 100 }}>{label}</Text><Text style={{ fontSize: 13, fontWeight: '600', color: '#1E1F24', flex: 1 }}>{value}</Text>
      </View>
    ) : null;
    return (
      <View style={{ flex: 1, backgroundColor: '#F5F6F8' }}>
        <View style={{ flexDirection: 'row', alignItems: 'center', paddingHorizontal: 16, paddingVertical: 14 }}>
          ...
          <Text style={{ flex: 1, fontSize: 18, fontWeight: '900', color: '#1E1F24' }}>Intervention terminee</Text>
        ...
        <ScrollView contentContainerStyle={{ paddingHorizontal: 16, paddingBottom: 40 }} showsVerticalScrollIndicator={false}>
          ...
          <GC style={{ borderLeftWidth: 4, borderLeftColor: '#4CAF50', padding: 20 }}>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12 }}>
              ...
              <View style={{ flex: 1 }}>
                ...
                <Text style={{ fontSize: 16, fontWeight: '900', color: '#1E1F24', marginTop: 4 }}>{alert_data?.alert_type === 'sos' ? 'SOS - Urgence' : alert_data?.alert_type === 'fall' ? 'Chute detectee' : 'Alerte'}</Text>
                <Text style={{ fontSize: 12, color: '#6B7084', marginTop: 2 }}>{iv.alert_message || alert_data?.message}</Text>
              </View>
            </View>
          </GC>
          {/* Report */}
          {rpt && (
            <GC style={{ borderLeftWidth: 4, borderLeftColor: '#4CAF50' }}>
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10, marginBottom: 14 }}>
                ...
                <Text style={{ fontSize: 16, fontWeight: '800', color: '#1E1F24' }}>Rapport d'intervention</Text>
              </View>
              ...
              {rpt.description && <View style={{ marginBottom: 8 }}><Text style={{ fontSize: 11, fontWeight: '700', color: '#6B7084', marginBottom: 4 }}>Description</Text><Text style={{ fontSize: 13, color: '#1E1F24', lineHeight: 20 }}>{rpt.description}</Text></View>}
              {rpt.actions_taken && <View style={{ marginBottom: 8 }}><Text style={{ fontSize: 11, fontWeight: '700', color: '#6B7084', marginBottom: 4 }}>Actions effectuees</Text><Text style={{ fontSize: 13, color: '#1E1F24', lineHeight: 20 }}>{rpt.actions_taken}</Text></View>}
              ...
              {rpt.completed_by && <Text style={{ fontSize: 10, color: '#6B7084', marginTop: 8 }}>Rapport par {rpt.completed_by} - {rpt.completed_at ? new Date(rpt.completed_at).toLocaleString('fr-FR') : ''}</Text>}
            </GC>
          )}
          {/* Beneficiary */}
          <GC>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10, marginBottom: 14 }}>
              ...
              <Text style={{ fontSize: 16, fontWeight: '800', color: '#1E1F24' }}>Beneficiaire</Text>
            </View>
          ...
          {intervenant && (
            <GC>
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10, marginBottom: 14 }}>
                ...
                <Text style={{ fontSize: 16, fontWeight: '800', color: '#1E1F24' }}>Intervenant</Text>
              </View>
          ...
          {alert_data && (
            <GC>
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10, marginBottom: 14 }}>
                ...
                <Text style={{ fontSize: 16, fontWeight: '800', color: '#1E1F24' }}>Alerte</Text>
              </View>
          ...
          {(iv.timeline || []).length > 0 && (
            <GC>
              <Text style={{ fontSize: 11, fontWeight: '700', color: '#6B7084', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 12 }}>CHRONOLOGIE</Text>
              {iv.timeline.map((t: any, i: number) => (
                <View key={i} style={{ flexDirection: 'row', gap: 10, marginBottom: 4 }}>
                  ...
                  <View style={{ flex: 1 }}><Text style={{ fontSize: 12, color: '#1E1F24' }}>{t.note || t.status}</Text><Text style={{ fontSize: 10, color: '#9CA3B0' }}>{t.time ? new Date(t.time).toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' }) : ''}</Text></View>
                </View>
              ))}
            </GC>
          )}
  ...
  return (
    <View style={{ flex: 1, backgroundColor: '#F5F6F8' }}>
      ...
      <View style={{ height: MAP_H, position: 'relative' }}>
        ...
        <TouchableOpacity onPress={() => router.back()} style={{ position: 'absolute', top: 50, left: 16, width: 44, height: 44, borderRadius: 24, backgroundColor: '#FFFFFF', justifyContent: 'center', alignItems: 'center', ...(Platform.OS === 'web' ? { boxShadow: '0 2px 12px rgba(0,0,0,0.15)' } : {}) }}>
        ...
        {iv.distance_km && !isCompleted && (
          <View style={{ position: 'absolute', bottom: 16, alignSelf: 'center', backgroundColor: '#F5F6F8', paddingHorizontal: 20, paddingVertical: 10, borderRadius: 20, flexDirection: 'row', alignItems: 'center', gap: 8, ...(Platform.OS === 'web' ? { boxShadow: '0 4px 16px rgba(0,0,0,0.3)', left: '50%', transform: 'translateX(-50%)' as any } : {}) }}>
            <Icon name="navigate" size={16} color="#1E1F24" />
      ...
      {/* SLIDE-UP CARD */}
      <View style={{ flex: 1, backgroundColor: '#FFFFFF', borderTopLeftRadius: 28, borderTopRightRadius: 28, marginTop: -24, ...(Platform.OS === 'web' ? { boxShadow: '0 -4px 24px rgba(0,0,0,0.1)' } : {}) }}>
        ...
        <ScrollView contentContainerStyle={{ paddingHorizontal: 20, paddingBottom: 40 }} showsVerticalScrollIndicator={false}>
          ...
          {iv.assigned_name ? (
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 14, marginBottom: 14 }}>
              ...
              <View style={{ flex: 1 }}>
                <Text style={{ fontSize: 18, fontWeight: '900', color: '#1E1F24' }}>{iAmIntervenant ? 'Vous intervenez' : iv.assigned_name}</Text>
                <Text style={{ fontSize: 12, color: '#6B7084' }}>{iv.structure_name || ''}{iv.distance_km ? ` - ${iv.distance_km} km` : ''}</Text>
              </View>
          ...
          {/* Patient info */}
          <Text style={{ fontSize: 11, fontWeight: '800', color: '#6B7084', letterSpacing: 1, marginBottom: 8 }}>PATIENT</Text>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12, marginBottom: 10 }}>
            <View style={{ width: 44, height: 44, borderRadius: 24, backgroundColor: '#E1F5FE', justifyContent: 'center', alignItems: 'center' }}>
              <Text style={{ fontSize: 18, fontWeight: '900', color: '#0288D1' }}>{(ben.name || '?').charAt(0)}</Text>
            </View>
            <View style={{ flex: 1 }}>
              <Text style={{ fontSize: 16, fontWeight: '900', color: '#1E1F24' }}>{ben.name || iv.beneficiary_name}</Text>
          ...
          {(ben.medical_conditions || ben.allergies || ben.blood_type) && (
            <View style={{ backgroundColor: '#FFF8E1', borderRadius: 12, padding: 12, marginBottom: 14, gap: 4 }}>
              {ben.medical_conditions && <Text style={{ fontSize: 12, color: '#1E1F24' }}><Text style={{ fontWeight: '700' }}>Pathologies:</Text> {ben.medical_conditions}</Text>}
              {ben.allergies && <Text style={{ fontSize: 12, color: '#1E1F24' }}><Text style={{ fontWeight: '700' }}>Allergies:</Text> {ben.allergies}</Text>}
              {ben.blood_type && <Text style={{ fontSize: 12, color: '#1E1F24' }}><Text style={{ fontWeight: '700' }}>Groupe:</Text> {ben.blood_type}</Text>}
              {ben.doctor_name && <Text style={{ fontSize: 12, color: '#1E1F24' }}><Text style={{ fontWeight: '700' }}>Medecin:</Text> {ben.doctor_name}</Text>}
            </View>
          )}
          ...
          {isCompleted && rpt && (
            <View style={{ marginBottom: 14 }}>
              <Text style={{ fontSize: 11, fontWeight: '800', color: '#6B7084', letterSpacing: 1, marginBottom: 8 }}>RAPPORT D'INTERVENTION</Text>
              <View style={{ backgroundColor: '#E8F5E9', borderRadius: 14, padding: 14, borderLeftWidth: 4, borderLeftColor: '#4CAF50' }}>
                ...
                {rpt.description && <Text style={{ fontSize: 12, color: '#1E1F24', marginBottom: 4 }}>{rpt.description}</Text>}
                {rpt.actions_taken && <Text style={{ fontSize: 12, color: '#1E1F24', marginBottom: 4 }}><Text style={{ fontWeight: '700' }}>Actions:</Text> {rpt.actions_taken}</Text>}
                {rpt.follow_up_needed && <Text style={{ fontSize: 12, color: '#E65100', fontWeight: '600' }}>Suivi necessaire: {rpt.follow_up_notes}</Text>}
                {rpt.completed_by && <Text style={{ fontSize: 10, color: '#6B7084', marginTop: 6 }}>Par {rpt.completed_by} - {rpt.completed_at ? new Date(rpt.completed_at).toLocaleString('fr-FR') : ''}</Text>}
              </View>
            </View>
          )}
          ...
          {(iv.timeline || []).length > 0 && (
            <View>
              <Text style={{ fontSize: 11, fontWeight: '800', color: '#6B7084', letterSpacing: 1, marginBottom: 8 }}>CHRONOLOGIE</Text>
              {iv.timeline.map((t: any, i: number) => (
                <View key={i} style={{ flexDirection: 'row', gap: 10, marginBottom: 4 }}>
                  <View style={{ width: 8, height: 8, borderRadius: 4, backgroundColor: i === iv.timeline.length - 1 ? sc : '#DDD', marginTop: 5 }} />
                  <View style={{ flex: 1 }}>
                    <Text style={{ fontSize: 12, color: '#1E1F24' }}>{t.note || t.status}</Text>
                    <Text style={{ fontSize: 10, color: '#9CA3B0' }}>{t.time ? new Date(t.time).toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' }) : ''}</Text>
                  </View>
                </View>
              ))}
            </View>
        ...
        <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'flex-end' }}>
          <View style={{ backgroundColor: '#FFFFFF', borderTopLeftRadius: 28, borderTopRightRadius: 28, padding: 24, maxHeight: '85%' }}>
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              <Text style={{ fontSize: 20, fontWeight: '900', color: '#1E1F24' }}>Rapport d'intervention</Text>
              <TouchableOpacity onPress={() => setShowReport(false)}><Icon name="close" size={24} color="#888" /></TouchableOpacity>
            </View>
            <ScrollView showsVerticalScrollIndicator={false}>
              <Text style={{ fontSize: 12, color: '#6B7084', marginBottom: 4 }}>Etat du patient</Text>
              ...
              <Text style={{ fontSize: 12, color: '#6B7084', marginBottom: 4 }}>Description</Text>
              ...
              <Text style={{ fontSize: 12, color: '#6B7084', marginBottom: 4 }}>Actions effectuees</Text>
              ...
              <TouchableOpacity style={{ flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 12 }}
                ...
                <Text style={{ fontSize: 14, color: '#1E1F24' }}>Suivi necessaire</Text>
              </TouchableOpacity>
              ...
              <TouchableOpacity style={{ backgroundColor: '#10B981', borderRadius: 14, paddingVertical: 16, alignItems: 'center', flexDirection: 'row', justifyContent: 'center', gap: 8 }}
                ...
                {submitting ? <ActivityIndicator color="#1E1F24" /> : (
                  <><Icon name="checkmark-circle" size={20} color="#1E1F24" /><Text style={{ color: '#FFF', fontSize: 15, fontWeight: '900' }}>TERMINER L'INTERVENTION</Text></>
                )}
              </TouchableOpacity>
            </ScrollView>
          </View>
