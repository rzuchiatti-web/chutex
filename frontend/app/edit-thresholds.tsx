  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: 'transparent' }}>
      <View style={{ flexDirection: 'row', alignItems: 'center', paddingHorizontal: 16, paddingVertical: 14 }}>
        ...
        <Text style={{ flex: 1, fontSize: 22, fontWeight: '900', color: '#111827' }}>Modifier les seuils</Text>
      </View>
      <ScrollView contentContainerStyle={{ paddingHorizontal: 16, paddingBottom: 40, alignItems: 'center' }}>
        ...
        <Text style={{ fontSize: 20, fontWeight: '900', color: '#111827', textAlign: 'center', textTransform: 'uppercase', marginBottom: 24 }}>{config.title}</Text>
        ...
        <View style={[{ width: '100%', backgroundColor: '#FFFFFF', borderRadius: 24, borderWidth: 1, borderColor: 'rgba(0,0,0,0.06)', padding: 24, marginBottom: 16, ...glass }]}>
          ...
          <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
            <Text style={{ fontSize: 11, fontWeight: '700', color: '#6B7280' }}>{config.min}{config.unit}</Text>
            <Text style={{ fontSize: 11, fontWeight: '700', color: '#6B7280' }}>{config.max}{config.unit}</Text>
          </View>
        </View>

        {/* Confirm */}
        <TouchableOpacity style={{ width: '100%', backgroundColor: '#FFFFFF', borderRadius: 9999, paddingVertical: 18, alignItems: 'center', ...(Platform.OS === 'web' ? { boxShadow: '0 4px 16px rgba(0,0,0,0.15)' } : {}) }} onPress={save} disabled={saving}>
          {saving ? <ActivityIndicator color="#111827" /> : <Text style={{ color: '#FFF', fontSize: 16, fontWeight: '800', textTransform: 'uppercase', letterSpacing: 1 }}>CONFIRMER LES NOUVEAUX SEUILS</Text>}
        </TouchableOpacity>
      </ScrollView>
    </SafeAreaView>
  );
