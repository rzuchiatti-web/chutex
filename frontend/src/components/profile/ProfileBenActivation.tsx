...
export function ProfileBenActivation({ show, onClose, user, apiFetch, token, refreshUser }: Props) {
  ...
  const toggleArr = (k: string, v: string) => { const arr = benForm[k] || []; setBenForm({ ...benForm, [k]: arr.includes(v) ? arr.filter((x: string) => x !== v) : [...arr.filter((x: string) => x !== t('none_female') && x !== t('none_label')), v] }); };
    ...
    <div style={POP as any}>
      <div onClick={(e: any) => e.stopPropagation()} style={{ width: '100%', maxWidth: 420, margin: '0 auto', padding: '70px 28px 120px', boxSizing: 'border-box' } as any}>
        ...
        {benStep === 2 && (<>
          ...
          <div style={GLASS}><div style={{ ...LBL, marginBottom: 10 }}>Pathologies / Antecedents</div><div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 } as any}>{[t('condition_diabetes'),t('condition_hypertension'),t('condition_cholesterol'),t('condition_arthrose'),t('condition_heart_failure'),t('condition_stroke'),t('condition_asthma'),t('condition_osteoporosis'),t('condition_parkinson'),t('condition_alzheimer'),t('condition_depression'),t('none_female')].map(c => <Chip key={c} label={c} sel={benForm.medical_conditions.includes(c)} click={() => { if (c === t('none_female')) uf('medical_conditions', [t('none_female')]); else toggleArr('medical_conditions', c); }} />)}</div></div>
          <div style={GLASS}><div style={{ ...LBL, marginBottom: 10 }}>Allergies</div><div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 } as any}>{[t('allergy_penicillin'),t('allergy_aspirin'),t('allergy_latex'),'Arachides',t('allergy_gluten'),t('allergy_lactose'),t('allergy_iodine'),t('none_female')].map(a => <Chip key={a} label={a} sel={benForm.allergies.includes(a)} click={() => { if (a === t('none_female')) uf('allergies', [t('none_female')]); else toggleArr('allergies', a); }} />)}</div></div>
        ...
        {benStep === 3 && (<>
          ...
          <div style={GLASS}><div style={{ ...LBL, marginBottom: 10 }}>Antecedents familiaux</div><div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 } as any}>{[t('condition_diabetes'),t('condition_hypertension'),'Maladie cardiaque',t('condition_stroke'),'Cancer',t('condition_alzheimer'),t('condition_parkinson'),t('none_label')].map(f => <Chip key={f} label={f} sel={benForm.family_history.includes(f)} click={() => { if (f === t('none_label')) uf('family_history', [t('none_label')]); else toggleArr('family_history', f); }} />)}</div></div>
