...
export default function HeroScore({ bioAge, realAge, status, statusColor, ai, subs, showDetail, setShowDetail, d }: Props) {
  ...
  return (
    <>
      {/* BioAge hero */}
      <div style={{ textAlign: 'center', padding: '24px 20px 16px', borderRadius: 24, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', marginBottom: 14 } as any}>
      ...
      {/* Comprendre mon corps button */}
      <div onClick={() => setShowDetail(true)} style={{ padding: '14px', borderRadius: 16, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, marginBottom: 14 } as any}>
        <i className="ri-body-scan-line" style={{ fontSize: 16, color: 'rgba(167,139,250,0.6)' }} />
        <span style={{ fontSize: 13, fontWeight: 700, color: 'rgba(255,255,255,0.6)' }}>Comprendre mon corps</span>
        <i className="ri-arrow-right-s-line" style={{ fontSize: 16, color: 'rgba(255,255,255,0.2)' }} />
      </div>

      {/* Glass popup — full body analysis */}
      {showDetail && (
        <div onClick={() => setShowDetail(false)} style={{ position: 'fixed', inset: 0, zIndex: 9999, backdropFilter: 'blur(32px)', WebkitBackdropFilter: 'blur(32px)', background: 'rgba(0,0,0,0.4)', overflowY: 'auto' } as any}>
          <div onClick={(e: any) => e.stopPropagation()} style={{ width: '100%', maxWidth: 420, margin: '0 auto', padding: '40px 24px 120px', boxSizing: 'border-box' } as any}>
            ...
            {/* AI explanations */}
            {ai.score_explain_up && (
              <div style={{ padding: '12px 16px', borderRadius: 14, background: 'rgba(16,185,129,0.06)', border: '1px solid rgba(16,185,129,0.12)', marginBottom: 8 } as any}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 } as any}><i className="ri-arrow-up-circle-line" style={{ fontSize: 14, color: '#10B981' }} /><span style={{ fontSize: 10, fontWeight: 700, color: '#10B981', textTransform: 'uppercase' }}>Points forts</span></div>
                <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.5)', lineHeight: 1.5 }}>{ai.score_explain_up}</div>
              </div>
            )}
            {ai.score_explain_down && (
              <div style={{ padding: '12px 16px', borderRadius: 14, background: 'rgba(245,158,11,0.06)', border: '1px solid rgba(245,158,11,0.12)', marginBottom: 8 } as any}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 } as any}><i className="ri-alert-line" style={{ fontSize: 14, color: '#F59E0B' }} /><span style={{ fontSize: 10, fontWeight: 700, color: '#F59E0B', textTransform: 'uppercase' }}>A surveiller</span></div>
                <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.5)', lineHeight: 1.5 }}>{ai.score_explain_down}</div>
              </div>
            )}

            {/* AI Correlations */}
            {ai.correlations && ai.correlations.length > 0 && (
              <div style={{ padding: '16px', borderRadius: 16, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)', marginBottom: 12 } as any}>
                <div style={{ fontSize: 10, fontWeight: 700, color: 'rgba(167,139,250,0.5)', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 10 }}>Correlations detectees</div>
                {ai.correlations.map((c: string, i: number) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 10, padding: '8px 0', borderTop: i > 0 ? '1px solid rgba(255,255,255,0.04)' : 'none' } as any}>
                    <i className="ri-links-line" style={{ fontSize: 13, color: 'rgba(167,139,250,0.4)', marginTop: 2, flexShrink: 0 }} /><div style={{ fontSize: 12, color: 'rgba(255,255,255,0.5)', lineHeight: 1.5 }}>{c}</div>
                  </div>
                ))}
              </div>
            )}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 } as any}>
              {[
                ...
                <div key={i} style={{ padding: '12px 14px', borderRadius: 14, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)' } as any}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 } as any}><i className={m.icon} style={{ fontSize: 12, color: m.color }} /><span style={{ fontSize: 9, color: 'rgba(255,255,255,0.3)', textTransform: 'uppercase' }}>{m.label}</span></div>
