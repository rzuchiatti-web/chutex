...
function CompanyPrescriptionsTab({ token, user }: { token: string; user: any }) {
        ...
        {showChallengeExplainer && (
          <div onClick={() => setShowChallengeExplainer(false)} style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, zIndex: 99999, backdropFilter: 'blur(32px)', WebkitBackdropFilter: 'blur(32px)', background: 'rgba(0,0,0,0.2)', overflowY: 'scroll', WebkitOverflowScrolling: 'touch' } as any}>
            <div onClick={(e: any) => e.stopPropagation()} style={{ width: '100%', maxWidth: 420, margin: '0 auto', padding: '70px 28px 120px', boxSizing: 'border-box' } as any}>
              ...
              {[
                { icon: 'ri-file-text-line', title: 'Prescrivez', desc: 'Vos gardiens creent des prescriptions. Quand le bénéficiaire souscrit, elle est validee.', color: '#D4845A' },
        ...
        <div style={{ flex: 1, overflowY: 'auto', position: 'relative', zIndex: 5, padding: '12px 20px 100px', WebkitOverflowScrolling: 'touch' } as any}>
          {filtered.map((p: any) => (
            <div key={p.id} onClick={() => router.push({ pathname: '/company-prescriber-detail', params: { prescriberId: p.id } })} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '14px 16px', borderRadius: 18, background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.08)', marginBottom: 8, cursor: 'pointer' } as any}>
              ...
              <div style={{ flex: 1 } as any}><div style={{ fontSize: 14, fontWeight: 700, color: '#FFF' }}>{p.name}</div><div style={{ fontSize: 11, color: 'rgba(255,255,255,0.4)' }}>{p.prescriber_structure || p.structure_name || t('prescriber')}</div></div>
              <i className="ri-arrow-right-s-line" style={{ fontSize: 16, color: 'rgba(255,255,255,0.25)' }} />
            </div>
          ))}
          {filtered.length === 0 && <div style={{ textAlign: 'center', padding: '40px 20px' } as any}><i className="ri-file-text-line" style={{ fontSize: 36, color: 'rgba(255,255,255,0.15)' }} /><div style={{ fontSize: 14, fontWeight: 700, color: '#FFF', marginTop: 10 }}>{search ? 'Aucun résultat' : 'Aucun prescripteur'}</div></div>}
        </div>
      </div>
    );
  }
  ...
  if (selectedPresc && Platform.OS === 'web') {
    ...
    const pRows = [
      (prescriberProfile.name || selectedPresc.guardian_name) && { icon: 'ri-user-settings-line', label: t('prescriber'), value: prescriberProfile.name || selectedPresc.guardian_name || selectedPresc.prescriber_name },
      selectedPresc.subscription_type && { icon: 'ri-vip-crown-line', label: t('subscription'), value: selectedPresc.subscription_type },
      ...
      selectedPresc.beneficiary_phone && { icon: 'ri-phone-line', label: 'Telephone bénéficiaire', value: selectedPresc.beneficiary_phone, phone: true },
      selectedPresc.beneficiary_email && { icon: 'ri-mail-line', label: 'Email bénéficiaire', value: selectedPresc.beneficiary_email },
      ...
      selectedPresc.beneficiary_address && { icon: 'ri-map-pin-line', label: t('address'), value: selectedPresc.beneficiary_address },
      selectedPresc.structure_name && { icon: 'ri-building-line', label: t('structure'), value: selectedPresc.structure_name },
    ].filter(Boolean);
    return (
      <div style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, display: 'flex', flexDirection: 'column', fontFamily: 'Inter, system-ui, sans-serif', overflow: 'hidden' } as any}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', position: 'relative', zIndex: 5, padding: '16px 20px 100px', WebkitOverflowScrolling: 'touch' } as any} data-animate>
          ...
          <div style={{ padding: '14px 16px', borderRadius: 20, background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', marginBottom: 10 } as any}>
            <div style={{ fontSize: 10, fontWeight: 600, color: 'rgba(255,255,255,0.35)', letterSpacing: 1, textTransform: 'uppercase', marginBottom: 8 }}>Détails de la prescription</div>
          ...
          {isValidated && (
            <div onClick={() => setShowBenPopup(true)} style={{ padding: '14px 16px', borderRadius: 20, background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', marginBottom: 10, cursor: 'pointer' } as any}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 } as any}><div style={{ fontSize: 10, fontWeight: 600, color: 'rgba(255,255,255,0.35)', letterSpacing: 1, textTransform: 'uppercase' }}>Bénéficiaire</div><i className="ri-arrow-right-s-line" style={{ fontSize: 16, color: 'rgba(255,255,255,0.3)' }} /></div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 } as any}>
        ...
        {showPrescriberPopup && <div onClick={() => setShowPrescriberPopup(false)} style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, zIndex: 9999, backdropFilter: 'blur(32px)', WebkitBackdropFilter: 'blur(32px)', background: 'rgba(0,0,0,0.2)', overflowY: 'scroll', WebkitOverflowScrolling: 'touch' } as any}><div onClick={(e: any) => e.stopPropagation()} style={{ width: '100%', maxWidth: 400, margin: '0 auto', padding: '70px 28px 120px', boxSizing: 'border-box' } as any}>
          ...
          {[prescriberProfile.phone && { icon: 'ri-phone-line', label: t('phone_label'), value: prescriberProfile.phone, phone: true }, prescriberProfile.email && { icon: 'ri-mail-line', label: t('email_label'), value: prescriberProfile.email }, prescriberProfile.profession && { icon: 'ri-stethoscope-line', label: 'Profession', value: prescriberProfile.profession }, (prescriberProfile.prescriber_structure || prescriberProfile.structure_name) && { icon: 'ri-building-line', label: t('structure'), value: prescriberProfile.prescriber_structure || prescriberProfile.structure_name }, prescriberProfile.address && { icon: 'ri-map-pin-line', label: t('address'), value: prescriberProfile.address }, { icon: 'ri-vip-crown-line', label: 'Type prescription', value: selectedPresc.subscription_type === 'bracelet_gilet' ? 'Bracelet + Gilet Elder' : 'Bracelet Elio' }, { icon: 'ri-money-euro-circle-line', label: 'Commission', value: `${selectedPresc.commission || getCommission(selectedPresc)} EUR${commLabel}` }, selectedPresc.created_at && { icon: 'ri-calendar-line', label: 'Date prescription', value: new Date(selectedPresc.created_at).toLocaleString('fr-FR', { day: '2-digit', month: 'long', year: 'numeric' }) }].filter(Boolean).map((item: any, i: number, arr: any[]) => (<div key={i}><div onClick={() => item.phone && (window.location.href = `tel:${item.value}`)} style={{ display: 'flex', alignItems: 'center', gap: 14, cursor: item.phone ? 'pointer' : 'default', padding: '13px 0' } as any}><div style={{ width: 34, height: 34, borderRadius: 10, background: item.phone ? 'rgba(16,185,129,0.1)' : 'rgba(255,255,255,0.08)', border: `1px solid ${item.phone ? 'rgba(16,185,129,0.2)' : 'rgba(255,255,255,0.1)'}`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 } as any}><i className={item.icon} style={{ fontSize: 15, color: item.phone ? '#10B981' : 'rgba(255,255,255,0.5)' }} /></div><div style={{ flex: 1 } as any}><div style={{ fontSize: 9, fontWeight: 600, color: 'rgba(255,255,255,0.3)', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 2 }}>{item.label}</div><div style={{ fontSize: 15, color: '#FFF', fontWeight: item.phone ? 700 : 500 }}>{item.value}</div></div></div>{i < arr.length - 1 && <div style={{ height: 1, background: 'rgba(255,255,255,0.08)' } as any} />}</div>))}
        </div></div>}
        {/* BENEFICIARY POPUP */}
        {showBenPopup && <div onClick={() => setShowBenPopup(false)} style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, zIndex: 9999, backdropFilter: 'blur(32px)', WebkitBackdropFilter: 'blur(32px)', background: 'rgba(0,0,0,0.2)', overflowY: 'scroll', WebkitOverflowScrolling: 'touch' } as any}><div onClick={(e: any) => e.stopPropagation()} style={{ width: '100%', maxWidth: 400, margin: '0 auto', padding: '70px 28px 120px', boxSizing: 'border-box' } as any}>
          ...
          <div style={{ fontSize: 10, fontWeight: 700, color: 'rgba(255,255,255,0.35)', letterSpacing: 2, textTransform: 'uppercase', marginBottom: 16 }}>Fiche bénéficiaire</div>
          ...
          {[selectedPresc.beneficiary_phone && { icon: 'ri-phone-line', label: t('phone_label'), value: selectedPresc.beneficiary_phone, phone: true }, selectedPresc.beneficiary_email && { icon: 'ri-mail-line', label: t('email_label'), value: selectedPresc.beneficiary_email }, selectedPresc.beneficiary_address && { icon: 'ri-map-pin-line', label: t('address'), value: selectedPresc.beneficiary_address }, selectedPresc.guardian_contact_name && { icon: 'ri-user-heart-line', label: 'Aidant', value: selectedPresc.guardian_contact_name }, selectedPresc.guardian_contact_phone && { icon: 'ri-phone-line', label: 'Telephone aidant', value: selectedPresc.guardian_contact_phone, phone: true }, { icon: 'ri-vip-crown-line', label: t('subscription'), value: selectedPresc.subscription_type || 'Standard' }, selectedPresc.subscribed_at && { icon: 'ri-calendar-check-line', label: 'Souscrit le', value: new Date(selectedPresc.subscribed_at).toLocaleString('fr-FR', { day: '2-digit', month: 'long', year: 'numeric' }) }, { icon: 'ri-check-double-line', label: 'Statut', value: 'Souscription validee' }].filter(Boolean).map((item: any, i: number, arr: any[]) => (<div key={i}><div onClick={() => item.phone && (window.location.href = `tel:${item.value}`)} style={{ display: 'flex', alignItems: 'center', gap: 14, cursor: item.phone ? 'pointer' : 'default', padding: '13px 0' } as any}><div style={{ width: 34, height: 34, borderRadius: 10, background: item.phone ? 'rgba(16,185,129,0.1)' : 'rgba(255,255,255,0.08)', border: `1px solid ${item.phone ? 'rgba(16,185,129,0.2)' : 'rgba(255,255,255,0.1)'}`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 } as any}><i className={item.icon} style={{ fontSize: 15, color: item.phone ? '#10B981' : 'rgba(255,255,255,0.5)' }} /></div><div style={{ flex: 1 } as any}><div style={{ fontSize: 9, fontWeight: 600, color: 'rgba(255,255,255,0.3)', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 2 }}>{item.label}</div><div style={{ fontSize: 15, color: '#FFF', fontWeight: item.phone ? 700 : 500 }}>{item.value}</div></div></div>{i < arr.length - 1 && <div style={{ height: 1, background: 'rgba(255,255,255,0.08)' } as any} />}</div>))}
        </div></div>}
      </div>
    );
  }
    ...
    return (
      <div style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, display: 'flex', flexDirection: 'column', fontFamily: 'Inter, system-ui, sans-serif', overflow: 'hidden' } as any}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', position: 'relative', zIndex: 5, padding: '24px 20px 100px', WebkitOverflowScrolling: 'touch' } as any}>
          ...
          {filteredPrescs.length === 0 && <div style={{ textAlign: 'center', padding: '40px 20px' } as any}><i className="ri-file-text-line" style={{ fontSize: 36, color: 'rgba(255,255,255,0.15)' }} /><div style={{ fontSize: 14, fontWeight: 700, color: 'rgba(255,255,255,0.5)', marginTop: 10 }}>{searchPresc ? 'Aucun résultat' : `Aucune prescription ${prescTab === 'pending' ? 'en cours' : 'validee'}`}</div></div>}
        </div>
      </div>
    );
  }
