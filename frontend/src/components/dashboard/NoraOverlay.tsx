...
export default function NoraOverlay({ token, endpoint, title, subtitle, onClose }: NoraOverlayProps) {
  ...
  return (
    <div data-testid="nora-overlay" style={{
      ...
      <div style={{
        ...
        {/* Early exit button */}
        <div data-testid="nora-early-back" onClick={onClose} style={{ position: 'fixed', top: 20, left: 20, width: 40, height: 40, borderRadius: 999, background: '#FFF', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', zIndex: 100001, boxShadow: '0 4px 16px rgba(0,0,0,0.3)' } as any}>
        ...
        {phase === 'intro' && (
          <div style={{ textAlign: 'center', animation: 'noraTextIn 0.6s ease 0.3s both' } as any}>
            ...
            <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.25)', marginTop: 8 }}>{subtitle}</div>
          </div>
        )}
...
/* Reusable Nora Button */
export function NoraButton({ label, sublabel, onClick }: { label: string; sublabel: string; onClick: () => void }) {
  return (
    <div data-testid="nora-btn" onClick={onClick}
      ...
      <div style={{ flex: 1 } as any}>
        ...
        <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.35)', marginTop: 1 }}>{sublabel}</div>
      </div>
