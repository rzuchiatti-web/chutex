...
interface Props { br: any; sc: any; vs: any; onStartWeighing?: () => void; weighings?: any[]; }
...
export default function DeviceCards({ br, sc, vs, onStartWeighing, weighings = [] }: Props) {
  ...
  const BatteryBar = ({ pct }: { pct: number }) => (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px 0' } as any}>
      <div style={{ flex: 1, height: 8, borderRadius: 4, background: 'rgba(255,255,255,0.06)', overflow: 'hidden' } as any}>
        <div style={{ height: 8, borderRadius: 4, width: `${pct}%`, background: batteryGrad(pct), transition: 'width 0.5s' } as any} />
      </div>
        ...
        <div onClick={() => setSelected(null)} style={{ position: 'fixed', inset: 0, zIndex: 9999, backdropFilter: 'blur(32px)', WebkitBackdropFilter: 'blur(32px)', background: 'rgba(0,0,0,0.4)', overflowY: 'auto' } as any}>
          <div onClick={(e: any) => e.stopPropagation()} style={{ width: '100%', maxWidth: 420, margin: '0 auto', padding: '40px 24px 120px', boxSizing: 'border-box' } as any}>
            ...
            <div style={{ display: 'flex', gap: 8, marginBottom: 8 } as any}>
              <div onClick={() => setSelected(null)} style={{ flex: 1, padding: '14px', borderRadius: 999, background: 'rgba(34,211,238,0.1)', border: '1px solid rgba(34,211,238,0.2)', cursor: 'pointer', textAlign: 'center', fontSize: 13, fontWeight: 700, color: '#22D3EE' } as any}><i className="ri-refresh-line" style={{ marginRight: 6 }} />Synchroniser</div>
              <div onClick={() => setSelected(null)} style={{ flex: 1, padding: '14px', borderRadius: 999, background: 'rgba(249,115,22,0.1)', border: '1px solid rgba(249,115,22,0.2)', cursor: 'pointer', textAlign: 'center', fontSize: 13, fontWeight: 700, color: '#F97316' } as any}><i className="ri-pulse-line" style={{ marginRight: 6 }} />ECG</div>
            </div>
            <div onClick={() => { if (window.confirm('Supprimer le Bracelet Elio ?')) setSelected(null); }} style={{ padding: '12px', borderRadius: 999, background: 'rgba(239,68,68,0.06)', border: '1px solid rgba(239,68,68,0.12)', cursor: 'pointer', textAlign: 'center', fontSize: 12, fontWeight: 600, color: 'rgba(239,68,68,0.5)' } as any}>Supprimer l'appareil</div>
          </div>
        </div>
      )}
      ...
      {selected === 'vest' && (
        <div onClick={() => setSelected(null)} style={{ position: 'fixed', inset: 0, zIndex: 9999, backdropFilter: 'blur(32px)', WebkitBackdropFilter: 'blur(32px)', background: 'rgba(0,0,0,0.4)', overflowY: 'auto' } as any}>
          <div onClick={(e: any) => e.stopPropagation()} style={{ width: '100%', maxWidth: 420, margin: '0 auto', padding: '40px 24px 120px', boxSizing: 'border-box' } as any}>
            ...
            <div onClick={() => { if (window.confirm('Supprimer le Gilet Elder S-AIRBAG ?')) setSelected(null); }} style={{ padding: '12px', borderRadius: 999, background: 'rgba(239,68,68,0.06)', border: '1px solid rgba(239,68,68,0.12)', cursor: 'pointer', textAlign: 'center', fontSize: 12, fontWeight: 600, color: 'rgba(239,68,68,0.5)' } as any}>Supprimer l'appareil</div>
          </div>
        </div>
      )}
      ...
      {selected === 'scale' && (
        <div onClick={() => setSelected(null)} style={{ position: 'fixed', inset: 0, zIndex: 9999, backdropFilter: 'blur(32px)', WebkitBackdropFilter: 'blur(32px)', background: 'rgba(0,0,0,0.4)', overflowY: 'auto' } as any}>
          <div onClick={(e: any) => e.stopPropagation()} style={{ width: '100%', maxWidth: 420, margin: '0 auto', padding: '40px 24px 120px', boxSizing: 'border-box' } as any}>
            ...
            <div onClick={() => { setSelected(null); onStartWeighing?.(); }} style={{ marginTop: 12, padding: '14px', borderRadius: 999, background: '#FFF', cursor: 'pointer', textAlign: 'center', fontSize: 14, fontWeight: 700, color: '#111' } as any}>
