...
export default function WeighingFlow({ onClose, d = {}, weighings = [] }: Props) {
  ...
  return (
    <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, zIndex: 99999, backdropFilter: 'blur(32px)', WebkitBackdropFilter: 'blur(32px)', background: 'rgba(0,0,0,0.3)', overflowY: 'scroll' } as any}>
      <div style={{ width: '100%', maxWidth: 400, margin: '0 auto', padding: '40px 28px 120px', boxSizing: 'border-box' } as any}>
        ...
        {step === 1 && (
          <div style={{ textAlign: 'center' } as any}>
            ...
            <div onClick={() => setStep(2)} style={{ marginTop: 20, padding: '16px', borderRadius: 999, background: '#FFF', cursor: 'pointer', fontSize: 15, fontWeight: 800, color: '#111' } as any}>Je suis pret</div>
          </div>
        )}
        ...
        {step === 2 && (
          <div style={{ textAlign: 'center' } as any}>
            ...
            <div onClick={() => { setStep(3); setTimeout(() => setStep(4), 10000); }} style={{ marginTop: 24, padding: '14px', borderRadius: 999, background: 'rgba(56,189,248,0.15)', border: '1px solid rgba(56,189,248,0.25)', cursor: 'pointer', fontSize: 13, fontWeight: 700, color: '#38BDF8' } as any}>Simuler la connexion</div>
          </div>
        )}
        ...
        {step === 4 && (() => {
          ...
          return (
            <div style={{ textAlign: 'center' } as any}>
              ...
              <div onClick={onClose} style={{ padding: '16px', borderRadius: 999, background: '#FFF', cursor: 'pointer', fontSize: 15, fontWeight: 800, color: '#111' } as any}>Voir le rapport de sante</div>
            </div>
          );
        })()}
      </div>
